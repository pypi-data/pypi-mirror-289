...
class PgEncrypt(Func):
    ...
    #     self.extra['key'] = self.key
    #     return super().as_sql(compiler, connection)
    
class PgDecrypt(Func):
...
class EncryptedCharField(models.TextField):
    def __init__(self, *args, **kwargs):
        ...
        self.key = self.all_keys[0].strip() if self.all_keys else settings.PGCRYPTO_DEFAULT_KEY.strip()
        self.schema = DatabaseUtils.get_current_schema()
        super().__init__(*args, **kwargs)

    def get_prep_value(self, value):
        if value is not None:
            return PgEncrypt(value, key=self.key)
        return value

    def from_db_value(self, value, expression, connection):
        if value is not None:
            return PgDecrypt(value, key=self.key)
        return value
...
class EncryptedIntegerField(models.BinaryField):
    ...
    def get_prep_value(self, value):
        if value is not None:
            # Convert the integer to a string for encryption
            return PgEncrypt(str(value), key=self.key)
        return value

    def from_db_value(self, value, expression, connection):
        if value is not None:
            # Decrypt and convert the result back to an integer
            decrypted_value = PgDecrypt(value, key=self.key)
