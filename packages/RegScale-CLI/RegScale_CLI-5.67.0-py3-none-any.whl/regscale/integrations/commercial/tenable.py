...
def fetch_vulns(query_id: int = 0, regscale_ssp_id: int = 0) -> Tuple[list, "DataFrame"]:
    ...
    if query_id and client._env_base == "TSC":
        ...
        consume_iterator_to_file(vulns, query_id, regscale_ssp_id)


def consume_iterator_to_file(iterator: AnalysisResultsIterator, query_id: int, ssp_id: int) -> None:
    """
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        process_list = []
        for dat in iterator:
            process_list.append(dat)
            if len(process_list) == len(iterator.page):
                executor.submit(
                    process_sc_chunk(
                        vulns=process_list,
                        ...
                        query_id=query_id,
                        ssp_id=ssp_id,
                        create_issue_from_recommendation=True,
                        app=app,
                    )
                )
                # reset list
                process_list = []
                logger.debug("Processing page %s of Tenable SC data...", page_number)
        page_number += 1
...
def process_sc_chunk(**kwargs) -> None:
    ...
    for vuln in tenable_vulns:
        findings.update(sc.parse_findings(vuln))
