...
class IntegrationIssue(ABC):
    ...
    def create_or_update_issues(
        ...
        insert_issues = [item for item in issues if item not in existing_issues]
        update_issues: Set[Issue] = set()
        for issue in issues:
            if issue in existing_issues:
                # get index of the issue
                ix = existing_issues.index(issue)
                issue.id = existing_issues[ix].id
