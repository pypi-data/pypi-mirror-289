...
class Defender(FlatFileImporter):
    ...
    def create_issue(self, **kwargs: dict) -> Optional[Issue]:
        ...
        kev_due_date = None
        if self.attributes.app.config["issues"]["defenderFile"]["useKev"]:
            kev_due_date = self.lookup_kev(cves[0] if dat.get("CVE") else None)
        iss = Issue(
        ...
        iss = self.update_due_dt(iss=iss, kev_due_date=kev_due_date, scanner="defender", severity=severity)
