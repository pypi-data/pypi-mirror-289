...
class wrapper_jira:
    ...
    def _miro_factory(self, issue: dict):
        ...
        sintese = []
        cfop = None
        if 'fornecedor' in issue["domain_data"] and len(issue["domain_data"]['fornecedor']['cfop']) > 0:
            cfop = issue["domain_data"]['fornecedor']['cfop']
        elif 'cfop_fornecedor' in issue["domain_data"] and len(issue["domain_data"]['cfop_fornecedor']['cfop']) > 0:
