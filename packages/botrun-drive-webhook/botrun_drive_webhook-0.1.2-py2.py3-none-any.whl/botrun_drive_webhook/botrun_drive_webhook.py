...
def botrun_drive_webhook(service_account_file, 
                         ...
                         notification_info_path="./users/botrun_drive_webhook/notification_info.json"
                        ):
    try:
...
def init_google_drive_webhook(service, 
    ...
    try:
        os.makedirs("./users/botrun_drive_webhook/", exist_ok=True)
...
def get_update_queue(time=None):
    env_data_dir = os.getenv("BOTRUN_DRIVE_WEBHOOK_FOLDER", "./users/botrun_drive_webhook/")
    ...
    updated_list, latest_file = file_update_queue.process_update_queue()
...
def register_folder_ids_from_csv(csv_url, file_update_queue, cache_dir="./users/botrun_drive_webhook/cache", cache_duration=24):
    # 創建緩存目錄（如果不存在）
    ...
    with open(cache_file, 'r', newline='', encoding='utf-8') as f:
        ...
        for row in csv_reader:
            ...
            if collection_id:
                file_update_queue.register_folder_ids(collection_id)
...
def main(*args):
    ...
    #################### 改成讀取 google sheet ####################!!!!!!!!!!!!!!!!!!!!
    csv_url=os.getenv("BOTRUN_DRIVE_WEBHOOK_REG_FOLDERIDS") 
    #file_update_queue.register_folder_ids(os.getenv("BOTRUN_DRIVE_WEBHOOK_DEFAULT_FOLDERID")) # 改成讀取 google sheet
    ...
    env_service_account_file = os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
    print(f"env_service_account_file22222222={env_service_account_file}")
