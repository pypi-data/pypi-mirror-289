    ...
    def search_bible(self, delimiter_verse, tbbname, bnumber, cnumber):
        ...
        try:
            ...
            for c in cu.fetchall():
                ...
                text = "{6}{8}{0} {1}.{2}{7}: {3}{4}{5}".format(c[4], cnumber, c[1], c[2], delimiter_verse, extra_delimiter, before_delimiter, cr_tot, mark)
                ref = [c[0], c[6]]
                item = [text, ref]
                lst.append(item)
...
ORDER BY c.crId ASC, bbNameOrder ASC'''
                ...
                for c in cu.fetchall():
                    ...
                    # BAK OF ORIG: text = "{6}{8}{0} {1}.{2}{7}: {3}{4}{5}".format(c[7], c[3], c[4], c[5], delimiter_verse, extra_delimiter, before_delimiter, cr_tot, mark)
                    text = "{6}{8}{0} {1}.{2}{7}: {3}{4}{5}".format(c[7], c[3], c[4], c[5], delimiter_verse, extra_delimiter, before_delimiter, cr_tot, mark)
                    ref = [c[0], c[1]]
                    item = [text, ref]
                    lst.append(item)
                    # sys.stdout.write("*** {0}".format(text))
                    # sys.stdout.write("*** {0}".format(text)) => NOK WITH ORIG
                    # sys.stdout.write("*** {0}".format(c[5])) => OK, NOK with text orig
