...
class ModelConfig(StrictModel):
    ...
    def run(self, mock_inputs: Dict[str, List[Any]]) -> pandas.DataFrame:
        ...
        model_file_path = Path(".").resolve() / self.model_file
        artifact_files = self.artifact_files or []
        all_file_paths = [model_file_path, *[Path(file).resolve() for file in artifact_files]]
        model_utils.validate(
            archive_root_path=Path(".").resolve(), model_file_path=model_file_path, files=all_file_paths
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            os.makedirs(Path(tmpdir) / os.path.dirname(self.model_file), exist_ok=True)
            shutil.copy(model_file_path, Path(tmpdir) / self.model_file)
