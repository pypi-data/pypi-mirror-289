...
class MaterializedFeatureView(FeatureView):
    ...
    def get_partial_aggregates(
        ...
        """
        Returns the partially aggregated tiles in between `start_time` and `end_time` for a Feature View
        that uses the Tecton Aggregation Engine.
        ...
        multiple sources, mocking some data sources and using raw data for others is supported.
        :type mock_inputs: Optional[Dict[str, Union[pandas.DataFrame, pyspark_dataframe.DataFrame]]]
...
class Prompt(_RealtimeOrPromptBase):
    ...
    def _get_schemas(self) -> _Schemas:
        view_schema = Schema.from_dict({"prompt": StringType()}).to_proto()
