...
def _custom_model_predict(model: CustomModelContainer, input_batch: pyarrow.RecordBatch) -> torch.Tensor:
    try:
    ...
    logger.warning("Failed to run model after multiple attempts. Bisecting batch and trying again.")
    return _custom_model_predict_with_bisect(model=model, input_batch=input_batch)


def _custom_model_predict_with_bisect(model: CustomModelContainer, input_batch: pyarrow.RecordBatch) -> torch.Tensor:
    ...
    return torch.cat([model.predict(first_half), model.predict(second_half)])


def _convert_pytorch_to_arrow(
    output: torch.Tensor, num_of_rows: int, output_column_type: pyarrow.DataType
) -> pyarrow.Array:
    ...
    if output.dim() == 2 and not isinstance(output_column_type, pyarrow.ListType):
