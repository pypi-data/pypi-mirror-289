...
from psycopg.rows import AsyncRowFactory, TupleRow, dict_row, tuple_row
...
def _ex_result(
    ...
    assert ex.pgresult.command_status is not None
    exec_status = ExecStatus(ex.pgresult.status)
    command_status = ex.pgresult.command_status.decode()
...
async def export_json(
    ...
    start = time.monotonic()
    async with conn.cursor() as cursor:
        cursor.row_factory = _row_factory(format)
        await cursor.execute(query.encode())
...
def _row_factory(format: ExportFormat) -> AsyncRowFactory[Any]:
