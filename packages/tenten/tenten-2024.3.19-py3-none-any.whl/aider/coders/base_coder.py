...
class Coder:
    ...
    def get_announcements(self):
        ...
        lines.append("Model: " + main_model.name)
        lines.append("Weak model: " + weak_model.name)
        lines.append("Edit format: " + self.edit_format)

        # Repo
        if self.repo:
            rel_repo_dir = self.repo.get_rel_repo_dir()
            num_files = len(self.repo.get_tracked_files())
            lines.append(f"Git: {rel_repo_dir}, {num_files:,} files")
            if num_files > 1000:
                lines.append(
        ...
        if self.repo_map:
            ...
            if map_tokens > 0:
                lines.append(f"Repo-map: max {map_tokens} tokens")
                max_map_tokens = 2048
                if map_tokens > max_map_tokens:
                    lines.append(
                        f"Warning: map-tokens > {max_map_tokens} is not recommended as too much"
                        " irrelevant code can confuse LLM."
                    )
            else:
