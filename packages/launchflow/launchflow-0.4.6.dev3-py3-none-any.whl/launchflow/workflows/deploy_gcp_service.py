...
_gce_service_healthy_timeout = timedelta(minutes=20)


async def poll_mig_updating(client: Any, mig_name: str, project: str, region: str):
    def get_mig():
        return client.get(
    ...
    while not mig.status.is_stable:
        now = time.time()
        if now - start_time > _gce_service_healthy_timeout.total_seconds():
            raise exceptions.GCEServiceNotHealthyTimeout(_gce_service_healthy_timeout)
        await asyncio.sleep(2)
        mig = await loop.run_in_executor(None, get_mig)
