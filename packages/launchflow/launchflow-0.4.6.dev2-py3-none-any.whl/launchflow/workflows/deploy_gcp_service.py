...
from google.cloud.compute_v1.services import disks  # type: ignore

from launchflow import exceptions
from launchflow.config import config
from launchflow.gcp.cloud_run import CloudRun
from launchflow.gcp.compute_engine import ComputeEngineService
...
async def release_docker_image_to_cloud_run(
    ...
    service = await client.get_service(
        name=f"projects/{gcp_environment_config.project_id}/locations/{gcp_environment_config.default_region}/services/{cloud_run_service._cloud_run_service_container.resource_id}"
    )
...
async def release_docker_image_to_compute_engine(
    ...
    template_op = template_client.insert(
        project=gcp_environment_config.project_id, instance_template_resource=template
    )
    template_op.result()  # type: ignore

    group_client = compute.RegionInstanceGroupManagersClient()
    set_op = group_client.set_instance_template(
    ...
    update_op = group_client.apply_updates_to_instances(
        project=gcp_environment_config.project_id,
        region=region,
        instance_group_manager=compute_engine_service._mig.resource_id,
        region_instance_group_managers_apply_updates_request_resource=compute.RegionInstanceGroupManagersApplyUpdatesRequest(
            # TODO: we probably want to allow the user to control how this happens
            all_instances=True
        ),
    )
