...
class SqlSession(object):
    ...
    def set_user_password(self, user_name, password, valid_until = None):
        ...
        if valid_until is not None:
            self.execute("ALTER USER %s WITH PASSWORD %s VALID UNTIL '%s';" % (user_name, escaped_password, valid_until.isoformat()))
        else:
            self.execute("ALTER USER %s WITH PASSWORD %s;" % (user_name, escaped_password))
