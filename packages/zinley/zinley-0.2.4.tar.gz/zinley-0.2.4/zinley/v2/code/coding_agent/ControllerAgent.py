...
class ControllerAgent:
    def __init__(self, directory_path, api_key, endpoint, deployment_id, max_tokens, scheme):
        ...
        self.techExplainer = TechnicalExplainerAgent(os.path.join(directory_path, "Zinley/Project_analysis"), api_key, endpoint, deployment_id, max_tokens)
    ...
    async def totalScan(self):
        """Scan updated files and log the updates."""
        print(f"Re-scanning in progress....")
    ...
    async def explainer_task_pipeline(self, files, finalPrompt, language, role):
        ...
        print(await self.get_explainer(files, finalPrompt, language, role))
