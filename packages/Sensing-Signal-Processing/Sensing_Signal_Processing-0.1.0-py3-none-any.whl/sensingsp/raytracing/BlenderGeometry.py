            ...
            if obj.type == 'MESH':
                if obj.name.startswith('Probe_')==False:
                    ...
                    for face in bm.faces:
                        ...
                        fn.normalize()
                        face_center_all.append([face_center,obj_hash,face.index,face.calc_area(),fn,RCS0])
    ...
    def get_Suite_Position(self, suite_information,depsgraph):
        ...
        for suite in suite_information:  # suite_planes[i]['radars'][j]["TXs"]
            ...
            for radar in suite['Radar']:
                ...
                WaveLength=radar['GeneralRadarSpec_Object']['Lambda_mm']
                Pt = 10**((radar['GeneralRadarSpec_Object']['Transmit_Power_dBm']-30)/10)
                Pt /= len(radar['TX'])
                Gt = 10**((radar['GeneralRadarSpec_Object']['Transmit_Antenna_Element_Gain_dbm']-30)/10)
                Gr = 10**((radar['GeneralRadarSpec_Object']['Receive_Antenna_Element_Gain_dbm']-30)/10)
