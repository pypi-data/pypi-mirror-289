...
from florestdevinstruments.ready_games import russian_rullet
def piroschock_rulletcka1(code:int):
    """Дорогой друг, с помощью данной функции вы можете поиграть в Азартную Игру, под названием Russian Ruletka!!! Спасибо, что используете мою библиотеку!!!"""
    return russian_rullet(code)
from florestdevinstruments.working_api import VK 
def piroschock_yt(token: str, id: int) -> VK: 
    """!!! Внимание !!! Данная функция возвращает объект во VKонтакте!"""
    return VK(token,id)
def joke():
    """С помощью данной функции вы можете запросить любую шуточку! АХАХАХАХХААХАХ"""
    from florestdevinstruments.florest_utilits import get_py_joke 
    return get_py_joke()
def minecraft(ip: str):
    """Дорогой друг! Данная функция помогает отслеживать статиску и информацию о сервере в игре Minecraft!!!"""
    from florestdevinstruments.florest_utilits import info_about_java_server
    return info_about_java_server(ip)
def webhooksender(url:str):
    """Данная функция имеет взаимодействие с веб-хуками!"""
    from florestdevinstruments.working_api import Discord
    return Discord.WebhookSender(url)