...
class GUI:
    def __init__(self):
        ...
        self.frame1=CTkFrame(self.main,width=1280,height=100,corner_radius=0,border_color='darkgrey',border_width=1)
        ...
        button_addpoint = CTkButton(self.frame1,text='ADD\nNODE',height=80,width=80,command=self.callback_button_addpoint)
        ...
        button_clear = CTkButton(self.frame1,text='CLEAR',height=80,width=40,command=self.callback_clear,fg_color='red',hover_color='darkred')
        button_clear.place(x=1220,y=10)


        button_addnode = CTkButton(self.frame1,text='+',height=35,width=35,command=self.callback_addnode,fg_color='green',hover_color='darkgreen',font=('Arial',20))
        button_addnode.place(x=340,y=15)
        button_subtractnode = CTkButton(self.frame1,text='-',height=35,width=35,command=self.callback_subtractnode,fg_color='red',hover_color='darkred',font=('Arial',20))
        button_subtractnode.place(x=340,y=55)
        self.varp1=StringVar(value=0)
        ...
        button_solve=CTkButton(self.frame1,text='SOLVE',height=80,width=80,command=self.callback_solve,fg_color='orange',hover_color='darkorange')
    ...
    def callback_button_addpoint(self):
        ...
        button_deletepoint=CTkButton(frame_entry,text='Delete',width=50,height=80,command=lambda frm=frame_entry: self.callback_delete_point(frm))
    ...
    def callback_addnode(self):
        if (int(self.varp1.get()),int(self.varp2.get())) not in self.conn and (int(self.varp2.get()),int(self.varp1.get())) not in self.conn :
            self.conn.append((int(self.varp1.get()),int(self.varp2.get())))

        plt.cla()
        self.ax1.plot(self.coord[:,0],self.coord[:,1],'.k',markersize=10)
        for n,i in enumerate(self.coord):
            self.ax1.text(i[0],i[1],str(n+1))
        self.ax1.set_xlabel('x')
        self.ax1.set_ylabel('y')
        self.ax1.set_title('Deformation Plot')
        self.ax1.axis('equal')
        for i in self.conn:
            self.ax1.plot([self.coord[i[0]-1,0],self.coord[i[1]-1,0]],[self.coord[i[0]-1,1],self.coord[i[1]-1,1]],'-b')
        self.canvas1.draw()
        
    def callback_subtractnode(self):
        for i in self.conn:
            if (int(self.varp1.get()),int(self.varp2.get()))==i or (int(self.varp2.get()),int(self.varp1.get()))==i:
                self.conn.remove(i)
    ...
    def callback_solve(self):
        ...
        for i in self.conn:
            self.ax1.plot([self.coord[i[0]-1,0],self.coord[i[1]-1,0]],[self.coord[i[0]-1,1],self.coord[i[1]-1,1]],'-b')
        for i in self.conn:
            self.ax1.plot([self.coord[i[0]-1,0]+U[i[0]-1,0],self.coord[i[1]-1,0]+U[i[1]-1,0]],[self.coord[i[0]-1,1]+U[i[0]-1,1],self.coord[i[1]-1,1]]+U[i[1]-1,1],'--r')
        self.canvas1.draw()
        ...
        dt=['Node','Ux','Uy']
        for j in range(2):
                txt=CTkEntry(self.summary,corner_radius=0,border_color='grey',justify=CENTER)
                txt.insert(END,dt[j])
