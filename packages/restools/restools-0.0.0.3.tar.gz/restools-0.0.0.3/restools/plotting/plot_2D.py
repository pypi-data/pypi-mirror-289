...
def plot_2D(ax, x, y, color, lab, y_min=None, y_max=None, y_var=None, is_smoothed=False):
    if(y_min is None):
        clip_min = -1.0e+10
    else:
        clip_min = y_min
    ...
    ax.plot(x, numpy.clip(y_mean, clip_min, clip_max), '-', color=color, label=lab)

    if(is_smoothed):
        ax.plot(x, numpy.clip(y, clip_min, clip_max), '-', color=color, alpha=0.10)
...
if __name__=="__main__":
    ...
    for cmd_idx, cmd in enumerate(args.inputs):
        ...
        if(args.smooth == 'false'):
            plot_2D(ax, x, y, color, label, y_min=args.y_min, y_max=args.y_max, y_var=yvar, is_smoothed=False)
        else:
            plot_2D(ax, x, y, color, label, y_min=args.y_min, y_max=args.y_max, y_var=yvar, is_smoothed=True)
