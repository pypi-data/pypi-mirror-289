...
class LDataFS(Fuse):
    ...
    def mkdir(self, path: str, mode: int):
        ...
        try:
            ...
            if parent_id is None:
                self.req_thread.call(
                    self.load_subtree_by_path_async(
                        str(PurePosixPath(path).parent), use_cache=False
                    ),
                    10,
                )
            else:
                with self.cache_lock:
                    del self.ldata_node_cache[parent_id]
    ...
    def mknod(self, path: str, mode: int, device: int):
        ...
        # invalidate the parent, so that if someone needs to use this node, they have to refresh cache
        parent_id = self.ldata_path_to_node_cache.get(str(PurePosixPath(path).parent))
        if parent_id is None:
            self.req_thread.call(
                self.load_subtree_by_path_async(
                    str(PurePosixPath(path).parent), use_cache=False
