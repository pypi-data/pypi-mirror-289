...
void init_Machine(){
    ...
    ACM.x[INDEX_THETA_D_ACCUM] = 0*15.0 / 180.0 * M_PI / INIT_NPP; // 我们现在的QEP模型，要在测量的地方加IPD offset才有效
    ...
    // MOTOR_PERMANENT_MAGNET_FLUX_LINKAGE 是指永磁体磁链的幅值，所以在恒功率坐标系下仿真时要经过恒功率变换，Clarke变换后得到的永磁体磁链向量的长度长于相坐标系下的，所以差一个系数sqrt(1.5)。
    // ACM.KE = MOTOR_PERMANENT_MAGNET_FLUX_LINKAGE * AMPL2POW; // Vs/rad 
    ACM.KE = INIT_KE; // Vs/rad 

    ACM.npp = INIT_NPP;
    ACM.npp_inv = 1.0 / ACM.npp;
    ACM.Js = INIT_JS * (1.0+LOAD_INERTIA);
    ...
    ACM.psi_stator[0] = INIT_KE; // TODO：这里假设了初始位置是alpha轴对着d轴的
...
void SM_Dynamics(double t, double *x, double *fx){
    ...
    // 机械子系统
    // 电磁转矩 Tem 计算
    ACM.Tem = CLARKE_TRANS_TORQUE_GAIN * ACM.npp * ( ACM.KE + (ACM.Ld - ACM.Lq)*x[0] ) * x[1]; // AMPL2POW*AMPL2POW = CLARKE_TRANS_TORQUE_GAIN
