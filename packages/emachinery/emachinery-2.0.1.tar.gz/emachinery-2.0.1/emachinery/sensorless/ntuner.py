...
def InstaSPIN_series_PI_tuner(
    ...
    bool_render=False):
    KT = 1.5*npp*KE
    currentKp = CLBW_Hz * 2 * np.pi * L
    currentKi = R / L
    speedKi = 2*np.pi * CLBW_Hz / delta**2 # THIS IS INTEGRAL GAIN
    speedKp = Js/npp / KT * delta * speedKi
    (mag, phase, omega), VLBW_Hz, output = getBodePlot(CLBW_Hz, currentKp, currentKi, speedKi, speedKp, L, KT, npp, Js, bool_render=bool_render)
    input = {
        'delta': delta,
        'CLBW_Hz': CLBW_Hz,
        'L': L,
    ...
    return VLBW_Hz, currentKp, currentKi, speedKp, speedKi, (mag, phase, omega), input, output
