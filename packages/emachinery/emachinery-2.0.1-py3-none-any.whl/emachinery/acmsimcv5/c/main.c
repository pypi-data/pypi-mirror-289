...
int main(){
    ...
    for(_=0;_<NUMBER_OF_STEPS*TS_UPSAMPLING_FREQ_EXE_INVERSE;++_)
    {
        ...
        if(++dfe_counter == TS_UPSAMPLING_FREQ_EXE_INVERSE)
        {
            ...
            #else
                // 生成转速指令
                REAL set_rpm_speed_command=50.0, set_iq_cmd=0.0, set_id_cmd=0.0;
                int set_current_loop = FALSE, flag_overwrite_theta_d = FALSE; REAL Overwrite_Current_Frequency=0.0;
                // printf("%d", flag_overwrite_theta_d);
                // printf("%d", flag_overwrite_theta_d);
                // printf("%d", flag_overwrite_theta_d);
                commands(&set_rpm_speed_command, &set_iq_cmd, &set_id_cmd,
                        &set_current_loop, &flag_overwrite_theta_d, &Overwrite_Current_Frequency);
                controller(set_rpm_speed_command, set_current_loop, set_iq_cmd, set_id_cmd, flag_overwrite_theta_d, Overwrite_Current_Frequency, 0.0, 0.0);
            #endif
            // printf("%.3f - %g\n", (*CTRL).timebase, ACM.rpm);
            // 电压指令(*CTRL).O->uab_cmd[0/1]通过逆变器，产生实际电压ACM.ual, ACM.ube（变换到dq系下得到ACM.ud，ACM.uq）
