...
void init_CTRL()
{
    ...
    // elec
    (*CTRL).motor->R = INIT_R;
    (*CTRL).motor->KE = INIT_KE; // * (0.1/0.1342); // 【实验编号：】
    (*CTRL).motor->Ld = INIT_LD;
    (*CTRL).motor->Lq = INIT_LQ;
...
#endif
    // mech
    (*CTRL).motor->npp = INIT_NPP;
    (*CTRL).motor->npp_inv = 1.0 / (*CTRL).motor->npp;
    (*CTRL).motor->Js = INIT_JS * (1.0 + LOAD_INERTIA);
...
#define MOTOR (*(*CTRL).motor)
    ...
    if ((*CTRL).S->the_vc_count++ >= SPEED_LOOP_CEILING)
    {
        ...
        // 速度信号通通变成机械角速度 mech
        pid1_spd.Ref = (*CTRL).I->cmd_omg_elec * MOTOR.npp_inv;
        pid1_spd.Fbk = (*CTRL).I->omg_elec * MOTOR.npp_inv;

#ifdef USE_BEZIER
        control_output(&pid1_spd, &BzController);
#else
        // 使用线性控制器
        // pid1_spd.Ki = 0;
        // pid1_spd.Kp = 0;
...
// 磁链环
    ...
    // TODO：这里需要1.5倍吗？
    // (*CTRL).I->Tem     = 1.5 * MOTOR.npp * (MOTOR.KE + (MOTOR.Ld-MOTOR.Lq)*(*CTRL).I->idq[0]) * (*CTRL).I->idq[1];
    // (*CTRL).I->Tem_cmd = 1.5 * MOTOR.npp * (MOTOR.KE + (MOTOR.Ld-MOTOR.Lq)*(*CTRL).I->idq_cmd[0]) * (*CTRL).I->idq_cmd[1];
    (*CTRL).I->Tem = MOTOR.npp * (MOTOR.KE + (MOTOR.Ld - MOTOR.Lq) * (*CTRL).I->idq[0]) * (*CTRL).I->idq[1];
    (*CTRL).I->Tem_cmd = MOTOR.npp * (MOTOR.KE + (MOTOR.Ld - MOTOR.Lq) * (*CTRL).I->idq_cmd[0]) * (*CTRL).I->idq_cmd[1];
...
/// 5.Sweep 扫频将覆盖上面产生的励磁、转矩电流指令
    ...
    pid1_iq.Ref = (*CTRL).I->idq_cmd[1];
    if (set_current_loop == 1)
    {
        pid1_iq.Ref = set_iq_cmd;
        (*CTRL).I->idq_cmd[1] = set_iq_cmd;
    }
                ...
                REAL angle_shift_for_second_inverter)
{
    ...
    /*最终生成cmd_omg_elec单位为rad/s，国际单位好评！*/
    ...
    if (flag_overwrite_theta_d)
    {
        if (fabs(Overwrite_Current_Frequency) > 0)
        {
            ...
            if (used_theta_d_elec > M_PI)
                used_theta_d_elec -= 2 * M_PI;
            if (used_theta_d_elec < -M_PI)
                used_theta_d_elec += 2 * M_PI;
        }
        else
    ...
    (*CTRL).S->cosT = cosf(used_theta_d_elec + angle_shift_for_first_inverter);
    (*CTRL).S->sinT = sinf(used_theta_d_elec + angle_shift_for_first_inverter);
    ...
    // (*CTRL).S->cosT2 = cosf(used_theta_d_elec + angle_shift_for_second_inverter);
    // (*CTRL).S->sinT2 = sinf(used_theta_d_elec + angle_shift_for_second_inverter);
