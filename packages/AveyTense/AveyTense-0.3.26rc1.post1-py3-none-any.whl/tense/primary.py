...
import tense.tcs as tcs, json, tkinter as tk, math, os, hashlib as ha, typing as tp, collections as ct, io, random as ra, warnings as wa
...
_SupportsAbs = tcs.FloatOrInteger
_ReleaseLevel = tcs.Literal["alpha", "beta", "candidate", "final"]
...
"""
@tcs.final
class _TenseVersion(tcs.Comparable):
    """
    \\@since 0.3.25 \\
    \\@author Aveyzan
    ```ts \\
    ...
    STRING_VER = tcs.VERSION # since 0.3.26b2
    """
    \\@since 0.3.26b3 \\
    \\@author Aveyzan
    ```ts \\
...
@tcs.final
class TenseType(tcs.Generic[_T]):
    """
...
def _reckon_prepend_init(*countables: _ReckonTypePre[_T]):
    import typing as tp, io, collections as ct, tense.tcs as tcs, tkinter as tk
    i = 0
    for e in countables:
        if isinstance(e, tcs.ReckonOperable): i += e.__reckon__()
...
class Time:
    ...
    def getMillennium(self):
        ...
        """
        return self.__math.trunc(int(self.datetime.now().strftime("%Y")) / 1000) + 1
    @classmethod
    def getCentury(self):
        """
        \\@since 0.3.25 \\
        ...
        but code itself in reality verifies current date and what not.
        """
        return self.__math.trunc(int(self.datetime.now().strftime("%Y")) / 100) + 1
    @classmethod
    def getDecade(self):
        """
        \\@since 0.3.25 \\
        ...
        """
        return self.__math.trunc(int(self.datetime.now().strftime("%Y")) / 10) + 1
    @classmethod
    def getYear(self):
        """
        \\@since 0.3.25 \\
        ...
        """
        return int(self.datetime.now().strftime("%Y"))
    @classmethod
    def getMonth(self):
        """
        \\@since 0.3.25 \\
        ...
        """
        return int(self.datetime.now().strftime("%m"))
    @classmethod
    def getDay(self):
        """
        \\@since 0.3.25 \\
        ...
        """
        return int(self.datetime.now().strftime("%d"))
    @classmethod
    def getHour(self):
        """
        \\@since 0.3.25 \\
        ...
        """
        return int(self.datetime.now().strftime("%H"))
    @classmethod
    def getMinute(self):
        """
        \\@since 0.3.25 \\
...
class Math:
    ...
    """
    import math as __math, tense.tcs as __tcs
    PI = tcs.PI
    """
    \\@since 0.3.25 \\
    \\@author Aveyzan
    ...
    """
    @classmethod
    def __verify(x: _FloatOrInteger, y: tcs.Optional[_FloatOrInteger] = None):
...
class ModernString:
    ...
    @classmethod
    def sha1(self, string: tcs.Union[str, bytes, None] = None, hexdigest: bool = True):
        """
        \\@since 0.3.24 \\
        \\@author Aveyzan
        ```ts \\
        ...
        if not isinstance(hexdigest, bool):
            ...
            raise err(s)
        if string is None:
            if hexdigest: return self.__ha.sha1(bytes(self.get())).hexdigest()
            else: return self.__ha.sha1(bytes(self.get())).digest()
        elif isinstance(string, bytes):
            if hexdigest: return self.__ha.sha1(string).hexdigest()
            else: return self.__ha.sha1(string).digest()
        else:
            if hexdigest: return self.__ha.sha1(bytes(string, "utf-8")).hexdigest()
            else: return self.__ha.sha1(bytes(string, "utf-8")).digest()
    @classmethod
    def sha224(self, string: tcs.Union[str, bytes, None] = None, hexdigest: bool = True):
        """
        \\@since 0.3.24 \\
        \\@author Aveyzan
        ```ts \\
        ...
        if not isinstance(hexdigest, bool):
            ...
            raise err(s)
        if string is None:
            if hexdigest: return self.__ha.sha224(bytes(self.get())).hexdigest()
            else: return self.__ha.sha224(bytes(self.get())).digest()
        elif isinstance(string, bytes):
            if hexdigest: return self.__ha.sha224(string).hexdigest()
            else: return self.__ha.sha224(string).digest()
        else:
            if hexdigest: return self.__ha.sha224(bytes(string, "utf-8")).hexdigest()
            else: return self.__ha.sha224(bytes(string, "utf-8")).digest()
    @classmethod
    def sha256(self, string: tcs.Union[str, bytes, None] = None, hexdigest: bool = True):
        """
        \\@since 0.3.24 \\
        \\@author Aveyzan
        ```ts \\
        ...
        if not isinstance(hexdigest, bool):
            ...
            raise err(s)
        if string is None:
            if hexdigest: return self.__ha.sha256(bytes(self.get())).hexdigest()
            else: return self.__ha.sha256(bytes(self.get())).digest()
        elif isinstance(string, bytes):
            if hexdigest: return self.__ha.sha256(string).hexdigest()
            else: return self.__ha.sha256(string).digest()
        else:
            if hexdigest: return self.__ha.sha256(bytes(string, "utf-8")).hexdigest()
            else: return self.__ha.sha256(bytes(string, "utf-8")).digest()
    @classmethod
    def sha384(self, string: tcs.Union[str, bytes, None] = None, hexdigest: bool = True):
        """
        \\@since 0.3.24 \\
        \\@author Aveyzan
        ```ts \\
        ...
        if not isinstance(hexdigest, bool):
            ...
            raise err(s)
        if string is None:
            if hexdigest: return self.__ha.sha384(bytes(self.get())).hexdigest()
            else: return self.__ha.sha384(bytes(self.get())).digest()
        elif isinstance(string, bytes):
            if hexdigest: return self.__ha.sha384(string).hexdigest()
            else: return self.__ha.sha384(string).digest()
        else:
            if hexdigest: return self.__ha.sha384(bytes(string, "utf-8")).hexdigest()
            else: return self.__ha.sha384(bytes(string, "utf-8")).digest()
    @classmethod
    def sha512(self, string: tcs.Union[str, bytes, None] = None, hexdigest: bool = True):
        """
        \\@since 0.3.24 \\
        \\@author Aveyzan
        ```ts \\
        ...
        if not isinstance(hexdigest, bool):
            ...
            raise err(s)
        if string is None:
            if hexdigest: return self.__ha.sha512(bytes(self.get())).hexdigest()
            else: return self.__ha.sha512(bytes(self.get())).digest()
        elif isinstance(string, bytes):
            if hexdigest: return self.__ha.sha512(string).hexdigest()
            else: return self.__ha.sha512(string).digest()
        else:
            if hexdigest: return self.__ha.sha512(bytes(string, "utf-8")).hexdigest()
            else: return self.__ha.sha512(bytes(string, "utf-8")).digest()
    @classmethod
    def md5(self, string: tcs.Union[str, bytes, None] = None, hexdigest: bool = True):
        """
        \\@since 0.3.24 \\
        \\@author Aveyzan
        ```ts \\
        ...
        if not isinstance(hexdigest, bool):
            ...
            raise err(s)
        if string is None:
            if hexdigest: return self.__ha.md5(bytes(self.get())).hexdigest()
            else: return self.__ha.md5(bytes(self.get())).digest()
        elif isinstance(string, bytes):
            if hexdigest: return self.__ha.md5(string).hexdigest()
            else: return self.__ha.md5(string).digest()
        else:
            if hexdigest: return self.__ha.md5(bytes(string, "utf-8")).hexdigest()
            else: return self.__ha.md5(bytes(string, "utf-8")).digest()
    @classmethod
    ...
    def replaceAll(self, oldStrings: _SupportsModernReplace, newStrings: _SupportsModernReplace, applyOn = REPLACE_ALL, /):
        ...
        # 2nd param is string - cause magic randomizer!
            ...
            tmp3 = tmp1[ra.randint(0, _reckon_init(tmp1) - 1)]
    ...
    @classmethod
    def sanitize(self, string: tcs.Optional[str] = None, mode: _SanitizeMode = SANITIZE_AROUND) -> str:
        """
...
_ReckonType = tcs.Union[_ReckonTypePre[_T], ModernString]
_AbroadValue1 = tcs.Union[_AbroadValue1Pre[_T], ModernString]
_AbroadValue2 = tcs.Union[_AbroadValue2Pre[_T], ModernString]
_AbroadModifier = tcs.Union[_AbroadModifierPre[_T], ModernString]
...
class Reckon:
    ...
    def get(self, specific: tcs.Optional[int] = None, /):
        """
        \\@since 0.3.25 (standard since 0.3.25) \\
        \\@author Aveyzan
        ```ts \\
...
class protogen(tp.MutableSequence[int]):
    """
...
class YamiTk:
    ...
    """
    import tkinter as __tk, tense.tcs as __tcs
    __loc_tk = None
    __loc_frame = None
    __loc_label = None
    __loc_button = None
    ...
    def __init__(self, screenName: tcs.Optional[str] = None, baseName: tcs.Optional[str] = None, className: str = "Tk", useTk: bool = True, sync: bool = False, use: tcs.Optional[str] = None) -> None:
        self.__loc_tk = tk.Tk(
    ...
    def mainloop(self, n = 0):
        ...
        if self.__loc_tk is not None:
            ...
            err, s = (self.__tcs.NotInitializedError, f"Class '{__class__.__name__}' was not initialized.")
            raise err(s)
        return self
...
# not for export
del os, json, tk, ra, wa, tp, io, ct, RAM
