...
If downloading `typing_extensions` module went unsuccessfully, try to manually running `pip install typing_extensions`.
"""
...
import os, tkinter as tk, typing as tp, types as ty, io, math as ma, collections as ct, warnings as wa, functools as fn, abc, collections.abc as abc2, asyncio as aio, subprocess as sub, enum, inspect as ins
...
try:
    ...
    sub.run(["pip", "install", "typing_extensions"])
else:
    import typing_extensions as tpe
...
VERSION_LIST = (
    ...
    "0.3.26a4", "0.3.26b2", "0.3.26b3", VERSION
)
...
IntegerFlag = enum.IntFlag
"\\@since 0.3.26c1"
IntegerEnum = enum.IntEnum
"\\@since 0.3.26c1"
Enum = enum.Enum
"\\@since 0.3.26c1"
...
class BitwiseOperable(BitwiseAndOperable[_any, _any], BitwiseOrOperable[_any, _any], BitwiseXorOperable[_any, _any], BitwiseLeftOperable[_any, _any], BitwiseRightOperable[_any, _any]):
    """
    \\@since 0.3.26c1
...
class BitwiseReassignable(BitwiseAndOperable[_any, _any], BitwiseOrOperable[_any, _any], BitwiseXorOperable[_any, _any], BitwiseLeftReassignable[_any, _any], BitwiseRightReassignable[_any, _any]):
    """
    \\@since 0.3.26c1
...
class BitwiseCollection(BitwiseReassignable, BitwiseOperable):
    """
    \\@since 0.3.26c1
...
class Comparable(LeastComparable[_any], GreaterComparable[_any], LeastEqualComparable[_any], GreaterEqualComparable[_any], EqualComparable[_any], InequalComparable[_any], InComparable[_any]):
    """
    \\@since 0.3.26b3
...
class LenOperable(_pro):
    ...
    An ABC with one method `__reckon__`, which equals invoking `len(self)`.
    """
...
TicTacTorBoarrd = list[list[str]] # 0.3.26b3
...
del os, ma, mmap, array, copy, tp, tpe, ty, ct, io, tk, wa, fn, abc, abc2, aio, sub, enum, ins, zi, url # Not for export
