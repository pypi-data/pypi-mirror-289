...
class rns:
    def __init__(self, val, basis):
        if isinstance(val, list) or isinstance(val, tuple):
            self.val = np.array(val, dtype=int)
    ...
    def __add__(self, other):
        if type(other) == rns:
            res = deepcopy(self)
            res.val = np.array([(int(x) + int(y)) % int(b) for x, y, b in zip(self.val, other.val, self.basis)], dtype='uint64')
            return res
        raise TypeError()
    ...
    def compose(self):
        return sum([int(x) * minv(self.modulus // int(b), int(b)) * (self.modulus // int(b)) for x, b in zip(self.val, self.basis)]) % self.modulus

    def decompose(val, basis):
        return np.array([val % b for b in basis], dtype='uint64')

    def get_basis(bit_length, size):
        basis = []
        while len(basis) < size:
            q = get_prime(bit_length)
            if False not in [q != b for b in basis]:
                basis.append(q)
        return np.array(basis, dtype='uint64')

class poly_ring:
    def __init__(self, val, size, coef_modulus):
        self.size = size

        self.coef_modulus = coef_modulus

        coef = np.zeros(self.size + 1, dtype=object)
        coef[0] = 1
        coef[-1] = 1
        self.poly_modulus = P(coef)

        self.val = self.mod(val, self.poly_modulus, self.coef_modulus)

    def __add__(self, other):
        if type(other) == rns:
            res = deepcopy(self)
            res.val = self.mod(self.val + other.val, self.poly_modulus, self.coef_modulus)
            return res
        raise TypeError()
    ...
    def __pos__(self):
        self.val = self.mod(+self.val, self.poly_modulus, self.coef_modulus)
        return self
    ...
    def mod(val, poly_modulus, coef_modulus):
        return P(np.array([int(c) % int(coef_modulus) for c in (val % poly_modulus).coef], dtype='uint64'))

def get_rand_poly(min, max, size):
    return P(np.array([get_rand(min, max) for _ in range(size)], dtype=object))

def get_int_gaussian_poly(sigma, size):
    return P(np.array(get_int_gaussian(0, sigma, size), dtype=object))
...
def keygen(N, t, q, sigma, T=2):
    ...
    params.roots = bit_reverse(params, np.array([mpow(Mth_root, i, params.t) for i in range(params.N)], dtype=object))
    params.roots_inv = np.array([minv(w, params.t) for w in params.roots], dtype=object)

    sk = poly_ring(get_rand_poly(-1, 2, params.N), params.N, params.q)

    a = poly_ring(get_rand_poly(0, params.q, params.N), params.N, params.q)
    e = poly_ring(get_int_gaussian_poly(params.sigma, params.N), params.N, params.q)
    pk = np.array([-(a * sk + e), a], dtype=object)
...
basis = rns.get_basis(20, 10)
x = [get_rand(0, pow(2, 20)) for _ in range(10)]
y = [get_rand(0, pow(2, 20)) for _ in range(10)]
x_rns = rns(x, basis)
y_rns = rns(y, basis)
...
# # v1 = np.array([(params.t // 2) - 1, -(params.t // 2) + 1], dtype=object)
# v1 = np.array([1, 2, -3], dtype=object)
# v2 = np.array([-4, 5, -6], dtype=object)
