...
from . import config
from ._event_result import _EventResult
from .exceptions import CochlSenseException, TimeoutException
from .window_result import SoundTag, WindowResult
...
class _EventClient:
    ...
    def __init__(
        ...
        self.internal_client: Optional[AudioSessionApi] = None

    def predict(self, file_path: str, timeout: Optional[float] = None) -> _EventResult:
        """
        ...
        """
        self.internal_client: AudioSessionApi = self._create_internal_client()

        # create Session
        session_id: str = self._create_session(file_path)

        # upload file chunks
        with open(file_path, 'rb') as file:
            chunk_sequence: int = 0
            while True:
                chunk: bytes = file.read(self.api_config.chunk_size.value)
                if not chunk:
        ...
        # get event results
        return self._get_all_results(session_id, timeout)

    def _create_internal_client(self) -> AudioSessionApi:
        if self.api_config and self.api_config.event_host:
            configuration = Configuration(host=self.api_config.event_host)
        else:
    ...
    def _create_session(self, file_path: str) -> str:
        ...
        content_type: str = f'audio/{file_format.lower()}'
        ...
        resp: SessionRefs = self.internal_client.create_session(
            CreateSession(
                type=AudioType('file'),
    ...
    def _upload_chunk(self, session_id: str, chunk_sequence: int, chunk: bytes):
        ...
        _resp: SessionRefs = self.internal_client.upload_chunk(
            session_id=session_id,
            chunk_sequence=chunk_sequence,
            audio_chunk=AudioChunk(encoded),
        )

    def _get_all_results(self, session_id: str, timeout: Optional[float] = None) -> _EventResult:
        retry_count: int = 0
        data_count: int = 0
        ...
        while True:
            ...
            if timeout is not None:
                if time.time() - start_time > timeout:
                    raise TimeoutException(session_id, timeout)

            response: SessionResult = self.internal_client.get_results(
                session_id=session_id,
                offset=data_count,
                limit=1024,
            )

            state: Optional[str] = response.get('state', None)
            has_more: Optional[bool] = response.get('has_more', None)
            ...
            if state == 'error':
                ...
                retrieved: List[WindowResult] = self._to_frame_results(response)
    ...
    def _get_file_format(cls, file_path: str) -> str:
        ...
        try:
            ...
            file_format: str = info.format.lower()
