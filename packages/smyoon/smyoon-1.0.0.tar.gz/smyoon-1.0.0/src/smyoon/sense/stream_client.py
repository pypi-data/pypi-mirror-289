...
from .window_result import SoundTag
...
class StreamClient:
    ...
    def __init__(
        ...
        if api_config.speaker_recognition:
            raise ValueError('speaker recognition not supported in StreamClient')
        if api_config is None:
            api_config = config.APIConfig()

        self.project_key: str = api_project_key
        self.audio_type: StreamAudioType = audio_type
    ...
    def predict(self, audio_data: bytes) -> WindowResult:
        with self.predict_lock:
            encoded: str = base64.b64encode(audio_data).decode('utf-8')

            resp: PredictResponse = self.internal_client.predict(
                self.session_id,
                PredictRequest(
            ...
            self.sequence: int = resp.next_sequence  # update sequence
    ...
    def _create_session(self) -> str:
        content_type: str = 'audio/x-raw; rate={}; format={}'.format(
            self.audio_type.sample_rate, self.audio_type.data_type.str_value)
