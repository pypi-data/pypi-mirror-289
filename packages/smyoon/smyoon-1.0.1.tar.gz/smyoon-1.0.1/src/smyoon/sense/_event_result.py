...
class _EventResult:
    ...
    def to_summarized_result(
        ...
        summarized_results: str = result_summary.minimize_details(self.window_results)
...
class ResultSummary:
    ...
    def __init__(
        ...
        self.hop_size: float = hop_size.second
        self.default_im: int = default_im
        self.tags_im: Optional[Dict[str, int]] = tags_im
        self._buffer: dict = {}
        self._file_mode: bool = False
        self._tag_name_other: str = TAG_NAME_OTHER
        self._min_im: int = 0
        if default_im == 0 and self.hop_size == WindowHop.HOP_500ms.second:
            self._min_im = -WindowHop.HOP_500ms.second

    def minimize_details(self, results: List[WindowResult] = None) -> str:
        """
        ...
        for window_result in results:
            line: str = self.minimize_details_window(window_result)
            if not line:
                continue
            output = self._append_line(output, line)

        for tag, (_, from_time, to_time) in self._buffer.items():
            line: str = f'At {from_time}-{to_time}s, [{tag}] was detected'
            output = self._append_line(output, line)
