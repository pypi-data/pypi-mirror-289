...
@click.pass_obj
def setup(env):
    """subcommands for managing workspaces for postreal"""
...
def _service(env, command):
    """List existing workspaces for postreal"""
    ...
    service_unit = f"""
[Unit]
Description=postreal Supervisor Service
...
def service(env, command):
    """Install a reset service postreal"""
...
def autocomplete(env):
    ...
    subprocess.run(
        [
            "_POSTREAL_COMPLETE=bash_source postreal > ~/.postreal-complete.bash"
        ],
        shell=True,
    )
    ...
    print("source ~/.postreal-complete.bash")
