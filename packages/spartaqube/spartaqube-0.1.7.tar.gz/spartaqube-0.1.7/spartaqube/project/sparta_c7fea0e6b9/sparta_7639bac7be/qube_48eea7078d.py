...
from project.sparta_c7fea0e6b9.sparta_cadaab6d8e import qube_b6d43ff92b as qube_b6d43ff92b
from project.sparta_c7fea0e6b9.sparta_6eb76fb447.qube_863ab3ebef import Email as Email
def sparta_d6cbf19e98(function):
	def A(request,*E,**C):
		A=request;B=_B
		...
		if not B:
			D=C.get(_H)
			if D is not None:F=qube_b6d43ff92b.sparta_015e806ddc(D);login(A,F)
		return function(A,*E,**C)
	return A
def sparta_fb40525a5b(function):
	def A(request,*B,**C):
		A=request
	...
	return A
def sparta_8ddf63f986(function):
	def A(request,*B,**C):
	...
	return A
def sparta_1a2996f2cf(function):
	def A(request,*D,**E):
		A=request;B=_C
		try:
			F=json.loads(A.body);G=json.loads(F[_I]);H=G[_H];C=qube_b6d43ff92b.sparta_015e806ddc(H)
			if C is not None:B=_B;A.user=C
		except Exception as I:print('exception pip auth');print(I)
		if B:return function(A,*D,**E)
		else:return HttpResponseRedirect(reverse(_G))
	return A
def sparta_05128eedbd(code):
	try:
...
def sparta_4800bd13a2():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_47e4f6bd23(json_data):
	D='ip_addr';A=json_data;del A[_J];del A[_K]
	try:A[D]=sparta_4800bd13a2()
	except:A[D]=-1
...
def sparta_1ec02864d3():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_04d6dd260c(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_b31a4c6939(guest_code,user_obj):
	...
	return _C
def sparta_f851f0972a(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_1c52609adf(email,captcha):
	D=sparta_2c0db3492a(captcha)
	if D[_A]!=1:return{_A:-1,_D:_L}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_08d2de0dd9(A.user,C);return{_A:1}
def sparta_08d2de0dd9(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_e3b0d20ba6(captcha,token,email,password):
	D=sparta_2c0db3492a(captcha)
	if D[_A]!=1:return{_A:-1,_D:_L}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
