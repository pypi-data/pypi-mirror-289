...
class ClickhouseConnector(EngineBuilder):
	...
	def get_data_table(E,table_name):
		try:
			A=E.connector;C=f"SELECT * FROM {table_name}";B=A.query(C);D=B.result_columns;B=A.query(C);D=B.column_names;F=B.result_rows;G=sparta_f9ee3f1f90(pd.DataFrame(F,columns=D))
			if A:A.close()
			return G
	...
	def get_data_table_query(E,sql,table_name=None):
		try:
			A=E.connector;C=sql;B=A.query(C);D=B.result_columns;B=A.query(C);D=B.column_names;F=B.result_rows;G=sparta_f9ee3f1f90(pd.DataFrame(F,columns=D))
			if A:A.close()
			return G
		except Exception as H:
			if A:A.close()
