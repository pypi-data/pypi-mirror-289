...
class MongoConnector(EngineBuilder):
	...
	def get_data_table(self,table_name):
		...
		try:db=self.connector;collection=db[collection_name];documents=list(collection.find({},{'_id':_A}));self.client.close();return sparta_f9ee3f1f90(documents)
		except Exception as e:self.client.close();raise Exception(e)
	def get_data_table_query(self,sql,table_name=None):
		try:exec(sql,globals(),locals());filters_to_apply=eval('filter_criteria');collection_name=table_name;db=self.connector;collection=db[collection_name];documents=list(collection.find(filters_to_apply,{'_id':_A}));self.client.close();return sparta_f9ee3f1f90(documents)
		except Exception as e:self.client.close();raise Exception(e)