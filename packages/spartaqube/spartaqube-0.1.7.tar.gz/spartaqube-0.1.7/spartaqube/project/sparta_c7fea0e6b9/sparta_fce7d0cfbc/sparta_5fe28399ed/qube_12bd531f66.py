...
class QuestDBConnector(EngineBuilder):
	...
	def get_data_table(A,table_name):E=f"{A.host}:{A.port}/exec";F=f"SELECT * FROM {table_name}";B=requests.get(E,params={_A:F},auth=HTTPBasicAuth(A.user,A.password));B.raise_for_status();C=B.json();G=[A['name']for A in C[_C]];D=sparta_f9ee3f1f90(C[_B]);D.columns=G;return D
	def get_data_table_query(A,sql,table_name=None):E=f"{A.host}:{A.port}/exec";F=sql;B=requests.get(E,params={_A:F},auth=HTTPBasicAuth(A.user,A.password));B.raise_for_status();C=B.json();G=[A['name']for A in C[_C]];D=sparta_f9ee3f1f90(C[_B]);D.columns=G;return D