...
class ScylladbConnector(EngineBuilder):
	...
	def get_data_table(A,table_name):
		try:B=A.connector;C=f"SELECT * FROM {table_name}";D=B.execute(C);E=[dict(A._asdict())for A in D];return sparta_f9ee3f1f90(E)
		except Exception as F:A.cluster.shutdown();raise Exception(F)
	def get_data_table_query(A,sql,table_name=None):
		try:B=A.connector;C=sql;D=B.execute(C);E=[dict(A._asdict())for A in D];return sparta_f9ee3f1f90(E)
		except Exception as F:A.cluster.shutdown();raise Exception(F)