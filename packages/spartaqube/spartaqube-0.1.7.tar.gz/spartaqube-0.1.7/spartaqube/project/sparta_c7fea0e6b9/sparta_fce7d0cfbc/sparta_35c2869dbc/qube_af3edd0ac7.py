...
class CouchdbConnector(EngineBuilder):
	...
	def get_data_table(self,table_name):
		...
		return sparta_f9ee3f1f90(pd.DataFrame(documents))
	def get_data_table_query(self,sql,table_name=None):A='selector';exec(sql,globals(),locals());selector_to_apply=eval(A);query={A:selector_to_apply};documents=[];db=self.connector[table_name];result=db.find(query);documents=[doc for doc in result];return sparta_f9ee3f1f90(pd.DataFrame(documents))