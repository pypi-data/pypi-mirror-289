...
from project.sparta_72a4072f41.sparta_07d0d78d96.qube_78b017fa56 import sparta_2c0db3492a,sparta_20d40ed804,sparta_6a5023d3fa
from project.sparta_c7fea0e6b9.sparta_7639bac7be.qube_48eea7078d import sparta_05128eedbd
def sparta_62d6a7aeef(json_data,user_obj):
...
def sparta_6317720598(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_d4960b9ce8(json_data,userObj):
	...
	return B
def sparta_2aa154e76f(json_data,userObj):
	...
	return A
def sparta_e5c9784b1a(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_05128eedbd(F):return{_A:-1,_B:'Invalid current code'}
	...
	return{_A:1}
def sparta_b92faffc77(json_data,userObj):A=userObj;C=json_data['base64image'];K=hashlib.sha256((str(A.id)+'_'+A.email+str(datetime.now())).encode(_D)).hexdigest();D,E=C.split(';base64,');F,L=D.split('/');G=F.split(':')[-1];B=UserProfile.objects.get(user=A);H=datetime.now();I=Avatar.objects.create(avatar=G,image64=E,date_created=H);B.avatar=I;B.save();J={_A:1};return J
def sparta_610c9e418e(json_data,userObj):B=json_data['bDarkTheme'];A=UserProfile.objects.get(user=userObj);A.is_dark_theme=B;A.save();C={_A:1};return C
def sparta_bc8247b35f(json_data,userObj):B=json_data['theme'];A=UserProfile.objects.get(user=userObj);A.editor_theme=B;A.save();C={_A:1};return C
def sparta_4f87c7ec56():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_7991ff7801(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_6a5023d3fa(E):return{_A:-1,_B:_I}
...
def sparta_4ec8059759(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_2c0db3492a(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_20d40ed804(F):return{_A:-1,_B:_I}
...
def sparta_9f6411575d(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_2c0db3492a(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_20d40ed804(G):return{_A:-1,_B:_I}
