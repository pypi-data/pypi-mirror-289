...
def sparta_8c536ebb03(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_463a0cf94e(G,F,A,B);H={_A:1,_C:C};return H
def sparta_463a0cf94e(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_9e34338d47(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_77d16a5798(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_463c0c55c6(json_data,user_obj):A=sparta_9e34338d47(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_03c2d5c8e0(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_bd3afd9948(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_463a0cf94e(G,A,I,C)
	J={_A:1};return J
def sparta_a4166233c9(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_a8e8cec34e(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_385b0f610a(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_94f82d15a5(json_data,user_obj):
