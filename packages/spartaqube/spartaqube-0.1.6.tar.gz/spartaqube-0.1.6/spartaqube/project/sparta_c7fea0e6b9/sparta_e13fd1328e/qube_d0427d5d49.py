...
from project.sparta_c7fea0e6b9.sparta_5d362fb63b import qube_be6523ebf2 as qube_be6523ebf2
def sparta_d4d27d2b09():return str(uuid.uuid4())
def sparta_027634b7c7(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_219bdb46b4(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_1195e003f4(path):
	with open(path,'a'):os.utime(path,None)
def sparta_97ea889f42(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_219bdb46b4(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_1195e003f4(B)
	...
	return{_A:1}
def sparta_35686bccd3(json_data,userObj):
	...
	return{_A:1}
def sparta_db26ae4ab5(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_219bdb46b4(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_bcf6a5318e(json_data,userObj):A=json_data[_B];B=qube_be6523ebf2.sparta_9a5f28278b(A);C={_A:1,'folderStructure':B};return C
def sparta_2f893cb1a1(json_data,userObj):
	A=json_data
	try:
...
def sparta_2175dd8ea5(json_data,userObj):
	...
	return{_A:1}
def sparta_4cadf9fe10(json_data,userObj):
	...
	return{_A:1}
def sparta_59d57c2929(json_data,userObj):
...
def sparta_fc96e7a3a5(json_data,userObj):
	...
	return{_A:-1}
def sparta_f8f6176c13(json_data,userObj):
