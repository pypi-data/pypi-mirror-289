...
from project.sparta_c7fea0e6b9.sparta_957b17c2d6.qube_4f323a8bf2 import sparta_f9ee3f1f90,sparta_6954500354
from project.sparta_c7fea0e6b9.sparta_51d16e4350 import qube_40d8b93f38 as qube_40d8b93f38
from project.sparta_c7fea0e6b9.sparta_957b17c2d6.qube_1cfd9bfb3c import sparta_f4b33e751e
...
def sparta_79b0d803a5(user_obj):
	A=qube_ad878641d5.sparta_77d16a5798(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_f6b35e3850(json_data,user_obj):
	D=user_obj;E=sparta_79b0d803a5(D)
...
def sparta_764942878f():return{_A:1,'available_engines':qube_180ac161b4.sparta_764942878f()}
def sparta_c33346ad90(json_data,user_obj):
	...
	return{_A:1}
def sparta_2a15f51f9b(json_data):
...
def sparta_8fe2af8289(json_data):
...
def sparta_ea1fa07d78(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_79b0d803a5(C)
...
def sparta_ac3daa0632(json_data,user_obj):
	C=json_data[_H];A=sparta_6375ef3937(C,user_obj)
...
def sparta_564c4aac4e(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_6375ef3937(H,user_obj)
...
def sparta_94690507cd(json_data,db_connector_obj):
...
def sparta_770268f572(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_6375ef3937(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_94690507cd(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_z]
	...
	return{_A:1,_E:sparta_6954500354(E)}
def sparta_9512eac1b7(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_6375ef3937(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_94690507cd(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_2bfd268f4a(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_9efacdf827(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_6375ef3937(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_94690507cd(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_6954500354(E)}
def sparta_f710d00485(json_data,user_obj):
	A=json_data;print('SAVE json_data');print(A);J=A[_L];D=_B
	if J:D=A[_s];D=qube_60a0d4ffd7.sparta_956c3fc62b(D)
...
def sparta_c0acf9951e(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_s];C=qube_60a0d4ffd7.sparta_956c3fc62b(C)
	...
	return{_A:1}
def sparta_48604e996a(json_data,user_obj):0
def sparta_b98efc38d2(json_data,user_obj):
	D=user_obj;F=sparta_79b0d803a5(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_8d33fc1f2e(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_6375ef3937(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_6954500354(sparta_f9ee3f1f90(i,variable_name=W))
...
def sparta_a75ed3ae1a(json_data,user_obj):
	...
	return{_A:1}
def sparta_22c585dc10(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_6954500354(sparta_f9ee3f1f90(D,variable_name=A))
	return B
def sparta_aebdc2d6a7(session_id):A=sparta_22c585dc10(session_id);B=sparta_f4b33e751e(b_return_type_id=_C);C=json.loads(A[_AP])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AQ];return{_A:1,_AR:A,_AS:A.get(_R,dict()),_Q:D}
def sparta_edc77e681a(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AB]=json.loads(B[_E])[_AB];F=sparta_22c585dc10(session_id);I=sparta_f4b33e751e(b_return_type_id=_C);J=json.loads(F[_AP])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AQ];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AR:F,_AS:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_104b13c3ea(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_79b0d803a5(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_60a0d4ffd7.sparta_2aa16b889a(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_6a80745453(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_60a0d4ffd7.sparta_956c3fc62b(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_AA,_M:A}
					except:return{_A:3,_F:_AA,_M:A}
			elif C.is_authenticated:
				G=sparta_79b0d803a5(C)
	...
	return{_A:-1}
def sparta_80bc47ad29(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_79b0d803a5(C)
	...
	return{_A:1,_AD:_D}
def sparta_7ed6c63186(plot_db_chart_obj):
	...
	for A in B.keys():
		if A in INPUTS_KEYS:
			if A==_x:C[A]=1
				...
				if D>0:C[A]=D
	return C
def sparta_b3681e49eb(json_data,user_obj):
	B=user_obj;D=sparta_79b0d803a5(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_s];C=qube_60a0d4ffd7.sparta_956c3fc62b(C)
...
def sparta_f2adba8d48(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_s];C=qube_60a0d4ffd7.sparta_956c3fc62b(C)
	...
	return{_A:1}
def sparta_2279ee65f3(json_data,user_obj):
	...
	return{_A:1}
def sparta_ded50f8c95(user_obj):
	B=user_obj;C=sparta_79b0d803a5(B)
	...
	return E
def sparta_09f242f534(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_80bc47ad29(A['widget_id'],B);D=C[_AD]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_8d33fc1f2e(A,B);return{_A:1,_E:[A[_E]for A in F[_A9]]}
	return{_A:-1}
def sparta_c65711eb50(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_40d8b93f38.sparta_3133a9bcfa(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_8ff194aac8(json_data,user_obj):
...
def sparta_db8b7803c2(json_data,user_obj):
