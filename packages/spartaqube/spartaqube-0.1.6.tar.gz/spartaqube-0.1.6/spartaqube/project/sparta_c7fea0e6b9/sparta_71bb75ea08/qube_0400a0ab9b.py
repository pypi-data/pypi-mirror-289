...
def sparta_c44903c589(json_data,user_obj):
	...
	for A in K:
		B=sparta_487e63ac34(model_to_dict(A));Q=int(A.typeObject)
		if not A.is_seen:print('Add 1');E+=1
		for R in P:B.pop(R,None)
		C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.date_created.astimezone(UTC);B[N]=G;B[O]=sparta_da11d3da2c(H)
		if Q==0:0
	...
	for A in L:
		...
		B=sparta_487e63ac34(model_to_dict(A));C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.dateCreated.astimezone(UTC);B[N]=G;B['type_object']=-2;B[O]=sparta_da11d3da2c(H);F.append(B)
...
def sparta_da11d3da2c(dateCreated):
	...
	return str(A)
def sparta_99993b96ea(json_data,user_obj):
