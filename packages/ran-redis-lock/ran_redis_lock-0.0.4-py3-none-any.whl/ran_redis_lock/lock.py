...
class RedisDistributedLock:
    def __init__(self, key: str, redis_client: aioredis.Redis, timeout: int = 10, lock_timeout: int = 60):
        """
        初始化分布式锁。
    ...
    async def acquire(self) -> bool:
        ...
        """
        lua_script = self.lua_scripts['acquire']
        script = self.redis_client.register_script(lua_script)
        lock_value = f"{self.key}-{id(self)}"
        acquired = await script(keys=[self.key], args=[lock_value, self.lock_timeout * 1000])
        if acquired:
            self.locked = True
    ...
    async def release(self) -> bool:
        ...
        lua_script = self.lua_scripts['release']
        script = self.redis_client.register_script(lua_script)
        released = await script(keys=[self.key], args=[self.lock_value])
        if released:
            self.locked = False
            self.lock_value = None
            return True
