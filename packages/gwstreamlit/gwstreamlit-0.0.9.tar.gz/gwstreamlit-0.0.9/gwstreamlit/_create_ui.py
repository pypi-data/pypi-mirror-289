...
def generate_text_input(gws, item, input_values, text_area=False):
    ...
    if text_area:
        gws.input_values[item.Key] = _fetch_tab(item).text_area(build_label(item), default_value, key=item.Key,
                                                                on_change=on_change, disabled=disabled_input)
    else:
        gws.input_values[item.Key] = _fetch_tab(item).text_input(build_label(item), default_value,
                                                                 key=item.Key,
                                                                 on_change=on_change, disabled=disabled_input)
...
def generate_dataframe(gws, item: InputFieldsBase, default_rows: dict):
    ...
    if item.Immutable:
        _fetch_tab(item).dataframe(st.session_state[df_key], hide_index=True, column_config=column_config,
                                   use_container_width=True, key=item.Key, selection_mode="single-row", on_select="rerun")
    else:
