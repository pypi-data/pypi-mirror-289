# coding: utf-8

"""
    Orbit Extrapolation API

    Orbit Extrapolation API

    The version of the OpenAPI document: 1.1.5
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from spacetower_orbit_extrapolation_python_client.models.box_object import BoxObject

class TestBoxObject(unittest.TestCase):
    """BoxObject unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> BoxObject:
        """Test BoxObject
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `BoxObject`
        """
        model = BoxObject()
        if include_optional:
            return BoxObject(
                x_dimension = 1.337,
                y_dimension = 1.337,
                z_dimension = 1.337,
                box_panel_radiation_coefficients = spacetower_orbit_extrapolation_python_client.models.reflection_absorption_coefficients.ReflectionAbsorptionCoefficients(),
                box_panel_drag_coefficient = 1.337,
                solar_array_geometry = spacetower_orbit_extrapolation_python_client.models.solar_array_geometry.SolarArrayGeometry(
                    radiation_coefficients = spacetower_orbit_extrapolation_python_client.models.reflection_absorption_coefficients.ReflectionAbsorptionCoefficients(), 
                    drag_coefficient = 1.337, 
                    surface_area = 1.337, 
                    normal_axis_in_space_object_frame = spacetower_orbit_extrapolation_python_client.models.vector3_d.Vector3D(
                        x = 1.337, 
                        y = 1.337, 
                        z = 1.337, ), )
            )
        else:
            return BoxObject(
                box_panel_radiation_coefficients = spacetower_orbit_extrapolation_python_client.models.reflection_absorption_coefficients.ReflectionAbsorptionCoefficients(),
                box_panel_drag_coefficient = 1.337,
                solar_array_geometry = spacetower_orbit_extrapolation_python_client.models.solar_array_geometry.SolarArrayGeometry(
                    radiation_coefficients = spacetower_orbit_extrapolation_python_client.models.reflection_absorption_coefficients.ReflectionAbsorptionCoefficients(), 
                    drag_coefficient = 1.337, 
                    surface_area = 1.337, 
                    normal_axis_in_space_object_frame = spacetower_orbit_extrapolation_python_client.models.vector3_d.Vector3D(
                        x = 1.337, 
                        y = 1.337, 
                        z = 1.337, ), ),
        )
        """

    def testBoxObject(self):
        """Test BoxObject"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
