...
""",
    ...
    group.add_option("--periods", dest="periods", help=f"Comma separated list of periods, default={default_eso_period} [optional]", default=str(default_eso_period))
    parser.add_option_group(group)
        ...
        for p_int in phase_intervals :
            ...
            if (
                ...
                if ob_exec_time < p_int_in_sec :
                    logging.warning(f'Phase interval {p_int[0]}--{p_int[1]} == {p_int_in_sec:3.1f}sec is too short to start and finish this OB with execution time of {ob_exec_time}secs.')
                if p_int_in_sec > min_ATI_length.to_value(u.second) :
                    logging.warning(f'Phase interval {p_int[0]}--{p_int[1]} == {p_int_in_sec:3.1f}sec is shorter than the minimum ATI length time of {min_ATI_length.to_value(u.second)}secs.')
                    ...
                    if (
                        ...
                        if (
                            ...
                            if not int_spans_nighttime :
                                logging.debug(f'Interval does not span any part of the night, original interval = {original_sf_int}, SSSR of previous and next nights {sssr[0][0].jd}--{sssr[0][1].jd} and {sssr[1][0].jd}--{sssr[1][1].jd}')
                            if ((sf_int[1]-sf_int[0])*24*60.*60. < ob_exec_time) :
                                logging.debug(f'Checked Interval shorter [{(sf_int[1]-sf_int[0])*24*60.*60.:3.1f}sec] than OB execution time [{ob_exec_time:3.1f}sec], original interval = {original_sf_int}, checked interval = {sf_int}')
                            if ((sf_int[1]-sf_int[0])*24*60.*60. < min_ATI_length.to_value(u.second)) :
                                logging.debug(f'Checked Interval shorter [{(sf_int[1]-sf_int[0])*24*60.*60.:3.1f}sec] than minimum ATI length time [{min_ATI_length.to_value(u.second):3.1f}sec], original interval = {original_sf_int}, checked interval = {sf_int}')
                pass
    ...
    for i,ATC in enumerate(ATCs) :
        ...
        if check_ATCs :
            ...
            if options.ephem is not None :
                phase_range=f"  [Phases: {(((t[0]-Time(T0,format='jd'))/P).to_value(u.day)%1):5.3f} -- {(((t[1]-Time(T0,format='jd'))/P).to_value(u.day)%1):5.3f}]"
            print(f"{i+1:3d} {ob_ATCs[-1]['from']} -- {ob_ATCs[-1]['to']}  [duration={((t[1]-t[0]).to_datetime())}]{phase_range}")
    if check_ATCs :
        '''
        logging.warning("****************************************************************************************")
        logging.warning("****************************************************************************************")
