...
class WMLProvider(ServiceProvider):
    ...
    def validate_configuration(self):
        ...
        if self.config.is_fmaas_scoring: 
            missing_values = []
            model_id = get(self.config.prompt_template_asset_json, "prompt.model_id")
            if model_id is None:
                missing_values.append("prompt_template_asset_details.prompt.model_id")
            is_template = get(self.config.prompt_template_asset_json, "is_template")
            if not is_template == False and self.config.platform_url is None:
                missing_values.append("platform_url")
                
            if len(missing_values) > 0:
                raise KeyError(f"Missing configuration information.Keys information:{missing_values}") 
        return
