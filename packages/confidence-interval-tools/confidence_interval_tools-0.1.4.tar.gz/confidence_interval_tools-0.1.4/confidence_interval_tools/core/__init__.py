...
class CI_Drawer(object):
    ...
    def __init__(
        ...
        if orientation == "vertical":
            ...
            opposite_axis = pandas.Series([y.loc[x == val_x].values for val_x in unique_x])
            ...
            opposite_axis = pandas.Series([x.loc[y == val_y].values for val_y in unique_y])
        else:
            pass
        ### calculate mean, median, q1 and q3 for each horizontal or vertical series
        mean = pandas.Series([original_opposite_axis.loc[original_axis == val_i].mean() for val_i in unique_axis])
        median = pandas.Series([original_opposite_axis.loc[original_axis == val_i].median() for val_i in unique_axis])
        q1 = pandas.Series([original_opposite_axis.loc[original_axis == val_i].quantile(0.25) for val_i in unique_axis])
        q3 = pandas.Series([original_opposite_axis.loc[original_axis == val_i].quantile(0.75) for val_i in unique_axis])
        ### calculate std or get it from the provided arguments
        if isinstance(std, type(None)):
            ### calculate std or use different methods
            std = pandas.Series([original_opposite_axis.loc[original_axis == val_i].std() for val_i in unique_axis]).fillna(0)
        elif issubclass(
            type(std), (float, int)
            ...
            if len(std) == len(unique_axis):
                ...
                if all([(std[numpy.roll(std[original_axis==val_i].index, 1)].values == std[original_axis==val_i].values).all() for val_i in unique_axis]):
                    pass  ## everything looks fine
                else:
                    raise ValueError(f"Different values of 'std' were provided for the same values of '{unique_axis_name}'.")
            else:
        ...
        if isinstance(lower, type(None)):
            ...
            type(lower), (float, int)
        ):
            ### same lower for the whole data
            lower = pandas.Series(
                [lower] * len(unique_axis)
            )
        elif isinstance(
            ...
            if len(lower) == len(unique_axis):
                ...
                ### Check for confusing duplicates
                if all([(lower[numpy.roll(lower[original_axis==val_i].index, 1)].values == lower[original_axis==val_i].values).all() for val_i in unique_axis]):
                    pass  ## everything looks fine
                else:
                    raise ValueError(f"Different values of 'lower' were provided for the same values of '{unique_axis_name}'.")
            else:
                ### reminder: this only happens if lower and upper are None
                raise ValueError(
        ...
        if isinstance(upper, type(None)):
            ...
            if len(upper) == len(unique_axis):
                ...
                ### Check for confusing duplicates
                if all([(upper[numpy.roll(upper[original_axis==val_i].index, 1)].values == upper[original_axis==val_i].values).all() for val_i in unique_axis]):
                    pass  ## everything looks fine
                else:
                    raise ValueError(f"Different values of 'upper' were provided for the same values of '{unique_axis_name}'.")
            else:
                ### reminder: this only happens if lower and upper are None
                raise ValueError(
    ...
    def as_dataframe(self):
        ...
        if self.params["orientation"] == "vertical":
            ...
            unique_axis_name = "x"
            opposite_axis = pandas.Series([self.y.loc[self.x == val_x].values for val_x in self.unique_x])
            opposite_axis_name = "y"
        elif self.params["orientation"] == "horizontal":
            unique_axis = self.unique_y
            unique_axis_name = "y"
            opposite_axis = pandas.Series([self.x.loc[self.y == val_y].values for val_y in self.unique_y])
            opposite_axis_name = "x"
        else:
