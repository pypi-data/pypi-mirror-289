...
def get_day_list(df: pd.DataFrame, datetime_column_name: str):
  """
...
def calculate_mape(x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
  """
...
def rmse(df: pd.DataFrame, value_column_name: str, forecast_column_name: str, datetime_column_name: str) -> np.ndarray:
  ...
  df = df.copy(deep=True)
  df['RMSE'] = float(0)
  df, day_list = get_day_list(df, datetime_column_name)
...
def mape(df: pd.DataFrame, value_column_name: str, forecast_column_name: str, datetime_column_name: str) -> np.ndarray:
  ...
  for i in range(len(day_list)):
    ...
    mape_values = calculate_mape(x1, x2)
    for j in range(len(copy1)):
      df.loc[copy1.index[j], 'MAPE'] = mape_values
  return np.array(df['MAPE'])


def rsquare(df: pd.DataFrame, value_column_name: str, forecast_column_name: str,
            datetime_column_name: str) -> np.ndarray:
  """
