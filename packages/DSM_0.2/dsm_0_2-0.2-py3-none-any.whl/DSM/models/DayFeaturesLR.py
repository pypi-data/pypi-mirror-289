...
class DayFeaturesLR(BaseModel):
  ...
  def _features_generation(self) -> np.ndarray:
    """
    ...
    for i in tqdm(range(self.circles_count * len(self.day_list))):
      ...
      common_data = df[df['date'].isin([list(self.day_list)[i % len(self.day_list)]])].copy()[
      ...
      for j in range(min(len(base_data), self.day_points)):
        ...
        if j != 0:
          if base_data.rolling(4).sum().sum() >= 0:
            features_data[1, j, i] = base_data.rolling(4).sum().sum() / 4
        else:
      ...
      for item in self.common_features_names:
        ...
        for j in range(self.day_points):
          features_data[self._features_number - 1, i, j] = common_data[item].index[j]
    ...
    for m in tqdm(range(self.circles_count)):  # multiple passes through the same data
      for s in range(0, len(self.day_list)):  # for each available day in the data
        ...
        for t in range(day_points):
          day = self.day
          if day > 7:
            y_before[t, :] = [features_data[0, t, day - 1], features_data[0, t, day - 7],
  ...
  def predict(self, X: Union[pd.DataFrame, dsm_timeseries], method: str = "All") -> np.ndarray:
    ...
    if method == "All":
      self.day = 0
      self._validate_dataframe(X, self.datetime_column_name, self.value_column_name)
        ...
        for t in range(self.day_points):
          ...
          if y_estimate[t, day] > 1.5 * np.max(features_data[0, t, day - 1]):
            ...
            if y_estimate[t, day] < 0:
              y_estimate[t, day] = 0
        self.day += 1
      new_data = self.df[self.df['date'].isin([list(self.day_list)[-2]])].copy()[
        ['date', 'time', self.value_column_name]]
      for j in range(len(new_data)):
        l = new_data.index[j]
        self.df.loc[l, 'forecast'] = float(y_estimate[j, self.circles_count - 1])
