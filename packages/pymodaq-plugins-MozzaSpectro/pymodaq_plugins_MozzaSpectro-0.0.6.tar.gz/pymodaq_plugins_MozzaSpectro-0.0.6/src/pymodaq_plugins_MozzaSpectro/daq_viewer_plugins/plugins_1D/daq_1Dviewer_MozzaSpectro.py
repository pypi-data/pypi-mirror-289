    ...
    def initialize_controller(self):
        try:
            ...
            if self.serials:
                ...
                print('Found Mozza device with serial:', self.serials[0])
                print('Commands:', [cmd for cmd in self.controller.handle[0].cmd])
                print('Errors:', [err for err in self.controller.handle[0].error])

                wls_nm = (
                self.settings.child('wavenumber_start').value(), self.settings.child('wavenumber_end').value())
