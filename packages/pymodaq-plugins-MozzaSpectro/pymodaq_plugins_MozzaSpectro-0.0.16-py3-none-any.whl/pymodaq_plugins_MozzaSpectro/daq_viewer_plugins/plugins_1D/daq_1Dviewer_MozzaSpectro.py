...
class DAQ_1DViewer_MozzaSpectro(DAQ_Viewer_base):
    ...
    def get_xaxis(self, ind_spectro):
        try:
            ...
            if hasattr(self.controller, 'acquisition_params') and hasattr(self.controller.acquisition_params,
                                                                          'wavenumber_array'):
                wavenumber_array = self.controller.acquisition_params.wavenumber_array
                if wavenumber_array.size > 0:
                    return wavenumber_array
                else:
                    print("Wavenumber array is empty")
                    return np.array([])
            else:
                print("Attribute 'wavenumber_array' not found in acquisition_params")
                return np.array([])
        except Exception as e:
