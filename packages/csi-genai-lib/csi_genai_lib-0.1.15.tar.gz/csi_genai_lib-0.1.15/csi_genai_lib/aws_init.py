...
class RefreshableBotoSession:
    ...
    def __get_new_session(self):
        ...
        if self.profile_name:
            ...
            return Session(region_name=self.region_name, profile_name=self.profile_name)
        # elif self.access_key and self.secret_key:
        #     print(f"RefreshableBotoSession: Using session credentials from environment variables")
        #     return Session(region_name=self.region_name, aws_access_key_id=self.access_key, aws_secret_access_key=self.secret_key)
        else:
    ...
    def __get_session_credentials(self):
        ...
        if self.sts_arn:
            ...
            try:
                ...
                print(f"[WARNING] assume_customer_role: Exception assuming Cross Account Role ${er}")
            else:
                credentials = {
    ...
    def refreshable_session(self) -> Session:
        ...
        session.set_config_variable("user_agent", user_agent)
...
class RefreshableBotoSession2:
    def __init__(
        ...
        if not self.access_key or not self.secret_key:
            print(f"[WARNING] __get_session_credentials: Failed 'CSI_AWS_ACCESS_KEY' and/or 'CSI_AWS_SECRET_KEY' not set.")
            raise ValueError("Failed 'CSI_AWS_ACCESS_KEY' and/or 'CSI_AWS_SECRET_KEY' not set.")
        if not self.sts_arn or not self.ext_id:
            print(f"[WARNING] __get_session_credentials: Failed 'CSI_TENANT_ROLE_ARN' and/or 'CSI_ROLE_EXTERNAL_ID' not set.")
    ...
    def __get_session_credentials(self):
        ...
        if not self.sts_client:
            try:
                # Create an STS client using Caber credentials from SecretsManager
                ...
                print(f"[WARNING] __get_session_credentials: Exception creating sts_client ${err}")
                raise ValueError(f"__get_session_credentials: Exception creating sts_client ${err}")
        ...
        try:
            ...
            print(f"[WARNING] __get_session_credentials: Exception assuming Cross Account Role ${err}")
            raise ValueError(f"__get_session_credentials: Exception assuming Cross Account Role ${err}")
        else:
            credentials = {
                "access_key": response.get("AccessKeyId"),
                "secret_key": response.get("SecretAccessKey"),
...
try:
    AWS = RefreshableBotoSession2().refreshable_session()  # boto3.session.Session()
    aws_credentials = AWS.get_credentials()
    aws_account_id = AWS.client('sts').get_caller_identity().get('Account', '')
except Exception as err:
