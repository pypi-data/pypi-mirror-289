...
class GenICamBase(_base.BaseModel):
    ...
    def __init__(self, dll_dir_path: str = r'C:\Program Files\Chromasens\GCT2\bin'):
        ...
        self.tl_producer_file_name = None
        self.dll = None
    ...
    def open_connection(self, tl_producer_file_name: Union[str, list, type(None)] = None, device_number: int = 1) -> (int, type(None), str):
        ...
        self.tl_producer_file_name = tl_producer_file_name
    ...
    def set_feature(self, feature_name: str, feature_value: Union[bool, int, float, str, bytes], module: int = _ds.CSI_DEVICE_MODULE) -> (int, type(None), str):
        ...
        Args:
            ...
            module (int, optional): Supported Values = [_ds.CSI_TRANSPORTLAYER_MODULE, _ds.CSI_INTERFACE_MODULE, _ds.CSI_DEVICE_MODULE, _ds.CSI_LOCAL_DEVICE_MODULE, _ds.CSI_STREAM_MODULE, _ds.CSI_BUFFER_MODULE]
        Returns:
            tuple (int, type(None), str): (0, None, 'OK') or (Error Code, None, 'Error Message')
        """
        ...
        if feature_param_dict['type'] in [_ds.CSI_BOOLEAN_TYPE]:
            ...
            retval = function(self.device_handle, name, value, size, module)
            if retval != self._error_codes.OK[0]:
                return self._error_description_helper(retval)
            ...
            retval = function(self.device_handle, name, module)
            if retval != self._error_codes.OK[0]:
                return self._error_description_helper(retval)
            ...
            t0 = time()
            while is_active:
                retval = function(self.device_handle, name, ctypes.byref(is_active), module)
        ...
        #execute the function         
        retval = function(self.device_handle, name, value, module)
        if retval != self._error_codes.OK[0]:
            return self._error_description_helper(retval)
    ...
    def get_feature_parameter(self, feature_name: str, module: int = _ds.CSI_DEVICE_MODULE) -> (int, dict, str):
        ...
        retval = function(self.device_handle, ctypes.c_char_p(feature_name.encode()), ctypes.byref(param), module)
        if retval != self._error_codes.OK[0]:
            return self._error_description_helper(retval)       
        
        param_info_dict = dict()
        for field in list(param._fields_):
            param_info_dict[field[0]] = getattr(param,field[0])
    ...
    def get_feature_enum_list(self, feature_name: str, module: int = _ds.CSI_DEVICE_MODULE) -> (int, list, str):
        ...
        for i in range(feature_param_dict['enumCounter']):
            ...
            retval = function(self.device_handle, ctypes.c_char_p(feature_name.encode()), i, ctypes.byref(param_loop), module)
    ...
    def discover_devices(self, tl_producer_file_path: Union[str, list, type(None)]  = None)  -> (int, dict, str):
        """
        ...
        if tl_producer_file_path is None:    
            tl_producer_file_name_str = os.environ.get('GENICAM_GENTL64_PATH')
            if tl_producer_file_name_str is None:
            ...
            tl_producer_file_name_str = ''
            for _tl_producer_file_path in tl_producer_file_path:
                
                if os.path.isdir(_tl_producer_file_path):
                    tl_producer_file_name_str += f'{_tl_producer_file_path};'
                elif os.path.isfile(_tl_producer_file_path):
                    tl_producer_file_name_str = f'{os.path.dirname(_tl_producer_file_path)};'
                    
                else:
                    # check, if the file exists
                    if not os.path.isfile(_tl_producer_file_path):
                        return self._retval_helper(self._error_codes.NOK_FILE_NOT_AVAILABLE[0])

        if tl_producer_file_name_str[-1] == ';':
            tl_producer_file_name_str = tl_producer_file_name_str[:-1]
        ...
        for i in range(discovery_info.numDevices):
            ...
            discovery_info_dict = dict()
            for field in list(discovery_info.devices[i]._fields_):
                discovery_info_dict[field[0]] = getattr(discovery_info.devices[i],field[0])
            
            discovery_info_dict_list.append(discovery_info_dict)
        
        return self._retval_helper(retval, discovery_info_dict_list)
...
class GenICamStreamable(GenICamBase):
    ...
    def grab_image_async(self, timeout_ms: int = 5000, raise_buffer_overflow_error: bool = False) -> (int, tuple, str):
        ...
        img_metadata_dict = dict()
        img_metadata_dict['imageID'] = data_struct.imageNr
        img_metadata_dict['bufferIdentifier'] = data_struct.bufferIdentifier
        img_metadata_dict['timestampMS'] = data_struct.timestampMS
        img_metadata_dict['timestampRaw'] = data_struct.timestampRaw
        
        if self.stream_image_counter is None:
            self.stream_image_counter = img_metadata_dict['imageID']
            self.start_imageID = img_metadata_dict['imageID'] - 1
        else:
            self.stream_image_counter += 1
            if raise_buffer_overflow_error:
                if self.stream_image_counter != img_metadata_dict['imageID']:
                    msg = f"NOK: Buffer overflow error, image acquisition if faster than image handling! -> Img-ID: {img_metadata_dict['imageID']} | Img-counter: {self.stream_image_counter}"
        ...
        img_metadata_dict['imageNr'] = img_metadata_dict['imageID'] - self.start_imageID
    ...
    def _convert_pointer_to_ndarray(self, p_img: ctypes.POINTER(ctypes.c_ubyte), info: _ds.csiImageInfo) -> (int, type(None), str):
        ...
        if info.format not in _ds.SUPPORTED_PIX_FORMATS:
            self.scan_img_stack_data['img_format_ok'] = False
            self._scan_image_stack_stop()
            return
...
def example_grab_images_sync(cam_list: list) -> (int, type(None), str):
    ...
    for i in range(x):
        for cam in cam_list:
            ...
            #...
            #...
            print(f'img ID: {img_info_dict["imageID"]:0>5} | img nr.: {img_info_dict["imageNr"]:0>5} |  first px. value: {img[0,0]}')
...
def example_grab_images_async(cam_list: list) -> (int, type(None), str):
    ...
    for i in range(x):
        for i, cam in enumerate(cam_list):
            ...
            #...
            #...
            print(f'cam: {i} | img ID: {img_info_dict["imageID"]:0>5} | img nr.: {img_info_dict["imageNr"]:0>5} |  first px. value: {img[0,0]}')
