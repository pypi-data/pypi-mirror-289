...
# 创建ImageFolder对象
data_train = ImageFolder(os.path.join(dirpath,'data/imagewoof/train'), train_tfms)
data_eval = ImageFolder(os.path.join(dirpath,'data/imagewoof/val'), valid_tfms)
...
model=fit.set(model,optimizer=optimizer,loss_func=loss_func,perform_func=perform_func,n_iter=10,device=device)
...
model=fit.set(model,optimizer=optimizer,loss_func=loss_func,grad_func=grad_func,n_iter=10,device=device)
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.0001, weight_decay=1e-4)
loss_func=torch.nn.CrossEntropyLoss()
model=fit.set(model,optimizer=optimizer,loss_func=loss_func,n_iter=10,device=device,n_batch_step=5)
...
def perform_func(y_hat,y):
    ...
    _,y_hat=y_hat.max(axis=1)
    return round((y_hat==y).sum().item()/len(y),4)
model=fit.set(model,optimizer=optimizer,loss_func=loss_func,perform_func=perform_func,n_iter=(10,20),device=device)
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.0001, weight_decay=1e-4)
loss_func=torch.nn.CrossEntropyLoss()
model=fit.set(model,optimizer=optimizer,loss_func=loss_func,n_iter=5,device=device,n_batch_plot=5,save_path=dirpath)

#训练
model.fit(train_loader=train_loader,eval_loader=eval_loader,mode=('input','target'))

checkpoint=torch.load(dirpath+'/ResNet_iter_2.checkpoint')
checkpoint.keys()
