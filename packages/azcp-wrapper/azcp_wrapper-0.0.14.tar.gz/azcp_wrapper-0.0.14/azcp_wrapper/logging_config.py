...
class CopyLogger:
    ...
    def __init__(self, root_log_directory=ROOT_LOG_DIRECTORY, log_level=LOGGING_LEVEL):
    ...
    def _configure(self):
        ...
        hms = current_datetime.strftime("%Y%m%d%H%M%S")
        # uid = Haikunator().haikunate()

        # Create run log and run job plan directories
        self.run_name = f"azpype_{hms}.log"
        self.base_run_directory = Path(self.root_log_directory).expanduser() / "logs" 

        self.run_log_directory = self.base_run_directory / day
...
run_name, run_log_directory, logger = CopyLogger().get_logger()
class JobsLogger():
