...
def controller(request, asset_client: AssetClient):
    ...
    with data_store, instance_context("sql"):
...
def controller_max_execution_timeout(request):
    ...
    controller = ExecutionController(data_store)
...
def isolated_controllers(request):
    ...
    keys = ["OSFamily", "ISA"]
    cache = LruActionCache(storage, 50)
    data_store_a = SQLDataStore(sql_provider, storage, action_cache=cache, property_keys=keys, match_properties=keys)
    data_store_b = SQLDataStore(sql_provider, storage, action_cache=cache, property_keys=keys, match_properties=keys)
    with data_store_b, data_store_a:
        yield ExecutionController(data_store_a), ExecutionController(data_store_b)
...
def test_get_metadata_for_leases(controller):
    ...
    with mock.patch("buildgrid.server.persistence.sql.impl.logstream_client", new=mock_logstream_client()):
        lease = scheduler.synchronize_bot_lease(bot_name, bot_id, BotStatus.OK.value, None)
        lease.state = LeaseState.ACTIVE.value
        lease = scheduler.synchronize_bot_lease(bot_name, bot_id, BotStatus.OK.value, lease)
