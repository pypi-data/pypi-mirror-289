...
from typing import Any, Dict, Mapping, Optional, Set, Type, Union, cast
...
class JwtParser:
    def __init__(
        ...
        if jwks_url is not None:
            try:
                ...
                self._jwks_client = jwt.PyJWKClient(jwks_url, lifespan=jwks_lifespan, max_cached_keys=AUTH_CACHE_SIZE)
            except NameError:
    ...
    def identity_from_jwt_payload(self, payload: Dict[str, Any]) -> ClientIdentity:
        ...
        workflow = ""
        if self._audience:
            workflow = self._audience
        else:
            audience_from_payload = payload.get("aud")
            if audience_from_payload:
...
class JWTAuthManager(AuthManager):
    def __init__(
        ...
        self._token_parser = JwtParser(secret, algorithm, jwks_url, audience, jwks_fetch_minutes)
