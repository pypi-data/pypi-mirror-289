...
class qwClient(object):
    ...
    async def auth(cls, user, token):
        ...
        async with grpc.aio.insecure_channel(server_host) as channel:
            stub =  qwdata_pb2_grpc.MarketDataServiceStub(channel)
            uuid = get_mac_address()
            response = await stub.Auth(qwdata_pb2.AuthRequest(user=user, token=token, uuid=uuid))
            if response.success:
    ...
    @classmethod    
    async def fetch_data(cls, exchange='binance', symbol='BTCUSDT', asset_type='spot', data_type='klines', start='2021-08-01 00:00:00', end='2024-07-17 00:00:00', batch_size=50):
        ...
        async with grpc.aio.insecure_channel(server_host) as channel:
            client = qwdata_pb2_grpc.MarketDataServiceStub(channel)

            # Greet
            response = await client.SayHello(qwdata_pb2.HelloRequest(name='world'))
            print(f'Greeting: {response.message}')

            # Authenticate
            # auth_response = await client.Auth(qwdata_pb2.AuthRequest(user='your_username', token='your_token', uuid='your_uuid'))
            ...
            # Fetch data
            request = qwdata_pb2.FetchDataRequest(
            ...
            try:
                async for response in client.FetchData(request):
                    ...
                    bdata = qwdata_pb2.BatchMinuDataMessages()
                    bdata.ParseFromString(decompressed_data)

                    # Print the received data
                    datalist.append(bdata.data)
...
def auth(user, token):
    asyncio.run(qwClient.auth(user, token))


def fetch_data():
    asyncio.run(qwClient.fetch_data())            
