...
class RTPSender:
    ...
    def process_video_queue2(self):
        ...
        begin = np.frombuffer(b'\x00\x00\x01\x06', dtype=np.uint8)
        end = np.frombuffer(b'\x00\x00\x00\x01\x65', dtype=np.uint8)
        p = np.frombuffer(b'\x00\x00\x00\x01\x61', dtype=np.uint8)
        ...
        def find_subarray(arr, subarr):
            ...
            for i in range(len_arr - len_subarr + 1):
                if np.all(arr[i:i + len_subarr] == subarr):
                    return i
            return -1

        @njit
        def starts_with(arr, subarr):
            len_subarr = len(subarr)
            if len(arr) < len_subarr:
                return False
            return np.all(arr[:len_subarr] == subarr)

        @njit
        def process_buffer(buffer_bytes, data, hard_encode, begin, end, p):
            buffer = buffer_bytes
            if hard_encode:
                pos = find_subarray(buffer_bytes, begin)
                if pos != -1:
                    pos = find_subarray(buffer_bytes, end)
                    if pos != -1:
                        buffer = np.concatenate((data, buffer_bytes[pos:]))
                    else:
                        pos2 = find_subarray(buffer_bytes, p)
                        if pos2 != -1:
                            buffer = buffer_bytes[pos2:]
                elif starts_with(buffer_bytes, end):
                    buffer = np.concatenate((data, buffer_bytes))
            else:
                if starts_with(buffer_bytes, begin):
                    pos = find_subarray(buffer_bytes, end)
                    if pos != -1:
                        buffer = np.concatenate((data, buffer_bytes[pos:]))
                elif starts_with(buffer_bytes, end):
        ...
        while not self.stop_event.is_set():
            try:
                img_frame = self.image_queue2.get_nowait()
            except queue.Empty:
                continue  # Skip iteration if the queue is empty
        # while not self.stop_event.is_set():
        #     img_frame = self.image_queue2.get()
            ...
            for packet in packets:
                ...
                # ttt1 = time()
                buffer = process_buffer(buffer_bytes, data, self.hard_encode, begin, end, p)
                # ttt2 = time()
                # count += 1
                # total_t += (ttt2 - ttt1)
                # if count % 1000 == 0:
                #     print("process_buffer avg time: ", total_t / count)
                #     count = 0
                #     total_t = 0
                # print("buffer: ", buffer[:5])
                j = 0
    ...
    def process_audio_queue2(self):
        ...
        while not self.stop_event.is_set():
            try:
                audio_data, is_16k = self.audio_queue2.get_nowait()
            except queue.Empty:
                continue  # Skip iteration if the queue is empty
