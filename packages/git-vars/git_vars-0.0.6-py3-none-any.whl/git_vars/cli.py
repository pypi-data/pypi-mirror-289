...
from .list_vars import listLocalVars, listRemoteVars
...
@cli.command()
@click.option("-f", "--filepath", help="Filepath to use")
def list_local_vars(filepath=None):
    """List all environment variables stored locally in the provided file"""

    logger.debug(f'Listing environment variables from: {filepath}')
    try:
        asyncio.run(listLocalVars(filepath))
    except Exception as err:
        logger.error(f'Error in push command: {err}')
        handle_error_exception(err)
...
def list_remote_vars(profile, scope=None, filepath=None):
    """List all environment variables stored locally in the provided file"""
    # Create a dictionary to store command-line arguments
    access_token = get_config_value(profile, 'access_token')
    repository_url = get_config_value(profile, 'repository_url')
    ...
    if scope is None:
        repository_url = get_config_value(profile, 'scope')
    ...
    logger.debug(f'Listing environment variables for the repository {repository_url} and {scope} scope')
    try:
        asyncio.run(listRemoteVars(vars_dict))
    except Exception as err:
        logger.error(f'Error in push command: {err}')
        handle_error_exception(err)
