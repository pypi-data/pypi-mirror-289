...
for i in range(len(files)):
    ...
    VTemp = poscar.readline()
    VTemp1 = poscar.readline().split()
    temp_id = VTemp1[-1] + '_' + number + '\n'
    poscar.close()
    #-----------------------------------------------------------------------
    with open(diret2 + '/' + files[i], "r") as file: line = file.readlines()
    line[0] = str(VTemp) + '_' + number + '\n'
    with open(diret2 + '/' + files[i], "w") as file: file.writelines(line)
    #---------------------------------------------------------------------
    current_name = diret2 + '/' + files[i]
    temp_rotulo = VTemp1[-1].replace('/', ' ').split()    
    new_name = temp_rotulo[1] + '_' + temp_rotulo[-1]
