...
for k in range(n_Lattice):
    ...
    for i in range(len(VTemp0)):
        temp_stoichiometry += str(VTemp0[i]) + str(VTemp1[i])
    stoichiometry.append(temp_stoichiometry)
...
if (n_structures != 0):
   ...
   for i in range(n_structures):
       ...
       # poscar_new.write(f'\n')
       #-----------------------------------------------------------------------------
       poscar_new.write(f'SAMBA {n_Lattice}L/{formula_quimica}/{nions_1}/{nions_2}/')
       if (n_Lattice == 3):  poscar_new.write(f'{nions_3}/')
       poscar_new.write(f'{mismatch_12[i]}/{angle1}/')
       if (n_Lattice == 3):  poscar_new.write(f'{mismatch_13[i]}/{angle3}/')
       poscar_new.write(f'{unique_id} \n')
       #----------------------------------


ID_Bilayer = 'SAMBA 1L/' + formula_quimica + '/' + unique_id
       #================================================
