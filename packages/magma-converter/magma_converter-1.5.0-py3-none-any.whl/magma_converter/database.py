...
class Sds(MagmaBaseModel):
    ...
    completeness = DecimalField(max_digits=5, decimal_places=2, auto_round=True)
    sampling_rate = DecimalField(max_digits=5, decimal_places=2)
...
class Database:
    ...
    def update_sds(sds: dict[str, Any]) -> int:
        _sds, created = Sds.get_or_create(**sds)
