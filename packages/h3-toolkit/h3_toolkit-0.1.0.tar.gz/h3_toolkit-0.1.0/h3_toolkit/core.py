...
from h3_toolkit.aggregation.aggregator import SumAggregation, AvgAggregation
...
def vector_to_cell_scale_up(
    ...
    aggregation_func: dict[AggFunc, list[Callable[..., pl.DataFrame]]] = {
        AggFunc.SUM.value: lambda df: _sum_agg(df, target_cols),
        AggFunc.AVG.value: lambda df: _avg_agg(df, target_cols),
        AggFunc.COUNT.value: lambda df: _sum_agg(df, target_cols),
        AggFunc.MAJOR.value: _major,
        AggFunc.PERCENTAGE.value: _percentage,
    }
...
#             raise ValueError("Aggregation strategy must be set")
...
#             raise ValueError("Target columns must be set")

#         if isinstance(data, gpd.GeoDataFrame):
#             logging.info("Converting GeoDataFrame to polars.DataFrame")
#             data = geom_to_wkb(data)

#         logging.info(f"======Start converting data to h3 cells with resolution {self.resolution}======")
#         result = (
#             data
#             .fill_nan(0)
#             .lazy()
