...
class ModelActivity:
    def __new__(cls, *args, **kwargs):
        ...
        try:
            ...
            logger = None
            correlation_id = None
        # Try to get user account from args
        # if something goes wrong, default to old module
        try:
            # throw as soon as possible if split didn't succeed
            assert split != None
            account = args[-1]
            # Retrieve the feature flag value using userKey
            split_value = split.get_treatment(
                account.get("email"), "PlatformNotificationsService"
            )
            if logger != None and correlation_id != None:
                logger.info(
