...
} = useFetch<IOrganisation[], (pageNumber: number) => void>(async (pageNumber: number) => {
  const { organisations: res, meta } = await OrderReferences.fetchOrganisations('', pageNumber);
  organisations.value = [...organisations.value, ...res];
  if (meta.pagination.page !== meta.pagination.pages) {
    fetchOrganisations(++meta.pagination.page);
  }
})
const {
