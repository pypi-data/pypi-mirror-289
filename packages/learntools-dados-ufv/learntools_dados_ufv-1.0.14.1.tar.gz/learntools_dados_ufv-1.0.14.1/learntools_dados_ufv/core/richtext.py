...
class Hint(PrefixedRichText):
    ...
    def __init__(self, txt, n=1, last=True):
        ...
        if not last:
            ...
            coda = '\n(Para outra dica, chame `.hint({})`)'.format(n+1)
            txt += coda
        super().__init__(txt)
    ...
    def label(self):
        if self.is_multi:
            return 'Dica {}'.format(self.n)
        return 'Dica'

class Correct(PrefixedRichText):
    @property
    def _label(self):
        return self.kwargs.get('_congrats', 'CORRETO')
    ...
    def load(cls, path):
        ...
        with open(path) as f:
            ...
            lines = [line[:-1] for line in lines
                    # Hack
                    if not line.startswith('from learntools_dados_ufv.python.solns')
                    ]
            return cls(*lines)
