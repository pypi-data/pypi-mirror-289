...
from learntools_dados_ufv.core.richtext import *
from learntools_dados_ufv.core.exceptions import *
from learntools_dados_ufv.core.problem import *
from learntools_dados_ufv.core import colors, tracking
...
class ProblemView:
    ...
    _not_attempted_msg = ("Depois de atualizar o código inicial, `check()` irá"
            " dizer se seu código está correto."
    )
    ...
    def check(self):
        try:
            ...
            msg = str(e) or 'Desculpa, o auto-checking não está disponível para essa questão.'
            self._track_check(tracking.OutcomeType.EXCEPTION,
                failureMessage=msg,
    ...
    def _get_injected_args(self):
        ...
        if len(missing) == 0:
            ...
            raise NotAttempted("Lembre-se, você deve criar a seguinte variável {}: {}"\
                    .format('s' if len(missing) > 1 else '',
                        ', '.join('`{}`'.format(v) for v in missing)
                        )
                    )
        else:
            raise Incorrect("Você ainda precisa definir a(s) variável(is): {}".format(
                    ', '.join('`{}`'.format(v) for v in missing)
                    ))
    ...
    def hint(self, n=1):
        ...
        if not hints:
            msg = 'Desculpa, sem dicas para você nessa questão...'
        ...
        assert n <= len(hints), "Sem dicas disponíveis!"
        hint = hints[n-1]
    ...
    def _assert_last_outcome(self, outcome):
        self.check()
        assert self._last_outcome == outcome, ("Esperado que a última saída seja {}, mas foi {}".format(
