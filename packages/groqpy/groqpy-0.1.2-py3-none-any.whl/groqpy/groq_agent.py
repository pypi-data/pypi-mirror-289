            ...
            # HTTP error handling
                ...
                try:
                    ...
                    if (error_type == 'tokens' and error_code == 'rate_limit_exceeded') or (error_type == 'requests' and error_code == 'rate_limit_exceeded'):
                        ...
                        # Added 'limit_used == 0' since groq lets you go over the limit by a few tokens sometimes
                        if limit_requested > limit_max and limit_used == 0:
                            raise Exception('Requested tokens exceed the maximum limit.')
                ...
                # If the error message is not in the expected format
                    ...
                    logging.error('Parsing error:', str(x))
                    logging.error('Original error:', str(e))
                    logging.error(content)
                    raise e
