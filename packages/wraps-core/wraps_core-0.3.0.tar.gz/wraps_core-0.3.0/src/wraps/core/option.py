...
[`Option[T]`][wraps.core.option.Option] represents an optional value:
every [`Option[T]`][wraps.core.option.Option] is either [`Some[T]`][wraps.core.option.Some] and
contains a value, or [`Null`][wraps.core.option.Null], and does not.

[`Option[T]`][wraps.core.option.Option] types can be very common in python code,
as they have a number of uses:

- Initial values (see [`ReAwaitable[T]`][wraps.futures.core.reawaitable.ReAwaitable]);
- Return values for functions not defined over their entire input range (partial functions);
- Return value for otherwise reporting simple errors, where [`Null`][wraps.core.option.Null]
  is returned on error;
- Optional function arguments (albeit slightly unergonomic).

[`Option[T]`][wraps.core.option.Option] is commonly paired with pattern matching to query
the presence of [`Some[T]`][wraps.core.option.Some] value (`T`) and take action,
always accounting for the [`Null`][wraps.core.option.Null] case:
...
Here, we know that [`Null`][wraps.core.option.Null] represents only one case,
that is, attempts to divide by zero. However, when we need to represent multiple errors
from one function, we might want to use [`Result[T, E]`][wraps.core.result.Result] instead,
as described in the [`result`][wraps.core.result] section.
"""
...
from wraps.core.reprs import empty_repr, wrap_repr
...
class OptionProtocol(AsyncIterable[T], Iterable[T], Protocol[T]):  # type: ignore[misc]
    ...
    def is_some(self) -> bool:
        """Checks if the option is [`Some[T]`][wraps.core.option.Some].
        ...
        Returns:
            Whether the option is [`Some[T]`][wraps.core.option.Some].
        """
        ...

    @required
    def is_some_and(self, predicate: Predicate[T]) -> bool:
        """Checks if the option is [`Some[T]`][wraps.core.option.Some] and the value
        ...
        Returns:
            Whether the option is [`Some[T]`][wraps.core.option.Some] and the predicate is matched.
        """
        ...
    ...
    async def is_some_and_await(self, predicate: AsyncPredicate[T]) -> bool:
        ...
        Returns:
            Whether the option is [`Some[T]`][wraps.core.option.Some] and
    ...
    def is_null(self) -> bool:
        """Checks if the option is [`Null`][wraps.core.option.Null].
        ...
        Returns:
            Whether the option is [`Null`][wraps.core.option.Null].
        """
        ...

    @required
    def expect(self, message: str) -> T:
        """Returns the contained [`Some[T]`][wraps.core.option.Some] value.
        ...
        Raises:
            Panic: Panics with the `message` if the option is [`Null`][wraps.core.option.Null].
    ...
    def extract(self) -> Optional[T]:
        """Returns the contained [`Some[T]`][wraps.core.option.Some] value or [`None`][None].
    ...
    def unwrap(self) -> T:
        ...
        Instead, prefer to use pattern matching and handle the [`Null`][wraps.core.option.Null]
        case explicitly, or call [`unwrap_or`][wraps.core.option.OptionProtocol.unwrap_or]
        or [`unwrap_or_else`][wraps.core.option.OptionProtocol.unwrap_or_else].
        ...
        Example:
            ...
            Traceback (most recent call last):
              ...
            wraps.core.panics.Panic: called `unwrap` on null
            ```

        Raises:
            Panic: Panics if the option is [`Null`][wraps.core.option.Null].
    ...
    def unwrap_or(self, default: T) -> T:  # type: ignore[misc]
        """Returns the contained [`Some[T]`][wraps.core.option.Some] value or the provided `default`.
    ...
    def unwrap_or_else(self, default: Nullary[T]) -> T:
        """Returns the contained [`Some[T]`][wraps.core.option.Some] value or
    ...
    def or_raise(self, error: AnyError) -> T:
        ...
        Arguments:
            error: The error to raise if the option is [`Null`][wraps.core.option.Null].

        Raises:
            AnyError: The error provided, if the option is [`Null`][wraps.core.option.Null].
    ...
    def or_raise_with(self, error: Nullary[AnyError]) -> T:
        ...
        Raises:
            AnyError: The error computed, if the option is [`Null`][wraps.core.option.Null].
    ...
    def inspect(self, function: Inspect[T]) -> Option[T]:
        """Inspects a possibly contained [`Option[T]`][wraps.core.option.Option] value.
    ...
    def map(self, function: Unary[T, U]) -> Option[U]:
        """Maps an [`Option[T]`][wraps.core.option.Option] to an [`Option[U]`][wraps.core.option.Option]
    ...
    def ok_or(self, error: E) -> Result[T, E]:
        """Transforms an [`Option[T]`][wraps.core.option.Option]
        into a [`Result[T, E]`][wraps.core.result.Result], mapping [`Some(value)`][wraps.core.option.Some]
        to [`Ok(value)`][wraps.core.result.Ok] and [`Null`][wraps.core.option.Null]
        to [`Error(error)`][wraps.core.result.Error].
    ...
    async def ok_or_else_await(self, error: AsyncNullary[E]) -> Result[T, E]:
        ...
        into a [`Result[T, E]`][wraps.core.result.Result], mapping [`Some(value)`][wraps.core.option.Some]
        to [`Ok(value)`][wraps.core.result.Ok] and [`Null`][wraps.core.option.Null]
        to [`Error(await error())`][wraps.core.result.Error].
    ...
    def and_then(self, function: Unary[T, Option[U]]) -> Option[U]:
        """Returns the option if it is [`Null`][wraps.core.option.Null],
    ...
    def filter(self, predicate: Predicate[T]) -> Option[T]:
        ...
        - [`Some(value)`][wraps.core.option.Some] if the contained `value` matches the predicate, and
        - [`Null`][wraps.core.option.Null] otherwise.
    ...
    def xor(self, option: Option[T]) -> Option[T]:
        """Returns [`Some[T]`][wraps.core.option.Some] if exactly one of `self` and `option`
        is [`Some[T]`][wraps.core.option.Option], otherwise returns [`Null`][wraps.core.option.Null].
    ...
    def zip(self, option: Option[U]) -> Option[Tuple[T, U]]:
        ...
        If `self` is [`Some(s)`][wraps.core.option.Some] and `option` is [`Some(o)`][wraps.core.option.Some],
        this method returns [`Some((s, o))`][wraps.core.option.Some]. Otherwise,
        [`Null`][wraps.core.option.Null] is returned.
    ...
    def zip_with(self, option: Option[U], function: Binary[T, U, V]) -> Option[V]:
        ...
        If `self` is [`Some(s)`][wraps.core.option.Some] and `option` is [`Some(o)`][wraps.core.option.Some],
        this method returns [`Some(function(s, o))`][wraps.core.option.Some]. Otherwise,
        [`Null`][wraps.core.option.Null] is returned.
    ...
    async def zip_with_await(self, option: Option[U], function: AsyncBinary[T, U, V]) -> Option[V]:
        ...
        If `self` is [`Some(s)`][wraps.core.option.Some] and `option` is [`Some(o)`][wraps.core.option.Some],
        this method returns [`Some(await function(s, o))`][wraps.core.option.Some]. Otherwise,
        [`Null`][wraps.core.option.Null] is returned.
    ...
    def unzip(self: OptionProtocol[Tuple[U, V]]) -> Tuple[Option[U], Option[V]]:
        ...
        If `self` is [`Some((u, v))`][wraps.core.option.Some], this method returns
        ([`Some(u)`][wraps.core.option.Some], [`Some(v)`][wraps.core.option.Some]) tuple.
        Otherwise, ([`Null`][wraps.core.option.Null], [`Null`][wraps.core.option.Null]) is returned.
    ...
    def flatten(self: OptionProtocol[OptionProtocol[U]]) -> Option[U]:
        """Flattens an [`Option[Option[T]]`][wraps.core.option.Option]
        to [`Option[T]`][wraps.core.option.Option].
    ...
    def early(self) -> T:
        ...
        [`@early_option`][wraps.core.early.decorators.early_option] decorators to work properly.
        """
        ...
...
class Null(OptionProtocol[Never]):
    """The [`Null`][wraps.core.option.Null] variant of [`Option[T]`][wraps.core.option.Option]."""
...
class Some(OptionProtocol[T]):
    """[`Some[T]`][wraps.core.option.Some] variant of [`Option[T]`][wraps.core.option.Option]."""
...
"""Optional value, expressed as the union of [`Some[T]`][wraps.core.option.Some]
and [`Null`][wraps.core.option.Null].
"""

NULL = Null()
"""The instance of [`Null`][wraps.core.option.Null]."""


def is_some(option: Option[T]) -> TypeIs[Some[T]]:
    """This is the same as [`Option.is_some`][wraps.core.option.OptionProtocol.is_some],
...
def is_null(option: Option[T]) -> TypeIs[Null]:
    """This is the same as [`Option.is_null`][wraps.core.option.OptionProtocol.is_null],
    except it works as a *type guard*.
    """
...
def wrap_optional(optional: Optional[T]) -> Option[T]:
    """Wraps [`Optional[T]`][typing.Optional] into [`Option[T]`][wraps.core.option.Option].

    If the argument is [`None`][None], [`NULL`][wraps.core.option.NULL] is returned.
    Otherwise the `value` (of type `T`) is wrapped into [`Some(value)`][wraps.core.option.Some].
...
from wraps.core.early.errors import EarlyOption
from wraps.core.result import Error, Ok, Result
from wraps.core.typing import OptionAsyncCallable, OptionCallable
...
class WrapOption(Generic[A]):
    ...
    [`Option[T]`][wraps.core.option.Option].

    Errors are handled via returning [`NULL`][wraps.core.option.NULL] on `error` of
    [`error_types`][wraps.core.option.WrapOption.error_types], wrapping the resulting
    `value` into [`Some(value)`][wraps.core.option.Some].
    """
...
def wrap_option_on(head: Type[A], *tail: Type[A]) -> WrapOption[A]:
    """Creates [`WrapOption[A]`][wraps.core.option.WrapOption] decorators.
    ...
    Returns:
        The [`WrapOption[A]`][wraps.core.option.WrapOption] decorator created.
    """
    return WrapOption(ErrorTypes[A].from_head_and_tail(head, *tail))


wrap_option = wrap_option_on(NormalError)
"""An instance of [`WrapOption[NormalError]`][wraps.core.option.WrapOption]
...
class WrapOptionAwait(Generic[A]):
    ...
    Errors are handled via returning [`NULL`][wraps.core.option.NULL] on `error` of
    [`error_types`][wraps.core.option.WrapOptionAwait.error_types], wrapping the resulting
    `value` into [`Some(value)`][wraps.core.option.Some].
    """
...
def wrap_option_await_on(head: Type[A], *tail: Type[A]) -> WrapOptionAwait[A]:
    """Creates [`WrapOptionAwait[A]`][wraps.core.option.WrapOptionAwait] decorators.
    ...
    Returns:
        The [`WrapOptionAwait[A]`][wraps.core.option.WrapOptionAwait] decorator created.
    """
    return WrapOptionAwait(ErrorTypes[A].from_head_and_tail(head, *tail))


wrap_option_await = wrap_option_await_on(NormalError)
"""An instance of [`WrapOptionAwait[NormalError]`][wraps.core.option.WrapOptionAwait]
(see [`NormalError`][typing_aliases.NormalError]).
"""
