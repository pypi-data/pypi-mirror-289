...
from wraps.core.early.errors import EarlyOption, EarlyResult
from wraps.core.option import NULL, Option
from wraps.core.result import Error, Result
from wraps.core.typing import (
    OptionAsyncCallable,
    OptionCallable,
    ResultAsyncCallable,
    ResultCallable,
...
def early_option(function: OptionCallable[P, T]) -> OptionCallable[P, T]:
    """Decorates the `function` returning [`Option[T]`][wraps.core.option.Option]
...
def early_option_await(function: OptionAsyncCallable[P, T]) -> OptionAsyncCallable[P, T]:
    """Decorates the asynchronous `function` returning [`Option[T]`][wraps.core.option.Option]
    to handle *early returns* via the `early` (`?` in Rust) operator.
...
def early_result(function: ResultCallable[P, T, E]) -> ResultCallable[P, T, E]:
    """Decorates the `function` returning [`Result[T, E]`][wraps.core.result.Result]
    to handle *early returns* via the `early` (`?` in Rust) operator.
...
def early_result_await(function: ResultAsyncCallable[P, T, E]) -> ResultAsyncCallable[P, T, E]:
    """Decorates the asynchronous `function` returning [`Result[T, E]`][wraps.core.result.Result]
    to handle *early returns* via the `early` (`?` in Rust) operator.
