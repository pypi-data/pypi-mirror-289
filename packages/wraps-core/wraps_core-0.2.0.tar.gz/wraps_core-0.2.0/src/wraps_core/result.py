"""Error handling with the [`Result[T, E]`][wraps_core.result.Result] type.

[`Result[T, E]`][wraps_core.result.Result] is the type used for returning and propagating errors.

It is an enum with the variants, [`Ok[T]`][wraps_core.result.Ok],
...
Functions return [`Result[T, E]`][wraps_core.result.Result] whenever errors are expected and recoverable.

For instance, our `divide` function from the [`option`][wraps_core.option] section:
...
"""Result value, expressed as the union of [`Ok[T]`][wraps_core.result.Ok]
and [`Error[E]`][wraps_core.result.Error].
"""
