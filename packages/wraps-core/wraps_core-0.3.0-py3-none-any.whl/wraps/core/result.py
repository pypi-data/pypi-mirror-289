"""Error handling with the [`Result[T, E]`][wraps.core.result.Result] type.

[`Result[T, E]`][wraps.core.result.Result] is the type used for returning and propagating errors.

It is an enum with the variants, [`Ok[T]`][wraps.core.result.Ok],
...
Functions return [`Result[T, E]`][wraps.core.result.Result] whenever errors are expected and recoverable.

For instance, our `divide` function from the [`option`][wraps.core.option] section:
...
from wraps.core.errors import ErrorTypes
from wraps.core.iters import async_empty, async_once, empty, once
from wraps.core.panics import panic
from wraps.core.reprs import wrap_repr
...
class ResultProtocol(AsyncIterable[T], Iterable[T], Protocol[T, E]):  # type: ignore[misc]
    ...
    @required
    def is_ok(self) -> bool:
        """Checks if the result is [`Ok[T]`][wraps.core.result.Ok].
        ...
        Returns:
            Whether the result is [`Ok[T]`][wraps.core.result.Ok].
        """
        ...

    @required
    def is_ok_and(self, predicate: Predicate[T]) -> bool:
        """Checks if the result is [`Ok[T]`][wraps.core.result.Ok] and the value
        ...
        Returns:
            Whether the result is [`Ok[T]`][wraps.core.result.Ok] and the predicate is matched.
        """
        ...
    ...
    async def is_ok_and_await(self, predicate: AsyncPredicate[T]) -> bool:
        ...
        Returns:
            Whether the result is [`Ok[T]`][wraps.core.result.Ok] and
    ...
    def is_error(self) -> bool:
        """Checks if the result is [`Error[E]`][wraps.core.result.Error].
        ...
        Returns:
            Whether the result is [`Error[E]`][wraps.core.result.Error].
        """
        ...

    @required
    def is_error_and(self, predicate: Predicate[E]) -> bool:
        """Checks if the result is [`Error[E]`][wraps.core.result.Error] and the value
        inside of it matches the `predicate`.
        ...
        Returns:
            Whether the result is [`Error[E]`][wraps.core.result.Error] and the predicate is matched.
        """
        ...
    ...
    async def is_error_and_await(self, predicate: AsyncPredicate[E]) -> bool:
        ...
        Returns:
            Whether the result is [`Error[E]`][wraps.core.result.Error] and
            the asynchronous predicate is matched.
        """
        ...
    ...
    def expect(self, message: str) -> T:
        """Returns the contained [`Ok[T]`][wraps.core.result.Ok] value.
        ...
        Raises:
            Panic: Panics with the `message` if the result is [`Error[E]`][wraps.core.result.Error].
    ...
    def expect_error(self, message: str) -> E:
        """Returns the contained [`Error[E]`][wraps.core.result.Error] value.
        ...
        Raises:
            Panic: Panics with the `message` if the result is [`Ok[T]`][wraps.core.result.Ok].
    ...
    @required
    def unwrap(self) -> T:
        """Returns the contained [`Ok[T]`][wraps.core.result.Ok] value (of type `T`).

        Because this function may panic, its use is generally discouraged.

        Instead, prefer to use pattern matching and handle the [`Error[E]`][wraps.core.result.Error]
        case explicitly, or call [`unwrap_or`][wraps.core.result.ResultProtocol.unwrap_or]
        or [`unwrap_or_else`][wraps.core.result.ResultProtocol.unwrap_or_else].
        ...
        Example:
            ...
            Traceback (most recent call last):
              ...
            wraps.core.panics.Panic: called `unwrap` on error
            ```

        Raises:
            Panic: Panics if the result is [`Error[E]`][wraps.core.result.Error].
    ...
    def unwrap_or(self, default: T) -> T:  # type: ignore[misc]
        """Returns the contained [`Ok[T]`][wraps.core.result.Ok] value or the provided `default`.
    ...
    def or_raise(self, error: AnyError) -> T:
        ...
        Arguments:
            error: The error to raise if the result is [`Error[E]`][wraps.core.result.Error].

        Raises:
            AnyError: The error provided, if the result is [`Error[E]`][wraps.core.result.Error].
    ...
    def or_raise_with(self, error: Nullary[AnyError]) -> T:
        ...
        Raises:
            AnyError: The error computed, if the result is
                [`Error[E]`][wraps.core.result.Error].
    ...
    def unwrap_error(self) -> E:
        ...
        Instead, prefer to use pattern matching and handle the [`Ok[T]`][wraps.core.result.Ok]
        case explicitly, or call [`unwrap_error_or`][wraps.core.result.ResultProtocol.unwrap_error_or]
        or [`unwrap_error_or_else`][wraps.core.result.ResultProtocol.unwrap_error_or_else].
        ...
        Example:
            ...
            Traceback (most recent call last):
              ...
            wraps.core.panics.Panic: called `unwrap_error` on ok
            ```

        Raises:
            Panic: Panics if the result is [`Ok[T]`][wraps.core.result.Ok].
    ...
    def unwrap_error_or(self, default: E) -> E:  # type: ignore[misc]
        """Returns the contained [`Error[E]`][wraps.core.result.Error] value (of type `E`)
    ...
    def raising(self: ResultProtocol[T, AnyError]) -> T:
        """Returns the contained [`Ok[T]`][wraps.core.result.Ok] value or raises the
        contained [`Error[AnyError]`][wraps.core.result.Error] value.
        ...
        Raises:
            AnyError: The contained error, if the result is [`Error[AnyError]`][wraps.core.result.Error].
    ...
    def ok(self) -> Option[T]:
        """Converts a [`Result[T, E]`][wraps.core.result.Result]
        into an [`Option[T]`][wraps.core.option.Option].

        Converts `self` into an [`Option[T]`][wraps.core.option.Option], discarding errors, if any.
    ...
    def error(self) -> Option[E]:
        """Converts a [`Result[T, E]`][wraps.core.result.Result]
        into an [`Option[E]`][wraps.core.option.Option].

        Converts `self` into an [`Option[E]`][wraps.core.option.Option],
    ...
    def inspect(self, function: Inspect[T]) -> Result[T, E]:
        """Inspects a possibly contained [`Ok[T]`][wraps.core.result.Ok] value.
    ...
    def inspect_error(self, function: Inspect[E]) -> Result[T, E]:
        """Inspects a possibly contained [`Error[E]`][wraps.core.result.Error] value.
    ...
    def map(self, function: Unary[T, U]) -> Result[U, E]:
        """Maps a [`Result[T, E]`][wraps.core.result.Result] to a [`Result[U, E]`][wraps.core.result.Result]
        by applying `function` to the contained [`Ok[T]`][wraps.core.result.Ok] value,
        leaving any [`Error[E]`][wraps.core.result.Error] values untouched.
    ...
    def map_error(self, function: Unary[E, F]) -> Result[T, F]:
        """Maps a [`Result[T, E]`][wraps.core.result.Result] to a [`Result[T, F]`][wraps.core.result.Result]
        by applying the `function` to the contained [`Error[E]`][wraps.core.result.Error] value,
        leaving any [`Ok[T]`][wraps.core.result.Ok] values untouched.
    ...
    async def map_await(self, function: AsyncUnary[T, U]) -> Result[U, E]:
        """Maps a [`Result[T, E]`][wraps.core.result.Result] to a [`Result[U, E]`][wraps.core.result.Result]
        by applying the asynchronous `function` to the contained [`Ok[T]`][wraps.core.result.Ok] value,
        leaving any [`Error[E]`][wraps.core.result.Error] values untouched.
    ...
    async def map_error_await(self, function: AsyncUnary[E, F]) -> Result[T, F]:
        """Maps a [`Result[T, E]`][wraps.core.result.Result] to a [`Result[T, F]`][wraps.core.result.Result]
        by applying the asynchronous `function` to the contained [`Error[E]`][wraps.core.result.Error]
        value, leaving any [`Ok[T]`][wraps.core.result.Ok] values untouched.
    ...
    def async_iter(self) -> AsyncIterator[T]:
        ...
        [`Ok[T]`][wraps.core.result.Ok] value.
    ...
    def async_iter_error(self) -> AsyncIterator[E]:
        """Returns an asynchronous iterator over the possibly contained
        [`Error[E]`][wraps.core.result.Error] value.
    ...
    def and_then(self, function: Unary[T, Result[U, E]]) -> Result[U, E]:
        """Returns the result if it is an [`Error[E]`][wraps.core.result.Error],
    ...
    def or_else(self, function: Unary[E, Result[T, F]]) -> Result[T, F]:
        """Returns the result if it is [`Ok[T]`][wraps.core.result.Ok], otherwise calls the `function`
    ...
    async def or_else_await(self, function: AsyncUnary[E, Result[T, F]]) -> Result[T, F]:
        """Returns the result if it is [`Ok[T]`][wraps.core.result.Ok], otherwise calls the asynchronous
    ...
    def try_flatten(self: ResultProtocol[ResultProtocol[T, E], E]) -> Result[T, E]:
        """Flattens a [`Result[Result[T, E], E]`][wraps.core.result.Result]
        into a [`Result[T, E]`][wraps.core.result.Result].

        This is equivalent to [`result.and_then(identity)`][wraps.core.result.ResultProtocol.and_then].
    ...
    def try_flatten_error(self: ResultProtocol[T, ResultProtocol[T, E]]) -> Result[T, E]:
        """Flattens a [`Result[T, Result[T, E]]`][wraps.core.result.Result]
        into a [`Result[T, E]`][wraps.core.result.Result].

        This is equivalent to [`result.or_else(identity)`][wraps.core.result.ResultProtocol.or_else].
    ...
    def flip(self) -> Result[E, T]:
        """Converts a [`Result[T, E]`][wraps.core.result.Result]
        into a [`Result[E, T]`][wraps.core.result.Result].

        [`Ok(value)`][wraps.core.result.Ok] and [`Error(error)`][wraps.core.result.Error] get swapped to
        [`Error(value)`][wraps.core.result.Error] and [`Ok(error)`][wraps.core.result.Ok] respectively.
    ...
    def into_ok_or_error(self: ResultProtocol[V, V]) -> V:
        """Returns the value contained within [`Result[V, V]`][wraps.core.result.Result], regardless
        of whether or not that result is [`Ok[V]`][wraps.core.result.Ok]
        or [`Error[V]`][wraps.core.result.Error].
    ...
    def into_either(self) -> Either[T, E]:
        """Converts a [`Result[T, E]`][wraps.core.result.Result]
        into an [`Either[T, E]`][wraps.core.either.Either].

        [`Ok(value)`][wraps.core.result.Ok] is mapped to [`Left(value)`][wraps.core.either.Left]
        and [`Error(error)`][wraps.core.result.Error] is mapped to [`Right(error)`][wraps.core.either.Right].
    ...
    def early(self) -> T:
        ...
        [`@early_result`][wraps.core.early.decorators.early_result] decorators to work properly.
        """
        ...
...
class Ok(ResultProtocol[T, Never]):
    """[`Ok[T]`][wraps.core.result.Ok] variant of [`Result[T, E]`][wraps.core.result.Result]."""
...
class Error(ResultProtocol[Never, E]):
    """[`Error[E]`][wraps.core.result.Error] variant of [`Result[T, E]`][wraps.core.result.Result]."""
...
"""Result value, expressed as the union of [`Ok[T]`][wraps.core.result.Ok]
and [`Error[E]`][wraps.core.result.Error].
"""


def is_ok(result: Result[T, E]) -> TypeIs[Ok[T]]:
    """This is the same as [`Result.is_ok`][wraps.core.result.ResultProtocol.is_ok],
...
def is_error(result: Result[T, E]) -> TypeIs[Error[E]]:
    """This is the same as [`Result.is_error`][wraps.core.result.ResultProtocol.is_error],
    except it works as a *type guard*.
    """
...
from wraps.core.early.errors import EarlyResult
from wraps.core.either import Either, Left, Right
from wraps.core.option import NULL, Null, Option, Some
from wraps.core.typing import ResultAsyncCallable, ResultCallable
...
class WrapResult(Generic[A]):
    ...
    [`Result[T, E]`][wraps.core.result.Result].

    Errors are handled via returning [`Error(error)`][wraps.core.result.Error] on `error` of
    [`error_types`][wraps.core.result.WrapResult.error_types], wrapping the resulting
    `value` into [`Ok(value)`][wraps.core.result.Ok].
    """

    error_types: ErrorTypes[A]
    """The error types to handle. See [`ErrorTypes[A]`][wraps.core.errors.ErrorTypes]."""
...
def wrap_result_on(head: Type[A], *tail: Type[A]) -> WrapResult[A]:
    """Creates [`WrapResult[A]`][wraps.core.result.WrapResult] decorators.
    ...
    Returns:
        The [`WrapResult[A]`][wraps.core.result.WrapResult] decorator created.
    """
    return WrapResult(ErrorTypes[A].from_head_and_tail(head, *tail))


wrap_result = wrap_result_on(NormalError)
"""An instance of [`WrapResult[NormalError]`][wraps.core.result.WrapResult]
...
def wrap_result_await_on(head: Type[A], *tail: Type[A]) -> WrapResultAwait[A]:
    """Creates [`WrapResultAwait[A]`][wraps.core.result.WrapResultAwait] decorators.
    ...
    Returns:
        The [`WrapResultAwait[A]`][wraps.core.result.WrapResultAwait] decorator created.
    """
    return WrapResultAwait(ErrorTypes[A].from_head_and_tail(head, *tail))


wrap_result_await = wrap_result_await_on(NormalError)
"""An instance of [`WrapResultAwait[NormalError]`][wraps.core.result.WrapResultAwait]
(see [`NormalError`][typing_aliases.NormalError]).
"""
