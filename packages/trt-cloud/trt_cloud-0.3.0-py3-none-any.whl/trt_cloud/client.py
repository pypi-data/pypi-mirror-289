...
class TRTCloud:
    ...
    def _select_function(
        ...
        for nvcf_func in available_functions:
            if (
                nvcf_func.os == os_name and
                nvcf_func.gpu == gpu and
    ...
    def start_trtllm_build(
        ...
        if isinstance(build_input_source, TRTLLMCheckpointSource):
            if build_return_type in [
                ...
                raise ValueError(
                    "Builds from TRT LLM checkpoints cannot currently produce metrics. "
                    "If metrics are needed, please build from a HF checkpoint."
                )
        ...
        if isinstance(build_input_source, TRTLLMCheckpointSource) and not build_input_source.is_url:
            with TemporaryDirectory() as weight_strip_output_dir, \
                ...
                logging.info(f"Uploading {trt_llm_pruned_checkpoint_filepath}")
                nvcf_asset_ids = self._cached_nvcf_asset_upload(trt_llm_pruned_checkpoint_filepath)
