...
from quickstats.utils.string_utils import split_str, get_field_names
...
class ExtendedModel(AbstractObject):
    ...
    def get_collected_distributions(self, categories:Optional[List[str]]=None,
                                    ...
                                    nbins:Optional[Union[Dict[str, int], int]]=None,
                                    n_pdf_bins:Optional[Union[Dict[str, int], int]]=None,
                                    bin_range:Optional[Union[Dict[str, Tuple[float]], Tuple[float]]]=None,
                                    remove_ghost:bool=True,
                                    merge:bool=False):
        if categories is None:
    ...
    def plot_distributions(self, categories:Optional[List[str]]=None,
                           current_distribution=True,
                           ...
                           unit:Optional[str]=None, blind:bool=False,
        ...
        if blind:
            blind_ranges = self.get_blind_range()
            if merge:
                blind_ranges[None] = list(blind_ranges.values())[0]
        else:
            blind_ranges = {}
        
        for i, (category, distributions) in enumerate(collected_distributions.items()):
            ...
            if discriminant is None:
                xlabel = obs_name or 'Observable'
            else:
                if 'category' in get_field_names(discriminant):
                    cat_label = category_label_map.get(category, category)
                    xlabel = discriminant.format(category=cat_label)
                else:
                    xlabel = discriminant
            # need to fix scaling for distributions with different bin widths
            all_bin_centers = [distribution['x'] for distribution in distributions.values()]
            all_bin_diffs = [np.diff(bin_centers) for bin_centers in all_bin_centers]
            all_bin_widths = []
            for bin_diffs in all_bin_diffs:
                if len(np.unique(bin_diffs)) != 1:
                    raise RuntimeError('Plotting with non-uniform bin width is not supported')
                all_bin_widths.append(bin_diffs[0])
            unique_bin_widths = np.sort(np.unique(all_bin_widths))
            assert len(unique_bin_widths) <= 2
            if len(unique_bin_widths) == 1:
                bin_width = unique_bin_widths[0]
            else:
                bin_width = unique_bin_widths[1]
                ratio = unique_bin_widths[1] / unique_bin_widths[0]
                for i, key in enumerate(distributions):
                    if all_bin_widths[i] == unique_bin_widths[0]:
                        # should only affect pdf distributions, so no yerr here
                        distributions[key]['y'] *= ratio

            bin_width = pretty_value(bin_width)
            ylabel    = f"Events / {bin_width}"
            if unit is not None:
                xlabel = f"{xlabel} [{unit}]"
                ylabel = f"{ylabel} {unit}"

            blind_range = blind_ranges.get(obs_name, None)
            plotter = PdfDistributionPlot(distributions,
                                          figure_index=i, **init_options)
            if plotter.analysis_label_options is not None:
                options = plotter.analysis_label_options
                if ('extra_text' in options) and \
                ('category' in get_field_names(options['extra_text'])):
                    cat_label = category_label_map.get(category, category)
                    options['extra_text'] = options['extra_text'].format(category=cat_label)
            targets = [key for key in distributions if not key.endswith('_binned')]
