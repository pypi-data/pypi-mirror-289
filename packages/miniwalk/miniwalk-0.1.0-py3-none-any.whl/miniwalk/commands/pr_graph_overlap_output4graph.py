    ...
    for c in call.index:
        ...
        for v in vcf.index:
            ...
            if v+1 != len(vcf.index):
                ...
                if P2 > F2:
                    F2 = P2
                if F1 > F2 and P1 >= vcf.iloc[v+1,1] and F1 - F2 >= ref_len and P1 - vcf.iloc[v+1,1] >= int(vcf.iloc[v+1,2].split('.')[1])*0.25 and (F1 - F2) + (P1 - vcf.iloc[v+1,1]) >= sam_len and call.iloc[c,2].split('.')[0] == vcf.iloc[v,7].split(';')[0].split('=')[1] and call.iloc[c,2].split('.')[0] == vcf.iloc[v+1,7].split(';')[0].split('=')[1]:
                    tp += 1
            ...
            if repe==True: #if our call and std SVs arent overlapping BUT they are in a TR region, this may still be the same SV, we will compare their adjacent sequences to make sure
                if call.iloc[c,2].split('.')[0] == vcf.iloc[v,2].split('.')[0] and vcf.iloc[v,1] >= rep_pos_start and vcf.iloc[v,1] <= rep_pos_end and int(aligner.score(clean_sequence(h37rv_seq[call.iloc[c,1]-50:call.iloc[c,1]+int(call.iloc[c,2].split('.')[1])+50]),clean_sequence(h37rv_seq[vcf.iloc[v,1]-50:vcf.iloc[v,1]+50+int(vcf.iloc[v,2].split('.')[1])]))) >= int((min(int(vcf.iloc[v,2].split('.')[1]),int(call.iloc[c,2].split('.')[1]))+100)*0.5):
                    ...
                    if call.iloc[c,2].split('.')[0] == "DEL":
                        ...
                        print("%s,%i,TP"%(call.iloc[c,2].split('.')[0],int(call.iloc[c,2].split('.')[1])))
                    break
            if v+1 != len(vcf.index) and F1 > F2 and P1 >= vcf.iloc[v+1,1] and call.iloc[c,1] >= vcf.iloc[v,1] and (F1 - F2) + (P1 - vcf.iloc[v+1,1]) >= sam_len and call.iloc[c,2].split('.')[0] == vcf.iloc[v,2].split('.')[0] and call.iloc[c,2].split('.')[0] == vcf.iloc[v+1,2].split('.')[0]:
                tp += 1
                vcf_tp.append(vcf.iloc[v,1])
                vcf_tp.append(vcf.iloc[v+1,1])
                sv = True
