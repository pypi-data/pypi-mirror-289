...
class UserData(BaseModel):
    ...
    user_department: constr(min_length=1)
    domain_name: str = Field(default='unknown')
    session_id: str = Field(default='unknown')
    action_type: constr(min_length=1)
    action_details: str = Field(default='')
    source_ip: str = Field(default='0.0.0.0')
    user_agent: str = Field(default='unknown')
    resource_id: str = Field(default=None)
    developer: str = Field(default='cameo')

    @model_validator(mode='after')
    def check_required_fields(cls, values):
        required_fields = ['user_name', 'user_department', 'action_type']
        missing_fields = [field for field in required_fields if not values.__dict__.get(field)]
        if missing_fields:
            raise ValueError(f"Missing required fields: {', '.join(missing_fields)}")
        # 檢查 action_details 是否為有效的 JSON
        try:
            json.loads(values.get('action_details', '{}'))
...
class BigQueryLogger:
    ...
    def insert_data_to_table(self, user_data: dict):
        ...
        try:
            ...
            errors = self.client.insert_rows_json(f"{self.client.project}.{dataset_id}.{table_id}", data)
            if errors:
