...
class SeekApiClient:
    ...
    def _filter_infos(
        ...
        for result in response.get("hits", {}).get("hits", []):
            ...
            if any(blacklisted_item in content for blacklisted_item in BLACKLIST):
                continue
    ...
    def search_documents(
        ...
        try:
            ...
            return self._filter_infos(response.raw, include_filename=display_filename)
    ...
    @staticmethod
    def _extract_information(text: str) -> Dict[str, set]:
        """
        Extrait les informations spécifiques (emails, numéros de téléphone, identifiants Steam et licences FiveM) à partir du texte.
