...
class SeekApiClient:
    ...
    @staticmethod
    def _filter_infos(response, include_filename: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def extracted_search(self, documents: List[Dict[str, Any]]) -> ExtractedInfo:
        ...
        """
        all_information: Dict[str, Set[str]] = {
            "emails": set(),
            "phones": set(),
            "fivem_licenses": set(),
            "steam_ids": set(),
        ...
        for doc in documents:
            ...
            if isinstance(content, str):
                try:
                    ...
                    logging.warning(
                        f"JSON decoding error in document {doc.get('filename', '')}"
                    )
                except TypeError:
                    logging.error(
                        f"Type error while processing content in document {doc.get('filename', '')}"
                    )
