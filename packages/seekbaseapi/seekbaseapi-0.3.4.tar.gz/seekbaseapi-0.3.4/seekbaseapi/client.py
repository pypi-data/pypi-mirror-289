...
class SeekApiClient:
    ...
    @staticmethod
    def _filter_infos(response) -> list:
        """
        Filtre les recherches de la requête API
        - Elements de la BLACKLIST (utils) sont supprimés

        :param response: Résultat de la requête SEEKBASE
        :return La liste des résultats triés
        """
        results = [
            result.get("_source", {}).get("content", "")
            for result in response["hits"]["hits"]
            if not any(
                term in result.get("_source", {}).get("content", "")
    ...
    def search_documents(
        ...
        search_query = {
            "size": size,
            "query": {"term": {"content.keyword": search_string.lower()}},
        }
        ...
        try:
            ...
            return self._filter_infos(response)
