...
def generic_backstepping_controller(num_states, state_equations, control_input, gains, tracking=False):
    # Define symbolic variables
    states = sp.symbols(f'x1:{num_states+1}')  # Create symbols x1, x2, ..., xn
    u = sp.Symbol(control_input)
    
    if tracking:
        refs = sp.symbols(f'x1_ref:{num_states+1}')  # Create reference symbols x1_ref, x2_ref, ..., xn_ref
        errors = [states[i] - refs[i] for i in range(num_states)]  # Create error terms ei = xi - xi_ref
    else:
        errors = states
    ...
    if tracking:
        return final_control_law, states, gains, refs, errors
    else:
