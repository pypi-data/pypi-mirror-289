...
def omit_false_entries(dictionary: dict[str, Any], 
                       except_: list[str] | None = None) -> dict[str, Any]:
    if except_ is None:
        except_ = []

    return { key: val for key, val in dictionary if val or key in except_ }
...
class StreamExtent:
    ...
    filters: list[PdfName] | None = None
    """The filters the document is encoded in."""
    
    decode_params: list[dict[str, Any]] | None = None
    """The decode parameters for each of the specified filters."""
    
    file: str | None = None
    """The file the stream's contents are stored in."""
    
    file_filters: list[PdfName] | None = None
    """Same as ``filters`` but applicable for ``file``"""
    
    file_decode_params: list[dict[str, Any]] | None = None
    """Same as ``decode_params`` but applicable for ``file``"""
    
    decoded_length: int | None = None
    """The number of bytes in the decoded stream. This value should only be used as 
    hint by parsers."""

    def to_dict(self) -> dict[str, Any]:
        return omit_false_entries({
            "Length": self.length,
            "Filter": self.filters,
            "DecodeParms": self.decode_params,
            "F": self.file,
            "FFilter": self.file_filters,
            "FDecodeParms": self.file_decode_params,
            "DL": self.decoded_length
        }, ["Length"])
...
class TrailerOld(TypedDict, total=False):
    Size: Required[int]
