    ...
    def viewRoutine(self,returnable=False):
        with Session(ENGINE) as session:
            ...
            if ct < 1:
                ...
                if returnable:
                    return lite
    def useRoutine(self,reset=False):
        try:
            start=datetime.now()
                ...
                with Session(ENGINE) as session:
                    ...
                    if reset:
                        for num, i in enumerate(rout):
                                ...
                                session.refresh(exercise)
                        return
                    returnIndex=session.query(Routine,Exercise).join(Exercise,Routine.exid==Exercise.exid).filter(Routine.Name==routine).order_by(Exercise.CurrentRep.asc()).first()
                    print("ct",ct,"completed",completed)
                    while completed <= ct:
                        for num, i in enumerate(rout):
                            exercise=session.query(Exercise).filter(Exercise.exid==i.exid).first()
                            if exercise.CurrentRep == exercise.Reps:
                                completed_reps+=1
                                continue
                            if returnIndex and len(returnIndex) > 1:
                                if exercise.CurrentRep > returnIndex[-1].CurrentRep:
                                    completed_reps+=1
                                    continue
                            if exercise.CurrentRep < exercise.Reps:
    ...
    def __init__(self):
        ...
        while True:
            ...
            if doWhat in [None,]:
                ...
                self.newRoutine()
            elif doWhat.lower() in ['vr','ViewRoutine']:
                self.viewRoutine()
            elif doWhat.lower() in ['ur','UseRoutine']:
                self.useRoutine()
            elif doWhat.lower() in ['rr','RemoveRoutine']:
                self.removeRoutine()
            elif doWhat.lower() in ['cr','ClearRoutine']:
                self.clearRoutine()