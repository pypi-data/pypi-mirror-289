...
def tokenize(
    ...
    encrypt = udf(
        crypto.make_deterministic_encrypter(derive_aes_key(private_key)), returnType=BinaryType()
    )
...
def transcrypt_out(
    ...
    """
    decrypt = udf(
        crypto.make_deterministic_decrypter(derive_aes_key(private_key)), returnType=BinaryType()
    )
    encrypt = udf(
...
def transcrypt_in(
    ...
    """
    decrypt = udf(
        crypto.make_asymmetric_decrypter(private_key), returnType=BinaryType()
    )
    encrypt = udf(
        crypto.make_deterministic_encrypter(derive_aes_key(private_key)), returnType=BinaryType()
    )
