...
def extract_torrent_info(html_content):
    ...
    # imdb_id (Extract from HTML links or plain text)
    imdb_id = None
    # 1. Check within <a> tags
    for link in soup.find_all('a', href=True):
        match = re.search(r'tt\d+', link['href'], re.IGNORECASE)
        if match:
            imdb_id = match.group(0).lower()
            break  # Stop if found in a link

    # 2. Check in plain text if not found in links
    if not imdb_id:
        match = re.search(r'tt\d+', text, re.IGNORECASE)
        if match:
            imdb_id = match.group(0).lower()
