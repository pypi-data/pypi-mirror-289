...
class WorkFlowLogic:
    ...
    def run_workflows(self, flows: list, flow_data_file: str = None):
        project_path = "_gmx"
        for flow in flows:
    ...
    def _process_flow(self, flow: str, project_path: str, flow_data_file: str = None):
        flow_path = os.path.join(project_path, 'workflows', f'{flow}.yaml.j2')
        ...
        try:
            if flow_data_file:
                ...
                with open(flow_path, 'r') as file:
                    workflow_string = file.read()
                workflow_template = Template(workflow_string)
                rendered_workflow = workflow_template.render()
                items = yaml.safe_load(rendered_workflow)
