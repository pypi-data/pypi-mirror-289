...
from pymongo import errors, MongoClient, database, collection
...
class Base:
    ...
    @classmethod
    def initialize_client(cls, client: MongoClient):
        # 클래스를 사용하기전에 클라이언트를 연결하여 초기화해준다.
        cls.client = client
        print(f"Base class client initialized to {extract_addr_from_client(cls.client)}")

    def __init__(self, db: str, table: str):
        if Base.is_there(db=db, table=table):
            self._db = Base.client[db]
    ...
    @db.setter
    def db(self, db: str):
        assert isinstance(Base.client, MongoClient), "You should set server connection first"
    ...
    def list_db_names(cls) -> list:
        assert isinstance(cls.client, MongoClient), "You should set server connection first"
        return sorted(cls.client.list_database_names())

    @classmethod
    def list_table_names(cls, db: str) -> list:
        assert isinstance(cls.client, MongoClient), "You should set server connection first"
        if Base.is_there(db=db):
            return sorted(cls.client[db].list_collection_names())
        else:
    ...
    def drop(cls, **kwargs) -> bool:
        ...
        if db != '' and table == '':
            ...
            try:
                ...
                if table in tables:
                    cls.client[db].drop_collection(table)
                    return True
                else:
...
class Corps(Base):
    ...
    def is_there(cls, **kwargs) -> bool:
        """
        서버내에 kwargs에 해당하는 code 및 page가 있는지 확인해서 참거짓 반환하는 함수
        :param kwargs: code, page
        :return:
        """
        code = kwargs.get('code', '')
        page = kwargs.get('page', '')
        assert utils.is_6digit(code) or code == '', f'Invalid value : {code}'
        assert page in cls.COLLECTIONS or page == '', f'Invalid value : {page}({cls.COLLECTIONS})'
        return Base.is_there(db=code, table=page)
    ...
    @classmethod
    def drop_c101_page_in_all(cls):
        codes = cls.list_all_codes()
        for code in codes:
            cls.client[code].drop_collection('c101')
    ...
    def _save_df(cls, code: str, page: str, df: pd.DataFrame, index_unique: bool, clear_table: bool):
        ...
        if clear_table:
            ...
            cls.client[code][page].delete_many({})
            import time
            time.sleep(.5)

        cls.client[code][page].insert_many(df.to_dict('records'))
        cls.client[code][page].create_index('항목', unique=index_unique)
...
class C101(Corps):
    ...
    def __init__(self, code: str):
        ...
        self._col.create_index('date', unique=True)
    ...
    def save(cls, code: str,  c101_data: dict) -> bool:
        ...
        if (set(c101_template) - set(c101_data.keys())) == set():
            ...
            try:
                result = cls.client[code][page].insert_one(c101_data)
            except errors.DuplicateKeyError:
                cls.client[code][page].delete_many(del_query)
                result = cls.client[code][page].insert_one(c101_data)
    ...
    @staticmethod
    def _merge_intro(c101_dict: dict) -> dict:
    ...
    def get_recent(self, merge_intro=False) -> dict:
        ...
        """
        try:
            d = dict(self._col.find({'date': {'$exists': True}}, {"_id": 0}).sort('date', pymongo.DESCENDING).next())
            # del doc['_id'] - 위의 {"_id": 0} 으로 해결됨.
            if merge_intro:
                d = C101._merge_intro(d)
    ...
    def get_trend(self, title: str) -> dict:
        ...
        items = dict()
        for doc in self._col.find({'date': {'$exists': True}}, {"_id": 0, "date": 1, f"{title}": 1}).sort('date', pymongo.ASCENDING):
...
class C106(Corps):
    ...
    def save(cls, code: str, page: str, c106_df: pd.DataFrame):
        ...
        clear_table = False
        if cls.client[code][page].count_documents({}) != 0:
            clear_table = True
...
class C104(C1034):
    ...
    def save(cls, code: str, page: str, c104_df: pd.DataFrame):
        ...
        clear_table = False
        try:
            stamp = cls.client[code][page].find_one({'항목': 'stamp'})['time']
        ...
        cls.client[code][page].update_one({'항목': 'stamp'}, {"$set": {'time': time_now}}, upsert=True)
...
class C108(Corps):
    ...
    def get_recent_date(self) -> datetime.datetime | None:
        ...
        try:
            r_date = self._col.find({'날짜': {'$exists': True}}).sort('날짜', pymongo.DESCENDING).next()['날짜']
        except StopIteration:
            # 날짜에 해당하는 데이터가 없는 경우
            return None
