...
def_mongo_addr = "mongodb://hj3415:piyrw421@192.168.100.175:27017/"
def_redis_addr = "192.168.100.175"
...
def load_redis_addr() -> str:
    ...
    if os.path.exists(FILE_PATH):
        with open(FILE_PATH, 'r') as file:
            settings = json.load(file)
            return settings[redis_addr_title]
    else:
        # 기본값
...
def db():
    # mongo db 초기화
    mongo_client = mongo.connect_mongo(load_mongo_addr())
    mongo.Base.initialize_client(mongo_client)
