...
# 加载默认响应模型
DJANGO_APIS_BASE_RESPONSE_CLASS = getattr(
    settings,
    "DJANGO_APIS_BASE_RESPONSE_CLASS",
    BaseResponse,
)
if isinstance(DJANGO_APIS_BASE_RESPONSE_CLASS, str):
    DJANGO_APIS_BASE_RESPONSE_CLASS = importutils.import_from_string(
        DJANGO_APIS_BASE_RESPONSE_CLASS
    )
...
class Apiview(object):
    ...
    def __call__(self, methods="GET"):
        ...
        def view(func):
            ...
            setattr(func, DJANGO_APIS_VIWE_FLAG_KEY, True)
    ...
    def is_request_method_allowed(self, request, methods):
        if isinstance(methods, str):
            return request.method == methods.upper()
        elif isinstance(methods, (list, set, tuple)):
            return request.method in methods
        else:
            return False

    def request_method_check(self, request, methods):
        if not self.is_request_method_allowed(request, methods):
