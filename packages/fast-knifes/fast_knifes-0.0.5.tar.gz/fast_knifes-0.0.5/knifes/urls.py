...
def get_domain_with_top_level_from_url(url) -> str | None:
    if not url:
        return url
...
def get_domain_from_url(url) -> str | None:
    if not url:
        return url
    u = parse.urlparse(url)
    host_list = u.hostname.split(".")
...
def guess_media_type(url) -> MediaType | None:
    ...
    if not mimetype:
        return extension_to_media_type_dict.get(
            get_extension(url_without_query.lower())
        )
...
def parse_path(url, index_: int | None = None) -> Union[list, str, None]:
