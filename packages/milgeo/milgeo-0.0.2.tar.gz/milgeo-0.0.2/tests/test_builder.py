...
def test_featurebuilder_basic_elements(sample_point_geometry):
    builder = FeatureBuilder(sample_point_geometry)
...
def test_featurebuilder_optional_properties(sample_point_geometry):
    sample_point_geometry.observation_datetime = "2024-01-01T00:00:00"
    sample_point_geometry.quantity = "5"
    builder = FeatureBuilder(sample_point_geometry)
...
def test_featurebuilder_geometry(sample_point_geometry):
    builder = FeatureBuilder(sample_point_geometry)
    builder.add_geometry()
    feature = builder.build()
    assert feature["geometry"]["type"] == sample_point_geometry.geometry_type
    assert feature["geometry"]["coordinates"] == sample_point_geometry.coordinates


def test_placemarkbuilder_basic_elements(sample_point_geometry):
    builder = PlacemarkBuilder(sample_point_geometry)
    builder.add_basic_elements()
...
def test_placemarkbuilder_optional_properties(sample_point_geometry):
    builder = PlacemarkBuilder(sample_point_geometry)
    builder.add_optional_properties()
    placemark = builder.build()
    placemark_xml = fromstring(placemark)
...
def test_placemarkbuilder_point_geometry(sample_point_geometry):
    builder = PlacemarkBuilder(sample_point_geometry)
    builder.add_geometry()
    placemark = builder.build()
    placemark_xml = fromstring(placemark)
