    ...
    def start_job(self, request, queryset):
        ...
        for (trigger, rate, id_, name, type_, time_) in zip(trigger_list, rate_list, id_list, name_list, action_type,
            ...
            try:
                ...
                if seconds < 60:
                    messages.warning(request, 'Please start %s again with %ss' % (job_name, str(60 - seconds)))
                    job_num -= 1
                    continue
                if hasattr(JobAction(), 'start_%s_job' % job_type):
                    ...
                    messages.error(request, '【start failed】: ' + job_name + ' job does not exists!!')
                    job_num -= 1
            except ConflictingIdError as e:
                logger.error(e)
                messages.error(request, job_name + ' job startup conflict occurred!!')
                job_num -= 1
            except Exception as e:
                logger.error(e)
                messages.error(request, job_name + ' job start error！<br/>Check the scheduling type or frequency')
                job_num -= 1
        JobAction().get_jobs()
        if job_num > 0 and len(id_list) > 1:
            messages.success(request,
                             ('the rest' if job_num != len(
                                 id_list) else 'selected') + ' %s jobs were successfully started' % str(job_num))
        elif job_num == len(id_list) == 1:
            messages.success(request, '%s job started successfully' % job_name)
        return messages
    ...
    def stop_job(self, request, queryset):
        ...
        for (trigger, id_, name) in zip(trigger_list, id_list, name_list):
            try:
                ...
                logger.error(e)
                job_num -= 1
                messages.error(request, job_name + " job does not exists!！！")
            except Exception as e:
                logger.error(e)
                job_num -= 1
                messages.error(request, job_name + ' job stop error！')
        JobAction().get_jobs()
        if job_num > 0 and len(id_list) > 1:
            messages.success(request,
                             ('the rest' if job_num != len(
                                 id_list) else 'selected') + ' %s jobs were successfully stopped' % str(job_num))
        elif job_num == len(id_list) == 1:
            messages.success(request, "%s has been stopped" % job_name)
        return messages
