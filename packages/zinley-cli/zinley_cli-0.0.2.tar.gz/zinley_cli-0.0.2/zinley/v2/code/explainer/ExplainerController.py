...
class ExplainerController:
    ...
    def __init__(self, directory_path, api_key, endpoint, deployment_id, max_tokens):
        ...
        self.preprompt = ExplainablePrePromptAgent(os.path.join(HOME_DIRECTORY, HIDDEN_ZINLEY_FOLDER, project_name, "Zinley/Project_analysis"), api_key, endpoint, deployment_id, max_tokens)
        self.normalExplainer = GeneralExplainerAgent(os.path.join(HOME_DIRECTORY, HIDDEN_ZINLEY_FOLDER, project_name, "Zinley/Project_analysis"), api_key, endpoint, deployment_id, max_tokens)
    ...
    def get_txt_files(self):
        ...
        if not os.path.exists(txt_path):
            logger.info(f"Directory does not exist: {txt_path}")
            return txt_files
