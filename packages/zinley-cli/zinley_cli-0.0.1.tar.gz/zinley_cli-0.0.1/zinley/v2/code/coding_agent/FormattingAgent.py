...
class FormattingAgent:
    ...
    def initial_setup(self, user_prompt, role):
        ...
        """
        prompt = (
            f"You are a senior {role} and formatter agent. Follow user request but must ensure the functionality remains the same. Respond with only the reformatted code for the file. Do not remove the file's default information at the top. Respond with only valid code without additional description or any Markdown symbols."
        )
    ...
    async def get_format(self, session, file):
        ...
        if file_content:
            ...
            prompt = f"Now work on this file. Respond with only valid code without additional description or any Markdown symbols: {file_content}"
