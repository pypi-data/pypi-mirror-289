...
from .types import TimeType, TimeDeltaType
...
Keyword arguments:
    ...
    min_fraction [float, default: 0.5]:
        In case sync(hronisation) is set, ensure at least 50% of the sampling
        interval is indeed sampled.  It means that the start can be delayed
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if sync:
                ...
                earliest = ceil(now - offset, sampling) + sampling 
                start = max(floor(t0, interval), earliest)
            else:
                end = now + interval
                start = now  
