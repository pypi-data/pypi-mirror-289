...
class GENERATOR():
    ...
    def create_pose_video_from_MP(self,img_centre,min_size,FPS,pose_tensor,GAN_model, only_pose=False):
        ...
        for input_frame in reader:
            ...
            heat_maps, hand_crops, copy_face = HEATMAPS().get_heatmaps_crops(input_frame=input_frame,
        ...
        if self.args.mouth_flag:
            file_path = f"./inputs/mouthing.npy"
            if os.path.exists(file_path):
                os.remove(file_path)                
            else:
                np.save(file_path,all_faces)
