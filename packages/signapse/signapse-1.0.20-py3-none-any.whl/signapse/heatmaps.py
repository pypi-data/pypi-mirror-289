...
class HEATMAPS():
    ...
    def crop_hand_gray_normalised(self,signer,frame, frame_no, hand,face_mask,SAM_hand=False, SAM_face=False, SAM_skin=False,SAM_hand_edge = False, SAM_face_edge = False, SAM_skin_edge = False):
        if SAM_hand:
        ...
        if SAM_skin:
            skin = self.skin_detection(frame,signer,include_hair = False) /255  #(SAM_face or SAM_hand)
        ...
        if signer == "Rachel" and SAM_hand and SAM_face and SAM_skin:
            ...
            if os.path.exists(file_path):
                ...
                if frame_no < len(data):
                    ...
                    skin = np.where(skin > -1, skin, mask_zeros) #np.where(mask_zeros > -1, mask_zeros, skin) #mask_zeros/2
            else:
                skin = self.Rachel_hair_removal(face_mask,hand_mask,skin_mask)
                skin = np.array(self.get_edge(torch.from_numpy(skin)))                
            return skin, skin[:,160:430,400:630]
    ...
    def get_heatmaps_crops(self,input_frame,MP_results_gen,GAN_model, opt,iii=0, hand_mask=None,face_mask=None):
        ...
        if opt.sam:
            # do sam
            cropped_hand, copy_face = self.crop_hand_gray_normalised(opt.signer,input_frame, iii,hand_mask,face_mask,SAM_hand=opt.hand_crop, SAM_face=opt.face_crop, SAM_skin=opt.skin_crop, \
            ...
            if opt.crop_normalise:
                ...
                if opt.hand_crop and opt.face_crop and opt.skin_crop:
                    ...
                    cropped_hand[0] = np.array(handcrop_transform(torch.from_numpy(cropped_hand[0].astype('float16')).unsqueeze(dim=0)))
                elif opt.face_crop : #and not opt.signer =="Rachel"
                    cropped_hand[1] = np.array(handcrop_transform(torch.from_numpy(cropped_hand[1].astype('float16')).unsqueeze(dim=0)))
                else:
                    cropped_hand[0] = np.array(handcrop_transform(torch.from_numpy(cropped_hand[0].astype('float16')).unsqueeze(dim=0)))
        ...
        return heat_map,cropped_hand, copy_face
