...
def te_fused_attn_bwd_qkvpacked(qkv,cu_seqlens,o,do,softmax_aux,_dqkv,_dbias,rng_state,b,h,d,total_seqs,max_seqlen,attn_scale,p_dropout,qkv_layout,bias_type,attn_mask_type,qkv_type):
    # The output variable's dtype use default value 'float32',
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_mode():
        outs = core.eager._run_custom_op("te_fused_attn_bwd_qkvpacked", qkv,cu_seqlens,o,do,softmax_aux,_dqkv,_dbias,rng_state,b,h,d,total_seqs,max_seqlen,attn_scale,p_dropout,qkv_layout,bias_type,attn_mask_type,qkv_type)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        helper.append_op(type="te_fused_attn_bwd_qkvpacked", inputs=ins, outputs=outs, attrs={'b' : b,'h' : h,'d' : d,'total_seqs' : total_seqs,'max_seqlen' : max_seqlen,'attn_scale' : attn_scale,'p_dropout' : p_dropout,'qkv_layout' : qkv_layout,'bias_type' : bias_type,'attn_mask_type' : attn_mask_type,'qkv_type' : qkv_type})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def te_fused_attn_bwd(q,k,v,cu_seqlens_q,cu_seqlens_kv,o,do,softmax_aux,_dq,_dk,_dv,_dbias,rng_state,b,h,d,max_seqlen_q,max_seqlen_kv,attn_scale,p_dropout,qkv_layout,bias_type,attn_mask_type,qkv_type):
    # The output variable's dtype use default value 'float32',
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_mode():
        outs = core.eager._run_custom_op("te_fused_attn_bwd", q,k,v,cu_seqlens_q,cu_seqlens_kv,o,do,softmax_aux,_dq,_dk,_dv,_dbias,rng_state,b,h,d,max_seqlen_q,max_seqlen_kv,attn_scale,p_dropout,qkv_layout,bias_type,attn_mask_type,qkv_type)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        if _dbias is not None:
            outs['dBias@OPTIONAL'] = _dbias
        helper.append_op(type="te_fused_attn_bwd", inputs=ins, outputs=outs, attrs={'b' : b,'h' : h,'d' : d,'max_seqlen_q' : max_seqlen_q,'max_seqlen_kv' : max_seqlen_kv,'attn_scale' : attn_scale,'p_dropout' : p_dropout,'qkv_layout' : qkv_layout,'bias_type' : bias_type,'attn_mask_type' : attn_mask_type,'qkv_type' : qkv_type})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def te_fused_attn_bwd_kvpacked(q,kv,cu_seqlens_q,cu_seqlens_kv,o,do,softmax_aux,_dq,_dkv,_dbias,rng_state,b,h,d,total_seqs_q,total_seqs_kv,max_seqlen_q,max_seqlen_kv,attn_scale,p_dropout,qkv_layout,bias_type,attn_mask_type,qkv_type):
    # The output variable's dtype use default value 'float32',
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_mode():
        outs = core.eager._run_custom_op("te_fused_attn_bwd_kvpacked", q,kv,cu_seqlens_q,cu_seqlens_kv,o,do,softmax_aux,_dq,_dkv,_dbias,rng_state,b,h,d,total_seqs_q,total_seqs_kv,max_seqlen_q,max_seqlen_kv,attn_scale,p_dropout,qkv_layout,bias_type,attn_mask_type,qkv_type)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
