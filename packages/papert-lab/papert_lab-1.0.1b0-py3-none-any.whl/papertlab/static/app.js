...
const App = () => {
    ...
    const renderMessageContent = (content, role) => {
        return (
            <span className={role === 'assistant' ? 'text-yellow-400' : ''}>
                {content}
            </span>
        );
    };
    ...
    React.useEffect(() => {
        const checkApiKeyAndInitialize = async () => {
            try {
                ...
                const data = await response.json();
                if (data.openai_key_present || data.anthropic_key_present) {
        ...
        const initializeApp = async () => {
            try {
                ...
                setMessages([{ role: 'system', content: `An error occurred while initializing: ${error.message}. Please try again.` }]);
            }
        };
    ...
    const getFullPath = (structure, path = '') => {
        const result = {};
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.files) {
                    ...
                    console.warn('No files data received when changing model');
                }
                setMessages(prevMessages => [
                    ...prevMessages,
                    ...
                    <div className="flex-grow overflow-y-auto mb-2 sm:mb-4 border border-green-700 p-2">
                        {messages.map((message, index) => (
                            <div key={index} className="mb-2 whitespace-pre-wrap">
                                <span className={`font-bold ${message.role === 'user' ? 'text-green-400' : message.role === 'assistant' ? 'text-blue-400' : 'text-yellow-400'}`}>
                        ...
                        {logs.map((log, index) => (
                            <div key={`log-${index}`} className="mb-1 text-xs sm:text-sm text-gray-400 whitespace-pre-wrap">
                                {renderMessageContent(log, 'system')}
                            </div>
                        ))}
