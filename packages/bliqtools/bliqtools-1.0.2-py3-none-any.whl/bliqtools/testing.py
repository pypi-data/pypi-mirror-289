...
class MemoryMonitor:
    ...
    def __init__(self, description=None, delay=0.1, no_output = False):
        ...
        if description is None:
            description = Debuggable._line_descript(frame_up=1, format_string="Memory monitor @line {2} in {1}")
    ...
    def monitor(self):
        ...
        while True:
            ...
            with self._lock:
                self.stats.append({"time":time_now, "available":memory_available_in_gb,"process":memory_used_by_process_in_gb})
                if self._must_exit:
                    break
    ...
    def report_stats(self):
        ...
        for i, memstats in enumerate(self.stats):
            time = memstats['time']
            available = memstats['available']
            process = memstats['process']
            print(f"{time:8.2f}\t{available:13.2f}\t{process:12.2f}")
        print(f"-- end stats")
...
class Progress:
    ...
    """
    def __init__(self, total, description=None, show_every=100, delay_before_showing=3, message=None, no_output=False):
        """
...
class Debuggable:
    ...
    def _save_state(self):
        ...
        """
        self.saved_state = self._get_state()
    ...
    def _get_differential_state(self):
        """
    ...
    def _dump_internals(self):
        """
        Dump to screem a debugger-like output with the value of variables to inspect.
        If variables are large, provide a shorter version with meaningful info.
        """
        line = self._line_descript()
        print(f"\n-- begin {line}\n")

        if self.saved_state:
            state = self._get_differential_state()
        else:
    ...
    @classmethod
    def _line_descript(cls, frame_up=2, format_string=None, short=True):
        """
...
class TestClasses(unittest.TestCase):
    def short_test_id(self):
        return self.id().split('.')[-1] # Remove complete class name before the function name

    def setUp(self):
        print(f"\n# Current : {self.short_test_id()}")
    ...
    def test_progress(self):
        N = 100
        with Progress(total=N, show_every=25, delay_before_showing=0, description="test_progress") as progress:
            for i in range(N):
                progress.next()

    def test_memory_monitor_with_print(self):
        N = 4
        with MemoryMonitor():
            arrays = []
            with Progress(N, description=f"Allocating memory in {self.short_test_id()}", show_every=1, delay_before_showing=0) as progress:
                for i in range(N):
                    a = [1]*1_000_000_000
                    arrays.append(a)
                    progress.next()
    ...
    def test_memory_monitor_without_print(self):
        with MemoryMonitor(description="Memory allocation loop [4Gb]", no_output=True) as monitor:
            N = 100
            arrays = []
            with Progress(N, description=f"Allocating memory in {self.short_test_id()}", show_every=10) as progress:
                for i in range(N):
                    a = [1]*10_000_000
                    arrays.append(a)
                    progress.next()
            ...
            max_process = max(process_mem)
            print(f"The minimum available was {min_available:.2f} Gb, the process used maximum {max_process:.2f} Gb")
    ...
    def test_debuggable(self):
        class MyTest(Debuggable):
            def __init__(self):
                ...
                self.b = "Test"
                self.c = [1]*200
                self.d = np.zeros(shape=(5,5,5))

        test = MyTest()
        test._dump_internals()

        test._save_state()
        test.a = 2
        test._dump_internals() # Will only show the modified variable a, not others
