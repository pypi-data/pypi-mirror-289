    ...
    BaseTrain.name():
        [
            Image(kind=Accelerator.NVIDIA,
                  name="iregistry.baidu-int.com/windmill-public/train/paddlepaddle:v1.3.4.1-hotfix-20240805")
        ],
    BaseEval.name():
        [
            Image(kind=Accelerator.NVIDIA,
...
def pipeline(accelerator: str = "T4",
    ...
    train.properties = Properties(images=NAME_TO_IMAGES[train.name()], modelFormats=["PaddlePaddle"])
    ...
    eval.properties = Properties(images=NAME_TO_IMAGES[eval.name()], modelFormats=["PaddlePaddle"])
