...
def pipeline(extra_fs_name: str = "vistudio",
    ...
    parse_record_env = {
        ...
        "RECORD_UTILS_PATH": "/home/workspace/record_utils",
    }
    parse_record_params.update(base_params)
    parse_record_env.update(base_env)
    parse_record = ContainerStep(name="parse-record",
                                docker_env="iregistry.baidu-int.com/acg_aiqp_algo/cv-dev/record:v0.1.5",
                                parameters=parse_record_params,
                                env=parse_record_env,
                                extra_fs=[ExtraFS(name=extra_fs_name, mount_path=extra_fs_mount_path)],
                                outputs={"output_uri": Artifact()},
                                command=f'cd $RECORD_UTILS_PATH && export PF_OUTPUT_ARTIFACT_OUTPUT_URI={{{{output_uri}}}} &&. ./run.sh '
                            )


    datamining_params = {"input_dataset_name": "`parse_record`",
    ...
    datamining_env = {"INPUT_DATASET_NAME": "`parse_record`",
                      "OUTPUT_DATASET_NAME": "{{output_dataset_name}}",
                      "LABEL_CONFIDENCE_THRESHOLD": "{{label_confidence_threshold}}",
                      "MODEL_STORE_NAME": "{{model_store_name}}",
                      "MODEL_NAME": "{{model_name}}",
    ...
    data_mining = ContainerStep(name="datamining",
                                docker_env="iregistry.baidu-int.com/windmill-public/mining/trt86:0.8.8",
                                ...
                                command=f'cd /root && '
                                        ...
                                        f':$LD_LIBRARY_PATH && '
                                        f'export PYTHONPATH=$PYTHONPATH:/opt/conda/envs/data/opt/conda/envs/data/lib/python3.11'
                                        f':/opt/conda/envs/data/opt/conda/envs/data/lib/python3.11/lib-dynload'
                                        f':/opt/conda/envs/data/opt/conda/envs/data/lib/python3.11/site-packages &&'
