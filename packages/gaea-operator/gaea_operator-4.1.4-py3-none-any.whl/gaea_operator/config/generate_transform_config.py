...
KEY_CHANNEL_ORDER = 'channel_order'
KEY_RESIZE_METHOD = 'resize_method'
...
def create_common_config(advanced_parameters: dict,
    ...
    source_framework = advanced_parameters.get(KEY_SOURCE_FRAMEWORK, '').lower()
    
    target_framework = advanced_parameters.get(KEY_TARGET_FRAMEWORK, '').lower()
    ...
    # check args
    if source_framework is not None and len(source_framework) > 0 and source_framework not in FRAMEWORK_LIST:
    ...
    elif target_framework == "paddle" and source_framework == "paddle":
        PaddleToPaddleParam = {}
        PaddleToPaddleParam["type"] = "PaddleToPaddle"
        PaddleToPaddleParam["model_name"] = model_name
        PaddleToPaddleParam["input_shape"] = input_shape
        PaddleToPaddleParam[KEY_PRECISION] = advanced_parameters.get(KEY_PRECISION, 'int8')
        PaddleToPaddleParam[KEY_CHANNEL_ORDER] = advanced_parameters.get(KEY_CHANNEL_ORDER, 'rgb')
        PaddleToPaddleParam[KEY_RESIZE_METHOD] = advanced_parameters.get(KEY_RESIZE_METHOD, 'bilinear')
        PaddleToPaddleParam["batch_size"] = max_batch_size
        if mean is not None and std is not None:
            PaddleToPaddleParam["mean"] = mean
            PaddleToPaddleParam["std"] = std
        PaddleToPaddleParam["transpose"] = transpose
        PaddleToPaddleParam["norm"] = norm
        pipeline_cfg.append(PaddleToPaddleParam)
    ...
    if 'mean' in alg_param and 'std' in alg_param:
        ...
        for n in model_input_name:
            if model_name.startswith('change-') and "ocrnet" in model_name:
                mean_dict[n] = alg_param['mean'] + alg_param['mean']
                std_dict[n] = alg_param['std'] + alg_param['std']
            else:
