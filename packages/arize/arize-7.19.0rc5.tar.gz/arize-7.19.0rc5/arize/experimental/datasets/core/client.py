...
class ArizeDatasetsClient:
    ...
    def run_experiment(
        ...
        dataset = self.get_dataset(space_id=space_id, dataset_id=dataset_id, dataset_name=dataset_name)
        ...
        try:
            ...
            writer, metadata_reader = flight_client.do_put(descriptor, tbl.schema, self.session.call_options)
            with writer:
    ...
    def get_dataset(
        ...
        if not only_one_of_id_or_name_specified:
            raise ValueError(f"one and only one of dataset_id={dataset_id} or dataset_name={dataset_name} is required")
    ...
    def get_dataset_versions(self, space_id: str, dataset_id: str) -> Optional[pd.DataFrame]:
        ...
        request = request_pb.DoActionRequest(
            get_dataset_versions=request_pb.GetDatasetVersionsRequest(space_id=space_id, dataset_id=dataset_id)
        )
    ...
    def list_datasets(self, space_id: str) -> Optional[pd.DataFrame]:
        ...
        """
        request = request_pb.DoActionRequest(list_datasets=request_pb.ListDatasetsRequest(space_id=space_id))
