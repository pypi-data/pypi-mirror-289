...
class Europe:
    ...
    def list_index_constituents(symbol: str, **kwargs) -> List[str]:
        ...
        # EU Indices require a different endpoint and do not have IV data.
        ...
        for symbol in symbols:
            ...
            url = (
                f"{base_url}_{symbol}.json"
                if symbol in INDEXES.index or symbol in TICKER_EXCEPTIONS
                else f"{base_url}{symbol}.json"
            )
            if symbol in EU_INDEXES.index:
                ...
                url = f"https://cdn.cboe.com/api/global/european_indices/index_quotes/{_symbol}.json"
            urls.append(url)

        responses = await amake_requests(urls)
        quotes_data = [d["data"] for d in responses]
        [d.pop("seqno") for d in quotes_data]
        [d.pop("exchange_id") for d in quotes_data]
        quotes = DataFrame(quotes_data).set_index("symbol")
        ...
        for symbol in symbols:
            iv_url = (
                base_url + f"_{symbol}.json"
                if symbol in TICKER_EXCEPTIONS or symbol in INDEXES.index
                else base_url + f"{symbol}.json"
            )
            # There is no IV data for the EU Indices, so we'll skip those symbols.
            if symbol not in EU_INDEXES.index:
                iv_urls.append(iv_url)

            # While iterating through the symbols, grab the names belonging to each ticker.
            if symbol in SYMBOLS.index:
                quotes.loc[symbol, "name"] = SYMBOLS.at[symbol, "name"]
            if symbol in INDEXES.index:
                quotes.loc[symbol, "name"] = INDEXES.at[symbol, "name"]

        iv_responses = await amake_requests(iv_urls)
        iv_data = [d["data"] for d in iv_responses]
        iv = DataFrame(iv_data).set_index("symbol")

        # Merge the IV data with the quotes data.
        concat([quotes, iv], axis=1)

        # First get the index and company directories so we know how to handle the ticker symbols.
        # Using CachedSession to cache the index and company directories for faster response times.
        async with CachedSession(cache=backend) as cached_session:
            try:
                if query.use_cache is True:
                    indexes = await cached_session.get(
                        "https://cdn.cboe.com/api/global/us_indices/definitions/all_indices.json",
                        timeout=10,
                    )
                    companies = await cached_session.get(
                        "https://www.cboe.com/us/options/symboldir/equity_index_options/?download=csv",
                        timeout=10,
                    )
                    indices = await indexes.json()
                    INDEXES = DataFrame(indices)
                    directory = await companies.read()
                    SYMBOLS = read_csv(BytesIO(directory))
                    SYMBOLS = SYMBOLS.rename(
                        columns={" Stock Symbol": "symbol", "Company Name": "name"}
                    ).set_index("symbol")
            finally:
                await cached_session.close()
        # If use_cache is False, get the index and company directories directly.
        if query.use_cache is False:
            INDEXES, SYMBOLS = await asyncio.gather(
                get_index_directory(use_cache=False, **kwargs),
                get_company_directory(use_cache=False, **kwargs),
            )
