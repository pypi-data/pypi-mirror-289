...
class GpInOut:
    def __init__(self, image: Image.Image, offset_x: int = 0, offset_y: int = 0, data: Any = None):
        self.image = image
...
def loadfrom_json(json_str, c_img, gp_name, gp_dir):
    ...
    str_body.append("def gp(image=capture()):")
    ...
    str_body.append(f"  inout = GpInOut(image)")
