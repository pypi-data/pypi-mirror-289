        ...
        for turn in section.find_all("turn"):
            ...
            for child in turn.children:
                ...
                lines = [
                    ...
                    if len(l) and not any(l.startswith(b) for b in EXCLUDE_BEGINNINGS)
                ]
                if not lines:
                    continue
                times = []
...
def try_parse(sgml_path: Path):
    ...
    try:
        ...
        raise ImportError(
            'Before running BroadcastNews data preparation, you should "pip install beautifulsoup4"'
        )
    try:
        ...
        encoding = (
            subprocess.check_output(f"file -bi {sgml_path}", shell=True, text=True)
            .split(";")[-1]
            .replace("charset=", "")
            .strip()
        )
        with NamedTemporaryFile() as f:
            subprocess.run(
                f"iconv -f {encoding} -t utf-8 -o {f.name} {sgml_path}",
                shell=True,
                check=True,
                text=True,
            )
...
def group_lines_in_time_marker(sgml_lines):
    ...
    return group(
        [
            ...
            for is_marker, lines in groupby(
                sgml_lines, key=lambda l: l.startswith("<time")
            )
        ],
        2,
    )
