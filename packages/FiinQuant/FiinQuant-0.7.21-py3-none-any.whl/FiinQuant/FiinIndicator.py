...
class FiinIndicator:
    class trend:
        ...
        class MACDIndicator:
            def __init__(self, column: pd.Series, window_slow: int = 26, window_fast: int = 12, window_signal: int = 9):
                self.column = column
            ...
            def _calculate_ema_slow(self):
                ema_slow = FiinIndicator.trend.EMAIndicator(self.column, self.window_slow).ema_indicator()
                return ema_slow

            def _calculate_ema_fast(self):
                ema_fast = FiinIndicator.trend.EMAIndicator(self.column, self.window_fast).ema_indicator()
                return ema_fast
            ...
            def _calculate_macd_signal(self):
                macd_signal = FiinIndicator.trend.EMAIndicator(self.macd_line, self.window_signal).ema_indicator()
                return macd_signal
        ...
        class ADXIndicator:
            ...
            def _calculate_adx(self):
                ...
                adx = FiinIndicator.trend.SMAIndicator(dx, self.window).sma_indicator()       
                return adx
    ...
    class momentum:
        ...
        class StochasticOscillator:
            ...
            def _calculate_stoch_signal(self):
                ...
                stoch_d = FiinIndicator.trend.SMAIndicator(stoch_k, self.smooth_window).sma()
                return stoch_d
    ...
    class volatility:
        class BollingerBands:
            ...
            def _calculate_bollinger_mavg(self):
                bollinger_mavg = FiinIndicator.trend.SMAIndicator(self.column, self.window).sma()
        ...
        class Supertrend:
            ...
            def _calculate_bands(self):
                atr = FiinIndicator.volatility.AverageTrueRange(self.high, self.low, self.close, self.window)
        ...
        class AverageTrueRange:
            ...
            def _calculate_atr(self):
                atr = FiinIndicator.trend.SMAIndicator(column=self.tr, window=self.window).sma()
