...
from xurpas_data_quality.data import get_series_descriptions, get_correlations, XTypeSet, Numeric
...
def describe(df: pd.DataFrame, data_types: dict=None)-> TableDescription:
    ...
    if isinstance(df, list):
        descriptions = []
        shared_columns = [dataframe['volume'] for dataframe in df]
        ...
        for dataframe in df:
            ...
            comparison_data = get_compare(dataframe)
            data.shared_values = shared_values
...
def describe_invalid(df: pd.DataFrame) -> TableDescription:
    """
...
def check_data(df: pd.DataFrame) -> list:
    try:
        df_empty = df[df['shipment_no'].isnull()]
