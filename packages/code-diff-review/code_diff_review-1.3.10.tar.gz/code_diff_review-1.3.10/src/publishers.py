...
class GitIssuePublisher(Publisher):
    ...
    def publish(self, data):
        ...
        try:
            ...
            updated_body = self.append_data_to_comment(existing_issue.body, data['message']['adherence'])
            existing_issue.edit(updated_body)
    ...
    def append_data_to_comment(self, existing_body: str, adherence: Dict[str, Any]) -> str:
        """
        Appends adherence data to the existing comment body.
        ...
        """
        adherence_line = f"| {self.sha} | {adherence['score']} {adherence['emoji']} |"
        if "| Commit | Score |\n|--------|-------|" in existing_body:
