...
class PhoenixVision:
    ...
    def start(self):
        ...
        while True:
            ...
            # Conversão para RGB antes da conversão para escala de cinza
            ...
            for (x, y, w, h) in faces:
                # Extrair a região de interesse em cores
                roi_color = rgb_frame[y:y+h, x:x+w]
                roi_gray = cv2.resize(roi_color, (200, 200))
                roi_gray = cv2.cvtColor(roi_gray, cv2.COLOR_RGB2GRAY)  # Converter para escala de cinza
