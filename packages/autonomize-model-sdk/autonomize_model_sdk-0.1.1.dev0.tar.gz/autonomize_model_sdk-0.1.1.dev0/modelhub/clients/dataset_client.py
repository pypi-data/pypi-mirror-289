...
class DatasetClient(BaseClient):
    ...
    def load_dataset(self, dataset_name: str, version: Optional[int] = None, split: Optional[str] = None, directory: Optional[str] = None) -> DatasetDict:
        """
        ...
        if nested_directories and not directory:
            raise ValueError(f"Multiple nested directories found: {nested_directories}. Please specify a directory path.")

        if directory and any(directory.endswith(split_name) for split_name in nested_directories):
            raise ValueError(f"The specified directory path '{directory}' includes a split name. Please specify the parent directory.")

        # Collect file URLs and determine splits
        split_file_urls = self._collect_split_file_urls(dataset["id"], version_data["files"], split, directory)

        # Load dataset using the `datasets` library
        return self._load_dataset_by_format(split_file_urls)


    def _load_dataset_by_format(self, split_file_urls: Dict[str, List[str]]) -> DatasetDict:
        """
        Load a dataset from file URLs, determining the format and handling nested paths.

        Args:
            split_file_urls (Dict[str, List[str]]): A dictionary where keys are split names and values are lists of file URLs.

        Returns:
            DatasetDict: The loaded dataset.
        """
        data_files = split_file_urls
        file_extensions = [self.get_file_extension(file) for files in split_file_urls.values() for file in files]
    ...
    def _collect_split_file_urls(self, dataset_id: str, files: List[Dict[str, Any]], split: Optional[str], directory: Optional[str]) -> Dict[str, List[str]]:
        """
        ...
        for file in files:
            ...
            split_file_urls[split_name].append(self.get_signed_url(dataset_id, file["file_path"]))
    ...
    def _check_nested_directories(self, files: List[Dict[str, Any]]) -> List[str]:
        ...
        for file in files:
            split_name = file["file_path"].split('/')[-2]
