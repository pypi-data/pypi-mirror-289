...
from .models import Config, InstrumentReq, InstrumentRes, RateRes, RateReq, CreateOrderReq, \
    CreateOrderRes
...
class QuickexSDK:
    ...
    def get_rate(self, body: RateReq) -> RateRes:
        ...
        if r.status_code in [200, 201]:
            return RateRes.model_validate_json(r.text, strict=False)
        else:
            response = r.json()
            raise Exception(response['message'] + str(r.status_code))

    def create_order(self, body: CreateOrderReq) -> CreateOrderRes:
        r = self.client.post('/api/v2/orders/public/create', body.model_dump_json())
        if r.status_code in [200, 201]:
            return CreateOrderRes.model_validate_json(r.text, strict=False)
        else:
            response = r.json()
            raise Exception(response.message + str(r.status_code))
