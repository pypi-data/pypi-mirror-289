from src.quickex_sdk.models import Config, InstrumentReq, RateReq, CreateOrderReq, ClaimedPublicRate
...
def test_sdk_create_order_class():
    config = Config(api_url=api_url_base, api_public=api_public_base, api_secret=api_secret_base)
    sdk = QuickexSDK(config)
    res = RateReq(
        instrumentFromCurrencyTitle="BTC",
    ...
    assert new_order.pair.instrumentFrom == req.instrumentFrom
    assert new_order.pair.instrumentTo == req.instrumentTo
    assert new_order.destinationAddress == req.destinationAddress
