...
static PyObject *C_get_log_perms(PyObject *self, PyObject *args) {
    ...
    if(PyArray_IS_F_CONTIGUOUS(Xo)){
		...
		dims[1] = n;
  		...
  		if(debug){
  			fprintf(stdout,"C style memory layout detected. Copying X anyways.\n");
  		}
  		xcopied = 1;
  		npy_intp dims[2];
		dims[0] = S;
  	...
  	if(!constant_ts){
  		if(debug){
  			fprintf(stdout,"The ts are not constant. Copying this matrix as well.\n");
  		}
  		if(PyArray_IS_F_CONTIGUOUS(to)){
			...
			dims[1] = n;
	  		...
	  		if(debug){
	  			fprintf(stdout,"C style memory layout detected. Copying anyway.\n");
	  		}
	  		tcopied = 1;
	  		npy_intp dims[2];
			dims[0] = S;
  		...
  		if(debug){
  			fprintf(stdout,"t is a vector. Copying t.\n");
  		}
  		tcopied = 1;
  		npy_intp dims[1];
		dims[0] = n;
  		to_new = (PyArrayObject*)PyArray_SimpleNew(1, dims, NPY_DOUBLE);
