    ...
    async def send(self, data: TLObject, wait_response: bool = True, timeout: float = WAIT_TIMEOUT):
        ...
        if wait_response:
            ...
            if isinstance(result, raw.types.RpcError):
                ...
                RPCError.raise_it(self.client, result, type(data))
    ...
    async def invoke(
        ...
        while True:
            try:
                ...
                log.warning('[%s] - [%s] Waiting for %s seconds before continuing (required by "%s")',
                            self.client.me.id, self.client.name, amount, query_name)
                ...
                (log.warning if retries < 2 else log.info)(
                    ...
                    self.client.me.id, Session.MAX_RETRIES - retries + 1,
