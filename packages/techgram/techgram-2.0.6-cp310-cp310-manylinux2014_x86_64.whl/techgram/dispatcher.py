...
class Dispatcher:
    ...
    def __init__(self, client: "techgram.Client"):
        ...
        async def message_parser(update, users, chats):
            return (
                await techgram.types.Message._parse(
                    self.client,
                    update.message,
                    users,
                    chats,
        ...
        async def chosen_inline_result_parser(update, users, chats):
            return (
                techgram.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                techgram.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                techgram.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ChatJoinRequestHandler
            )

        async def story_parser(update, users, chats):
            return (
                await techgram.types.Story._parse(self.client, update.story, users, chats, update.peer),
                StoryHandler
            )

        async def pre_checkout_query_parser(update, users, chats):
            return (
                await techgram.types.PreCheckoutQuery._parse(self.client, update, users),
    ...
    async def handler_worker(self, lock):
        while True:
            ...
            try:
                ...
                pass
            except techgram.errors.FloodWait:
                pass
            except Exception as e:
                log.exception(e)
