...
class Game:
    ...
    def _get_player_node(self, player_index: int = -1) -> int:
        """Return the node that the player owns, or -1 if none."""
        if player_index == -1:
            player_index = self.current_player_index

        state_id = Vertex.State.P1 if player_index == 0 else Vertex.State.P2
    ...
    def is_my_turn(self, player_id: str) -> bool:
        ...
        return player_id in [self.player1_id, self.player2_id] and player_id == [self.player1_id, self.player2_id][self.current_player_index]
    ...
    def is_game_over(self) -> bool:
        ...
        if any(edge.state == Edge.State.NONE for edge in self.edges):
            return False
        return self._get_player_node(0) != -1 and self._get_player_node(1) != -1
    ...
    def set_game_state(self, state: Dict[str, any], validate: bool = True) -> None:
        ...
        if validate:
            ...
            # Set the game state without validation
    ...
    def get_legal_moves(self, player_id: str) -> List[List[int, int, int]]:
        ...
        if (player_id != [self.player1_id, self.player2_id][self.current_player_index]):
    ...
    def make_move(self, player_id:str,  move_type: int, move_index: int, move_state: int):
        ...
        if move_type == Game.MoveType.NODE.value:
            ...
            if move_state not in [0, 1]:
                raise InvalidMoveError("Selected node state not valid.")
            
            node.state = Vertex.State.P1 if move_state == 0 else Vertex.State.P2
