...
class Danila_vagon_text_detect_base:
    def __init__(self, yolov5_dir, vagon_text_detect_params):
        print('reading and loading - VAGON_NUMBER_DETECT_MODEL')
        if vagon_text_detect_params.vagon_text_detect_version == 1:
            self.vagon_number_detect_model = Vagon_number_detect_class(VAGON_NUMBER_DETECT_MODEL_ADDRESS, yolov5_dir)
        elif vagon_text_detect_params.vagon_text_detect_version == 2:
            self.vagon_number_detect_model = Vagon_number_detect_class(VAGON_NUMBER_DETECT_MODEL_ADDRESS_2, yolov5_dir)
