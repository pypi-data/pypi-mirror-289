...
class Client:
    def __init__(self, api_key: str | None = None, bearer_token: str | None = None):
        ...
        self._client = httpx.AsyncClient(base_url="https://api.unthread.io/api/")
    ...
    async def list_conversations(
        ...
        while True:
            response = await self._client.post(
                "/conversations/list",
                headers=self._headers,
                json={k: v for k, v in {"cursor": cursor, "where": where}.items() if v},
            )
            response.raise_for_status()
            data = response.json()
        ...
        *,
        ...
        response = await self._client.patch(
            ...
            json={
                ...
                "ticketTypeId": (
                    str(_as_uuid(ticket_type_id)) if ticket_type_id else None
                ),
    ...
    async def assign_ticket_type(
        ...
        *,
        ...
        if langsmith_thread_id or langsmith_thread_url:
            if not ticket_type:
    ...
    async def get_user_by_email(self, email: str) -> User:
        where_field = WhereField(field="email", operator="==", value=email)
        async for user in self.list_users(where=[where_field]):
            return user
    ...
    async def list_tags(
        ...
        while True:
            response = await self._client.post(
                "/tags/list",
                headers=self._headers,
                json={"cursor": cursor, "where": where},
            )
            response.raise_for_status()
            data = response.json()
    ...
    async def create_ticket_type(
        ...
        #  POST https://langchain.unthread.io/api/ticket-types {name: name}
        if if_exists == "skip":
    ...
    async def list_ticket_types(
        ...
        while True:
            response = await self._client.post(
                "/ticket-types/list",
                headers=self._headers,
                json={"cursor": cursor, "where": where_fields},
            )
            response.raise_for_status()
            data = response.json()
