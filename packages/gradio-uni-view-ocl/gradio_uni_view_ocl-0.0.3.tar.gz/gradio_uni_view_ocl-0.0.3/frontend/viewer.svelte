...
<script lang="ts">
    ...
    onMount(() => {
        ...
        editor.setAtomHightlightCallback((atom: number, selected: boolean) => {
            highlightAtom = selected ? atom : undefined;
        });
        editor.setBondHightlightCallback((bond: number, selected: boolean) => {
            highlightBond = selected ? bond : undefined;
        });
...
<div>
    ...
    >
        <div
            ...
            on:click={() => {
                const { model, drawPane, } = editor;
                if (highlightAtom !== undefined) {
                    const selected = !model.mMol.isSelectedAtom_0(highlightAtom);
                    model.mMol.setAtomSelection_0(highlightAtom, selected);
                    drawPane.draw();
                } else if (highlightBond !== undefined) {
                    const atomA = model.mMol.mBondAtom[0][highlightBond];
                    const atomB = model.mMol.mBondAtom[1][highlightBond];
                    const selected = !model.mMol.isSelectedBond_0(highlightBond);
