...
from embedding_server_client.schema.document import (
    ...
    DocumentRetrievalResponse, DocumentSplitsRetrievalRequest, DocumentSplitsRetrievalResponse,
    DocumentSummariesRetrievalRequest, DocumentSummariesRetrievalResponse,
)
...
class AsyncEmbeddingClient:
    def __init__(self, host: str, timeout: int = 360000) -> None:
        """
        Initializes a new instance of the AsyncEmbeddingClient.
        ...
        self.context: Optional[zmq_asyncio.Context] = None
        self.socket: Optional[zmq_asyncio.Socket] = None
    ...
    async def _send_request(self, zmq_message_type: ZmqMessageType, request: Optional[T] = None) -> Optional[T]:
        """
        Asynchronously sends a request to the server, according to the specified message type, and waits for a response.
        ...
        try:
            resp_messages: List[bytes] = await self.socket.recv_multipart()
    ...
    async def send_document_insertion_request(self, request: DocumentInsertionRequest) -> Optional[DocumentInsertionResponse]:
        """
        ...
        return await self._send_request(ZmqMessageType.DOCUMENT_INSERTION, request)

    async def send_document_retrieval_request(self, request: DocumentRetrievalRequest) -> Optional[DocumentRetrievalResponse]:
        """
        ...
        return await self._send_request(ZmqMessageType.DOCUMENT_RETRIEVAL, request)

    async def send_document_splits_retrieval_request(
            self,
        ...
        return await self._send_request(ZmqMessageType.DOCUMENT_SPLITS_RETRIEVAL, request)

    async def send_document_summaries_retrieval_request(
            self,
        ...
        return await self._send_request(ZmqMessageType.DOCUMENT_SUMMARIES_RETRIEVAL, request)

    async def send_query_request(self, request: DocumentQueryRequest) -> Optional[DocumentQueryResponse]:
        """
        Asynchronously sends a query request to the server and waits for a response.
        ...
        return await self._send_request(ZmqMessageType.DOCUMENT_QUERY, request)

    async def send_split_retrieval_request(self, request: SplitRetrievalRequest) -> Optional[SplitRetrievalResponse]:
        """
        ...
        return await self._send_request(ZmqMessageType.SPLIT_RETRIEVAL, request)

    async def send_summary_retrieval_request(self, request: SummaryRetrievalRequest) -> Optional[SummaryRetrievalResponse]:
        """
        ...
        return await self._send_request(ZmqMessageType.SUMMARY_RETRIEVAL, request)
