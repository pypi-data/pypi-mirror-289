...
class XTSystemsSDK:
    ...
    def create_ticket(self, ticket_data: Dict[str, Any]) -> Dict[str, Any]:
        response = requests.post(
            f"{self.base_url}/v1/tickets", headers=self.headers, json=ticket_data
        )
        if self.verbose:
            parse_response(response)
        return response.json()
    ...
    def update_asset_external_data(
        self, asset_id: str, external_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Update the external data for an asset.
        """
        data = {"external_data": external_data}
        response = requests.put(
            f"{self.base_url}/v1/assets/{asset_id}/external-data",
            headers=self.headers,
            json=data,
        )
        if self.verbose:
