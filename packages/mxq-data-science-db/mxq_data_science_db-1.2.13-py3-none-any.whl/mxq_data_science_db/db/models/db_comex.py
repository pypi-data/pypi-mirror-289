...
class ComexReleaseCalendar(Base, BaseModel):
    ...
    next_updat_at = sa.Column(sa.DATETIME())
    updated_at = sa.Column(
        sa.DATETIME(), nullable=False, default=sa.text("CURRENT_TIMESTAMP")
    )
    ...
    @classmethod
    def delete_before_insert(cls, df: pd.DataFrame):
        """Delete all records for the period_ref in the DF

        Args:
            df (pd.DataFrame): dataframe
        """
        list_period_ref = list(set(df[cls.period_ref.name]))

        assert len(list_period_ref) != 0, "No year set"

        with SQLConnection() as con:
            logger.info(f"Delete before upsert, ano={list_period_ref}")
            stmt = delete(cls).where(cls.period_ref.in_(list_period_ref))
            con.execute(stmt)
