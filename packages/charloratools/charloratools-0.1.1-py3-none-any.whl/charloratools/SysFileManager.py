...
class ImgManager:
  ...
  def __str__(self):
    return f"Source:{self.path};Size:{self.dim[0]}x{self.dim[1]};Deleted:{self.deleted};Hash:{self.hash}"
...
def refresh_decorator(func):
  def wrapper(*args,**kwargs):
    ...
    self.images = utils.dirisvalid(self.path, return_info=True,hashtype=self.hashtype)
...
class GalleryManager:
  ...
  def __add__(self,other):
    if isinstance(other,self.__class__):
      ...
      utils.dirisvalid(new_dir_name, create_if_not_found=True)
