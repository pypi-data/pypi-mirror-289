...
class MLPRModeller(Modeller):
    ...
    def _pairwise_predict_old(self, train_dataset: Dataset, predict_dataset: Dataset, model: MLPRegressor):
        """
    ...
    def _pairwise_predict(self, train_dataset: Dataset, predict_dataset: Dataset, model: MLPRegressor):
    ...
    def predict(self, dataset: Dataset, dataset_train: Dataset = None, return_uncertainty=False):
        ...
        # Since MLPRegressor does not support individual estimator predictions,
        # We will not compute uncertainty in the traditional sense here.
        # This simplifies the predict method and removes the unsupported operation.
        if self.use_pairwise and isinstance(dataset_train, Dataset):
            preds, std_dev = self.PADRE_predict(self.model, dataset.X, dataset_train.X, dataset_train.y)
            ...
            if return_uncertainty:
                # Here we just return zeros for uncertainty as a placeholder since MLP doesn't support this natively.
