...
def process_epochs(mne_epochs, tmin=0, tmax=None, tslide=None, sort=False,
    ...
    labels : array of integer
        ...
        ``999`` is reserved for unlabelled epochs and will be unchanged.
...
def extract_features(nirs, feature_list):
    ...
    feature_list : list of strings
        ...
        the time axis, ``'kurt'`` for the kurtosis along the time axis,
        ``'ttp'`` for the time to peak (requires channels to have been sorted
        beforehand: all HbO, all HbR), ``'peak'`` for the value of the peak
        (max value for HbO and min value for HbR, requires channels to have
        been sorted beforehand: all HbO, all HbR).
