...
def process_data(data, index):
    ...
    try:
        ...
        d = str(data).strip()
    print(f"i...  {bg.blue}   receivd: {d}  on port {config.CONFIG['netport']}+{index}  {bg.default}")
    print()
...
def serve_port( PORT, TCP=True):
    ...
    while True:
        ...
        with conn:
            data = conn.recv(1024)
            if data:
                process_data(data, PORT - int(config.CONFIG['netport']))
...
def watch_named_fifo(PORT, fifon = '/tmp/flashcam_fifo'):
    ...
    with open(fifoname, 'r') as fifo_file:
        while True:
            ...
            if data:
                config.load_config()
                print(f'i... named pipe Received: {data}')
                process_data(data, PORT - int(config.CONFIG['netport']))
