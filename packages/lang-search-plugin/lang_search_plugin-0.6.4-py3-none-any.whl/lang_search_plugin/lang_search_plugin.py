...
class LangSearchPlugin(BasePlugin):
    ...
    def _get_selected_language(self, config, page=None):
        ...
        if page and 'language' in page.meta:
            log.debug(f"LSP Selected language from page meta: {page.meta['language']}")
            return page.meta['language']
        log.debug(f"LSP Selected language from config: {config.get('site_language', self.config['default_language'])}")
