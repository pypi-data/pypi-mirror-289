...
class LangSearchPlugin(BasePlugin):
    ...
    def on_pre_build(self, config):
        try:
            log.debug(f"LSP Plugins available: {config['plugins']}")
            language = self.selected_language
            log.info(f"LSP Adjusting search index for language: {language}")
            search_plugin = config['plugins'].get('material/search')
            if search_plugin:
                search_plugin.config['prebuild_index'] = False
            else:
                log.error("LSP Search plugin 'material/search' not found.")
        except Exception as e:
    ...
    def on_post_build(self, config):
        try:
            ...
            search_plugin = config['plugins'].get('material/search')  # Adjusted reference
            if search_plugin:
