...
class LangSearchPlugin(BasePlugin):
    ...
    def _get_language_from_path(self, path):
        ...
        if path.startswith('pt/'):
            log.debug("LSP Detected Portuguese language for path: {path}")
            return 'pt'
        log.debug("LSP Detected English language for path: {path}")
        return 'en'
    ...
    def _page_is_in_language(self, page, language):
        ...
        path_list = self.config['paths'].get(language, [])
        return page.file.src_path in path_list
