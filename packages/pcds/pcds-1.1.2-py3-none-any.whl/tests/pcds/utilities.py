...
Last Modified: Wednesday August 07 2024 1:10 PM
Version:1.6.8
...
Content: dataType(arg), str2type(strarg), str2data(strdata), data2str(data),
    ...
    int2str(strint=0), min(datalist=[]), max(datalist=[]), copy(data)
...
# It returns True if address is a valid .pcds file, otherwise False is returned 
...
# copy the basic data and data structures
# supported types: 'int','float','str','bool','NoneType','list','tuple','dict'
def copy(data):
    if dataType(data) in ['int','float','str','bool','NoneType']: return data
    elif dataType(data) in ['list','tuple']:
        tmplist=[]
        for dataitem in data:
            tmplist.append(dataitem)
        if dataType(data) == 'list': return tmplist
        elif dataType(data) == 'tuple': return tuple(tmplist)
    elif dataType(data) == 'dict':
        tmpdict={}
        ...
        raise ValueError("Error: Invalid data type of argument of copy() function.")
