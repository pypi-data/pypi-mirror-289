...
__version__: str = '2024.8.6'  # this must be here for pyproject.toml to find it.
...
class junkie:
    ...
    func = definition for abstract function
    ...
    def __init__(self, volume: np.ndarray|str, channel_strs: Optional[Tuple[str]] = ('', ),
                 cmap: str = 'gray', figsize: Tuple[int, int] = (4, 4), func: Optional[Callable] = None):
        ...
        self.abstract_function: function = func
    ...
    def display_slice(self, reset_slices: Optional[bool] = False):
        ...
        if reset_slices or self.view_plane != self._plane_selection.value:
            ...
            if self.abstract_function is not None:
                self.final_image_data = self.apply_function()
            else:
            ...
            plt.figure(self.fig)
            self.im_axes = plt.imshow(self.final_image_data[self.curslice, :, :], vmin=self._vmin_slider.value,
                                      vmax=self._vmax_slider.value, cmap=self.cmap)
        ...
        # Plot slice for the given plane and slice.
            ...
            self.im_axes.set(data=self.final_image_data[self.curslice, :, :],
                             clim=[self._vmin_slider.value, self._vmax_slider.value], cmap=self.cmap)

    def apply_function(self):
        args = dict([[this_widget.description, this_widget.value] for this_widget in self.abstract_widgets])
        return self.abstract_function(self.image_data.copy(), **args)
    ...
    def _add_abstract_widgets(self):
        parameters = getattr(self.abstract_function, '__annotations__', None)
        defaults = getattr(self.abstract_function, '__defaults__', None)
        if parameters is None or parameters is False:
            raise TypeError("ERROR: the provided function does not have type annotations.")
        elif defaults is None:
            raise ValueError("ERROR: the provided function does not have variable defaults.")
            
        self.abstract_widgets = []
        for p, this_parameter in enumerate(parameters.keys()):
            if parameters[this_parameter] is bool:
                self.abstract_widgets.append(ipyw.Checkbox(value=defaults[p],
                                                           description=this_parameter,
                                                           continuous_update=False))
            elif parameters[this_parameter] is int:
                self.abstract_widgets.append(ipyw.IntText(value=defaults[p],
                                                          description=this_parameter,
                                                          continuous_update=False))
            elif parameters[this_parameter] is float:
                self.abstract_widgets.append(ipyw.FloatText(value=defaults[p],
                                                            description=this_parameter,
                                                            continuous_update=False))

            self.abstract_widgets[p].observe(self._abstract_change, 'value')

        self.abstract_widgets_box = ipyw.Tab()
        self.abstract_widgets_box.children = self.abstract_widgets
