...
def compute_A21_hat(A21, Rk, A12, Q, regularization_factor=1e-10):
    product = Rk @ A12 @ Rk
    product = np.clip(product, -1e10, 1e10)  # 限制數值範圍
    A21_hat = (A21 + product) @ Q
    A21_hat = np.clip(A21_hat, -1e10, 1e10)  # 再次限制數值範圍
    A21_hat += regularization_factor * np.eye(A21_hat.shape[0])  # 引入正則化項
    return A21_hat
...
def image_compression_algorithm(A, P, epsilon, max_iterations=1000):
    ...
    while k < max_iterations:
        ...
        Rk_next = X @ Q.T
        Rk_next = np.clip(Rk_next, -1e10, 1e10)  # 限制Rk的值範圍
        rk_next = np.linalg.norm(Rk_next)
        
        if rk_next < epsilon or np.isnan(rk_next) or np.isinf(rk_next):
            return Rk, A11_tilde
