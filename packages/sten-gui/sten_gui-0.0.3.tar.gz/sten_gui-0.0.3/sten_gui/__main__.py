...
from sten_gui.config import Json
from sten_gui.consts import *
from sten_gui.crypto import ALPHABET, Hill, ciphers
from sten_gui.data import Border, Color, FilePath, Hotkey, Url, VEvent
from sten_gui.error import CryptoErrorGroup
from sten_gui.icons import *
from sten_gui.utils import nona, splitext
from sten_gui.version import VERSION
...
def encode(event: tk.Event) -> None:
    ...
    for pix, (band, lsb) in product(pixels, Glob.bb):
        if i >= bits_length:
            break
...
def decode(event: tk.Event) -> None:
    ...
    for bb in possibilities if cnf['BruteLSB'].get() else (Glob.bb,):
        bits, message = '', ''

        for pix, (band, lsb) in product(pixels, bb):
    ...
    if nona(message, ALPHABET):
        mb.showerror(
            message='Message contains a non-alphabet character.',
            detail='ARE YOU SURE THIS MESSAGE WAS CREATED USING STEN GUI?',
        )
        return
...
def check_for_updates() -> None:
    ...
    try:
        with urllib.request.urlopen(Url.RSS, timeout=9) as feed:  # nosec
            latest = ElementTree.fromstring(feed.read()).findtext(
                'channel/item/title'
            )
    except urllib.error.URLError as err:
        mb.showerror(message=str(err))
    else:
        mb.showinfo(message=('Outdated.', 'All good!')[VERSION == latest])
...
def refresh(event: tk.Event) -> None:
    ...
    if widget is not X_ciphers:
        ...
        E_key['vcmd'] = nv[cname]  # Update validate command
    ...
    limit = ((Picture.pixel * sum(bb.values())) // B) - len(DELIMITER)
...
root.wm_iconphoto(True, tk.PhotoImage(data=IMAGE_DATA_STEN_GUI))

root.wm_title('Sten GUI')
...
M_help.add_command(
    command=lambda: mb.showinfo(message=VERSION),
    compound=tk.LEFT,
    image=IMAGE_ABOUT,
    label='About',
    state=tk.NORMAL,
