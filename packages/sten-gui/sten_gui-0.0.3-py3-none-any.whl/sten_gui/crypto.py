...
from sten_gui.data import Action
...
class Caesar(Cipher):
    ...
    def _do(self, job: T.JOB) -> str:
        ...
        for char in self.txt:
            ...
            txt += ALPHABET[jobs[job](i_txt, i_key) % len(ALPHABET)]
...
class Hill(Cipher):
    ...
    def __init__(self, key: str, txt: str = '') -> None:
        ...
        if math.gcd(determinant, len(ALPHABET)) != 1:
        ...
        self._inv = pow(determinant, -1, len(ALPHABET))
    ...
    def _do(self, matrix: NDArray) -> str:
        ...
        return ''.join(
            ALPHABET[i] for i in (np.concatenate(transposed) % len(ALPHABET))
        )
