...
from wraps_core.reprs import empty_repr

from wraps_futures_core.reawaitable import ReAwaitable

if TYPE_CHECKING:
    from wraps_futures_core.typing import FutureCallable, FutureUnary
...
class Future(Awaitable[T]):
    ...
    def create(cls, awaitable: Awaitable[U]) -> Future[U]:
        """Creates a [`Future[U]`][wraps_futures_core.future.Future]
    ...
    def future_map(self, function: Unary[T, U]) -> Future[U]:
        """Maps a [`Future[T]`][wraps_futures_core.future.Future] to a [`Future[U]`][wraps_futures_core.future.Future]
    ...
    def future_map_await(self, function: AsyncUnary[T, U]) -> Future[U]:
        """Maps a [`Future[T]`][wraps_futures_core.future.Future]
        to a [`Future[U]`][wraps_futures_core.future.Future]
    ...
    def then(self, function: FutureUnary[T, U]) -> Future[U]:
        ...
        [`Future[U]`][wraps_futures_core.future.Future].
    ...
    def future_flatten(self: Future[Future[U]]) -> Future[U]:
        """Flattens a [`Future[Future[U]]`][wraps_futures_core.future.Future]
        to a [`Future[U]`][wraps_futures_core.future.Future].
    ...
    async def async_iter(self) -> AsyncIterator[T]:
        ...
        [`Future[T]`][wraps_futures_core.future.Future].
    ...
    def from_value(cls, value: U) -> Future[U]:
        """Wraps the `value` of type `U` into a [`Future[U]`][wraps_futures_core.future.Future].
...
"""An alias of [`Future.from_value`][wraps_futures_core.future.Future.from_value]."""
...
def wrap_future(function: AsyncCallable[P, T]) -> FutureCallable[P, T]:
    ...
    returning [`Future[T]`][wraps_futures_core.future.Future].
