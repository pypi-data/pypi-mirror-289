...
REGRESSION_REFERENCE: t.Optional[float] = 11.7 # 11.7
...
def target_from_metadata(e: Experiment,
    ...
    """
    graph = metadata['graph']['graph_labels']
    
    # Here we extract the SMILES representation of the molecule from the metadata and convert it into a RDKit molecule
    # so we can then count the number of carbon atoms as the new target value.
    smiles = metadata['repr']
    mol = Chem.MolFromSmiles(smiles)
    target = np.array([sum([int(atom.GetSymbol() == 'C') for atom in mol.GetAtoms()])])
