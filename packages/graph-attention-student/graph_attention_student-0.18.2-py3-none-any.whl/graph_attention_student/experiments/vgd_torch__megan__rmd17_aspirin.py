...
VISUAL_GRAPH_DATASET: str = 'rmd17_aspirin_all'
# :param DATASET_TYPE:
...
REGRESSION_REFERENCE: t.Optional[float] = -4.062747
...
def target_from_metadata(e: Experiment,
    ...
    target = metadata['graph']['graph_labels']
    metadata['graph']['graph_labels'] = target
    metadata['target'] = target
    
    # metadata['graph']['node_attributes'] = np.concatenate([metadata['graph']['node_attributes'], metadata['graph']['node_coordinates']], axis=1)
    # metadata['graph']['edge_attributes'] = np.concatenate([metadata['graph']['edge_attributes'], metadata['graph']['edge_lengths']], axis=1)
    metadata['graph']['node_attributes'] = metadata['graph']['node_coordinates']
    metadata['graph']['edge_attributes'] = metadata['graph']['edge_lengths']
    return target
...
def after_dataset(e: Experiment,
    ...
    e.log(f'target statistics'
          f' - mean: {np.mean(target_values):.6f}'
          f' - median: {np.median(target_values):.6f}')
