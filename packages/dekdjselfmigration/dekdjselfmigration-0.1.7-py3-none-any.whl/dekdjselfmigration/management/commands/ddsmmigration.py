...
from dekdjtools.utils.migration import list_migration_paths, list_migration_entries, project_dir
...
class Command(CommandBasic):
    def handle(self):
        ...
        if mrc:
            ...
            if all(migrations.get(k, set()) == set(vv) for k, vv in mrc.migrations.items()):
                ...
                MigrationRecordCorrection.objects.delete()
        ...
        if need_record:
            MigrationRecordCorrection.objects.create(content=content, migrations=migrations)
        self.do_migrate()
        if need_record:
    ...
    def do_makemigrations(cls, *args, **kwargs):
        call_command('pgmakemigrations' if cls.has_psqlextra_backend() else 'makemigrations', *args, **kwargs)
