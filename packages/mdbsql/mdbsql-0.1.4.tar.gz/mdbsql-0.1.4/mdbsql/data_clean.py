    ...
    def change_and_sort(self, path=None):
        ...
        if self.set_up_to_mogo:
            ...
            d = mongo_update.UploadMongo(username=username, password=password, host=host, port=port,
                                         upload_path=self.path, drop_duplicates=False
                                         )
        ...
        for root, dirs, files in os.walk(path, topdown=False):
            for name in files:
                ...
                for tg_name in tg_names:
                    if tg_name in name and '汇总' not in name and name.endswith('.csv'):  # 人群报表排除达摩盘报表： 人群报表汇总
                        ...
                        if '万里马' in name:
                            ...
                            if self.set_up_to_mogo:
                                d.add2mongo(df=df, db_name='天猫数据1', collection_name=f'天猫_推广_{tg_name}')
                            if self.set_up_to_mysql:
                ...
                if name.endswith('.csv') and '超级直播' in name:
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='天猫数据1', collection_name='天猫_推广_超级直播')
                    if self.set_up_to_mysql:
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='天猫数据1', collection_name='天猫_推广_短直联投')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root_new, new_name2)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='天猫数据1', collection_name='天猫_推广_超级短视频')
                    if self.set_up_to_mysql:
                ...
                if '人群报表汇总' in name:
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='天猫数据1', collection_name='天猫_达摩盘_DMP报表',)
                    if self.set_up_to_mysql:
                ...
                if name.endswith('.xls') and '生意参谋' in name and '无线店铺流量来源' in name:
                    ...
                    if '经营优势' in df['一级来源'].tolist():  # 新版流量
                        if '数据周期' in df.columns.tolist():
                            if self.set_up_to_mogo:
                                d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_店铺来源_月数据_新版')
                            if self.set_up_to_mysql:
                                m.df_to_mysql(df=df, db_name='生意参谋数据1', tabel_name='生意参谋_店铺来源_月数据_新版')
                        else:
                            if self.set_up_to_mogo:
                                d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_店铺来源_日数据_新版')
                            if self.set_up_to_mysql:
                                m.df_to_mysql(df=df, db_name='生意参谋数据1', tabel_name='生意参谋_店铺来源_日数据_新版')
                    else:  # 旧版流量
                        if '数据周期' in df.columns.tolist():
                            if self.set_up_to_mogo:
                                d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_店铺来源_月数据')
                            if self.set_up_to_mysql:
                                m.df_to_mysql(df=df, db_name='生意参谋数据1', tabel_name='生意参谋_店铺来源_月数据')
                        else:
                            if self.set_up_to_mogo:
                                d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_店铺来源_日数据')
                            if self.set_up_to_mysql:
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_商品排行')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df,db_name='生意参谋数据1', collection_name='生意参谋_自助取数_整体日报')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_自助取数_每日流量')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_自助取数_商品sku')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_自助取数_店铺流量_月数据')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意经1', collection_name='生意经_宝贝指标')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意经1', collection_name='生意经_店铺指标')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意经1', collection_name='生意经_地域分布_省份城市分析')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意经1', collection_name='生意经_订单数据')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_直播间成交订单明细')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_直播间大盘数据')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意参谋数据1', collection_name='生意参谋_直播业绩')
                    if self.set_up_to_mysql:
                    ...
                    for sheet4 in sheets4:
                        ...
                        if len(df) < 1:
                            ...
                            if self.set_up_to_mogo:
                                d.add2mongo(df=df, db_name='天猫数据1', collection_name='天猫_推广_品销宝')
                            if self.set_up_to_mysql:
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='市场数据1', collection_name='淘宝店铺数据')
                    if self.set_up_to_mysql:
                    ...
                    if len(df) == 0:
                        ...
                        continue
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='天猫数据1', collection_name='万相台_人群洞察')
                    if self.set_up_to_mysql:
                ...
                # ----------------------- 京东数据处理分界线 -----------------------
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df,db_name='京东数据1', collection_name='京东_流量来源_日数据')
                    if self.set_up_to_mysql:
                    ...
                    if self.set_up_to_mogo:
                        if 'sku' in new_name:
                            d.add2mongo(df=df,db_name='京东数据1', collection_name='京东_sku_商品明细')
                        elif 'spu' in new_name:
                            d.add2mongo(df=df,db_name='京东数据1', collection_name='京东_spu_商品明细')
                    if self.set_up_to_mysql:
                        if 'sku' in new_name:
                    ...
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df,db_name='京东数据1', collection_name='京东_商品词下排名')
                    if self.set_up_to_mysql:
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df,db_name='京东数据1', collection_name='京东_商品排名')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df,db_name='京东数据1', collection_name='京东_竞店监控_日数据')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='京东数据1', collection_name='京东_自助取数_店铺日报')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df,db_name='京东数据1', collection_name='京东_商家榜单')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='属性设置1', collection_name='京东商品信息')
                    if self.set_up_to_mysql:
                    ...
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='京东数据1', collection_name='京东_推广_京准通')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name2)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='京东数据1', collection_name='京东_推广_搜索词报表')
                    if self.set_up_to_mysql:
                    ...
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                        d.add2mongo(df=df, db_name='生意经1', collection_name='E3_零售明细统计')
                    if self.set_up_to_mysql:
            ...
            for name in files:
                if name.endswith('.xlsx') and '~' not in name:
                    ...
                    if '商品白底图' in col and '方版场景图' in col:
                        ...
                        try:
                            d.add2mongo(df=df, db_name='属性设置1', collection_name='淘宝天猫商品素材导出')
