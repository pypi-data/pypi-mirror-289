        ...
        try:
            with connection.cursor() as cursor:
                ...
                try:
                    ...
                    print(f'{now}正在更新 mysql ({self.host}) 数据库: {tabel_name}')
                    ...
                    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S: ")
                    print(f'{now}{tabel_name}数据异常, 正在回滚: {e}')
                    connection.rollback()
        finally:
            connection.close()
...
def download_datas(tabel_name, save_path, start_date):
    ...
    for result in results:
        ...
        df = m.read_mysql(db_name='pandas数据源', tabel_name=tabel_name, start_date=start_date, end_date=end_date)
        if len(df) == 0:
            continue
...
if __name__ == '__main__':
    ...
    if platform.system() == 'Windows':
        path = os.path.join('C:\\同步空间\\BaiduSyncdisk\\原始文件2', '属性设置\\城市等级')
    elif platform.system() == 'Darwin':
        import getpass
        path = os.path.join('/Users', getpass.getuser(), '数据中心/原始文件2', '属性设置')
    ...
    # print(df)
    m.df_to_mysql(df=df, db_name='属性设置 1', tabel_name='城市等级')
