        ...
        for root, dirs, files in os.walk(path, topdown=False):
            for name in files:
                ...
                for tg_name in tg_names:
                    if tg_name in name and '汇总' not in name and name.endswith('.csv'):  # 人群报表排除达摩盘报表： 人群报表汇总
                        ...
                        if '万里马' in name:
                            ...
                            dtypes = json_data.read_dypes(df=df, db_name='天猫数据2', collection_name=f'天猫_推广_{tg_name}')
                ...
                if name.endswith('.csv') and '超级直播' in name:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='天猫数据2', collection_name=f'天猫_推广_超级直播')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='天猫数据2', collection_name='天猫_推广_短直联投')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='天猫数据2', collection_name='天猫_推广_超级短视频')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root_new, new_name2)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                ...
                if '人群报表汇总' in name:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='天猫数据2', collection_name='天猫_达摩盘_DMP报表')
                    df = df.astype(dtypes, errors='ignore')
                ...
                if name.endswith('.xls') and '生意参谋' in name and '无线店铺流量来源' in name:
                    ...
                    if '经营优势' in df['一级来源'].tolist():  # 新版流量
                        if '数据周期' in df.columns.tolist():
                            ...
                            dtypes = json_data.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_店铺来源_月数据_新版')
                            df = df.astype(dtypes, errors='ignore')
                            if self.set_up_to_mogo:
                            ...
                            dtypes = json_data.read_dypes(df=df, db_name='生意参谋数据2',
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_商品排行')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_自助取数_整体日报')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意经2', collection_name='生意经_宝贝指标')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意经2', collection_name='生意经_店铺指标')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意经2', collection_name='生意经_地域分布_省份城市分析')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意经2', collection_name='生意经_订单数据')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_直播间成交订单明细')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    for sheet4 in sheets4:
                        ...
                        if len(df) < 1:
                            ...
                            dtypes = json_data.read_dypes(df=df, db_name='天猫数据2',
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='市场数据2', collection_name='淘宝店铺数据')
                    df = df.astype(dtypes, errors='ignore')
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='天猫数据2', collection_name='万相台_人群洞察')
                    df = df.astype(dtypes, errors='ignore')
                    if self.set_up_to_mogo:
                ...
                # ----------------------- 京东数据处理分界线 -----------------------
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_流量来源_日数据')
                    df = df.astype(dtypes, errors='ignore')
                    if self.set_up_to_mogo:
                    ...
                    if self.set_up_to_mogo:
                        if 'sku' in new_name:
                            ...
                            dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_sku_商品明细')
                            df = df.astype(dtypes, errors='ignore')
                            d.df_to_mongo(df=df,db_name='京东数据1', collection_name='京东_sku_商品明细')
                        elif 'spu' in new_name:
                            
                            dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_spu_商品明细')
                            df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_商品词下排名')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_商品排名')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_竞店监控_日数据')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_自助取数_店铺日报')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_商家榜单')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='属性设置2', collection_name='京东商品信息')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_推广_京准通')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='京东数据2', collection_name='京东_推广_搜索词报表')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name2)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = json_data.read_dypes(df=df, db_name='生意经2', collection_name='E3_零售明细统计')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
            ...
            for name in files:
                if name.endswith('.xlsx') and '~' not in name:
                    ...
                    if '商品白底图' in col and '方版场景图' in col:
                        ...
                        dtypes = json_data.read_dypes(df=df, db_name='属性设置2', collection_name='淘宝天猫商品素材导出')
                        df = df.astype(dtypes, errors='ignore')
...
class DataTypes:
    ...
    def read_dypes(self, db_name, collection_name, df=pd.DataFrame()):
        """ 读取 dtypes ，如果不存在json ，则根据 df 创建 dtypes """
        if len(df) == 0:
            return
        ...
        if not self.datas:  # 如果不存在本地 json 文件, 直接返回即可
            ...
            return dtypes[db_name][collection_name]
    ...
    def read_dypes_bak(self, db_name, collection_name, df=pd.DataFrame()):
