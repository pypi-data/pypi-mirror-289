...
import platform
import pathlib
from mdbsql import mongo_update as mdb
...
class DataClean:
    ...
    # @try_except
    def change_and_sort(self, host, port, username, password, path=None):
        """数据转换"""
        if not path:
            path = self.path
        ...
        for root, dirs, files in os.walk(path, topdown=False):
            for name in files:
                ...
                for tg_name in tg_names:
                    if tg_name in name and '汇总' not in name and name.endswith('.csv'):  # 排除达摩盘报表： 人群报表汇总
                ...
                if name.endswith('.csv') and '超级直播' in name:
                    ...
                    self.save_to_csv(df, root_new, new_name)
                    if self.set_up_to_mogo:
                    ...
                    self.save_to_csv(df, root_new, new_name2)
                    os.remove(os.path.join(root, name))
                ...
                if name.endswith('.xls') and '生意参谋' in name and '无线店铺流量来源' in name:
                    ...
                    for sheet4 in sheets4:
                        ...
                        if len(df) < 1:
                            ...
                            self.save_to_csv(df, root_new, new_file_name4)
                            if self.set_up_to_mogo:
                ...
                # ----------------------- 京东数据处理分界线 -----------------------
                    ...
                    df['日期'] = pd.to_datetime(df['日期'], format='%Y-%m-%d', errors='ignore')
                    self.save_to_csv(df, root, new_name2)
                    if self.set_up_to_mogo:
...
def main():
    ...
    c.change_and_sort(host=host, port=port, username=username, password=password)
