        ...
        for root, dirs, files in os.walk(path, topdown=False):
            for name in files:
                ...
                for tg_name in tg_names:
                    if tg_name in name and '汇总' not in name and name.endswith('.csv'):  # 人群报表排除达摩盘报表： 人群报表汇总
                        ...
                        if '万里马' in name:
                            ...
                            dtypes = d_j.read_dypes(df=df, db_name='天猫数据2', collection_name=f'天猫_推广_{tg_name}')
                ...
                if name.endswith('.csv') and '超级直播' in name:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='天猫数据2', collection_name=f'天猫_推广_超级直播')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='天猫数据2', collection_name='天猫_推广_短直联投')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='天猫数据2', collection_name='天猫_推广_超级短视频')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root_new, new_name2)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                ...
                if '人群报表汇总' in name:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='天猫数据2', collection_name='天猫_达摩盘_DMP报表')
                    df = df.astype(dtypes, errors='ignore')
                ...
                if name.endswith('.xls') and '生意参谋' in name and '无线店铺流量来源' in name:
                    ...
                    if '经营优势' in df['一级来源'].tolist():  # 新版流量
                        if '数据周期' in df.columns.tolist():
                            ...
                            dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_店铺来源_月数据_新版')
                            df = df.astype(dtypes, errors='ignore')
                            if self.set_up_to_mogo:
                            ...
                            dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2',
                                                    collection_name='生意参谋_店铺来源_日数据_新版')
                            df = df.astype(dtypes, errors='ignore')
                            if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_商品排行')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_自助取数_整体日报')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2',
                                            collection_name='生意参谋_自助取数_每日流量')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2',
                                            collection_name='生意参谋_自助取数_商品sku')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2',
                                            collection_name='生意参谋_自助取数_店铺流量_月数据')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意经2', collection_name='生意经_宝贝指标')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意经2', collection_name='生意经_店铺指标')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意经2', collection_name='生意经_地域分布_省份城市分析')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意经2', collection_name='生意经_订单数据')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意参谋数据2', collection_name='生意参谋_直播间成交订单明细')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    for sheet4 in sheets4:
                        ...
                        if len(df) < 1:
                            ...
                            dtypes = d_j.read_dypes(df=df, db_name='天猫数据2',
                                                    collection_name='天猫_推广_品销宝')
                            df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='市场数据2', collection_name='淘宝店铺数据')
                    df = df.astype(dtypes, errors='ignore')
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='天猫数据2', collection_name='万相台_人群洞察')
                    df = df.astype(dtypes, errors='ignore')
                    if self.set_up_to_mogo:
                ...
                # ----------------------- 京东数据处理分界线 -----------------------
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_流量来源_日数据')
                    df = df.astype(dtypes, errors='ignore')
                    if self.set_up_to_mogo:
                    ...
                    if self.set_up_to_mogo:
                        if 'sku' in new_name:
                            ...
                            dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_sku_商品明细')
                            df = df.astype(dtypes, errors='ignore')
                            d.df_to_mongo(df=df,db_name='京东数据1', collection_name='京东_sku_商品明细')
                        elif 'spu' in new_name:
                            
                            dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_spu_商品明细')
                            df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_商品词下排名')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_商品排名')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_竞店监控_日数据')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_自助取数_店铺日报')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_商家榜单')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='属性设置2', collection_name='京东商品信息')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_推广_京准通')
                    df = df.astype(dtypes, errors='ignore')
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='京东数据2', collection_name='京东_推广_搜索词报表')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name2)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
                    ...
                    dtypes = d_j.read_dypes(df=df, db_name='生意经2', collection_name='E3_零售明细统计')
                    df = df.astype(dtypes, errors='ignore')
                    self.save_to_csv(df, root, new_name)  # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                    if self.set_up_to_mogo:
        ...
        for root, dirs, files in os.walk(path, topdown=False):
            for name in files:
                ...
                if name.endswith('.csv') and '无线店铺流量来源' in name:
                    ...
                    bib(t_path, _as_month=True)
                elif name.endswith('.csv') and '客户画像' in name:
                    t_path = str(pathlib.Path(self.source_path, '生意参谋/客户画像'))
                    bib(t_path, _as_month=True)
            ...
            for name in files:
                if name.endswith('.xlsx') and '~' not in name:
                    ...
                    if '商品白底图' in col and '方版场景图' in col:
                        ...
                        dtypes = d_j.read_dypes(df=df, db_name='属性设置2', collection_name='淘宝天猫商品素材导出')
                        df = df.astype(dtypes, errors='ignore')
                        # mysql 可能改变 df 列名，所以在上传 mysql 前保存 csv
                        self.save_to_csv(df, new_save_path, new_name, encoding='utf-8_sig')
                        try:
                            if self.set_up_to_mogo:
                                d.df_to_mongo(df=df, db_name='属性设置1', collection_name='淘宝天猫商品素材导出')
                            if self.set_up_to_mysql:
                                m.df_to_mysql(df=df, db_name='属性设置1', tabel_name='淘宝天猫商品素材导出')
                        except Exception as e:
                            print(e)
                        os.remove(os.path.join(root, name))
...
class DataTypes:
    def __init__(self):
        if platform.system() == 'Windows':
            self.path = f'C:\\Users\\{getpass.getuser()}'
        elif platform.system() == 'Darwin':
            self.path = f'/Users/{getpass.getuser()}'
        else:
            self.path = 'Downloads'  # 不可用
        self.__num__ = 0  # 避免无限递归

    def update_json(self, df, db_name, collection_name):
        """ 根据 df 创建 json 文件，信息包含 dtypes """
        if len(df) == 0:
            return
        ...
        try:
            with open(os.path.join(self.path, 'data_types.json'), 'r', encoding='utf-8_sig') as json_file:
                dtypes_before = json.load(json_file)
                for k, v in dtypes_before.items():
                    # 允许通过文件手动指定，所以用 dtypes 更新 dtypes_before
                    if db_name in k:
                        for j, p in dtypes_before[db_name].items():
                            if collection_name in j:
                                dtypes[db_name][collection_name].update(dtypes_before[db_name][collection_name])
                            else:
                                dtypes[db_name].update(dtypes_before[db_name])
                    else:
                        dtypes.update(dtypes_before)
        except:
            pass

        with open(os.path.join(self.path, 'data_types.json'), 'w', encoding='utf-8_sig') as json_file:
            json.dump(dtypes, json_file, ensure_ascii=False, sort_keys=True, indent=4)
        time.sleep(0.5)

    def read_dypes(self, db_name, collection_name, df=pd.DataFrame()):
        """ 读取 dtypes ，如果不存在json ，则根据 df 创建 json 并返回 dtypes """
        try:
            with open(os.path.join(self.path, 'data_types.json'), 'r', encoding='utf-8_sig') as json_file:
                dtypes_before = json.load(json_file)
                return dtypes_before[db_name][collection_name]
            time.sleep(0.5)
        except:
            if len(df) == 0:
                print(f'写 data_types.json 文件报错，需要传入 df')
                return
            self.update_json(df=df, db_name=db_name, collection_name=collection_name)
            if self.__num__ == 0:
                self.__num__ += 1
                return self.read_dypes(df=df, db_name=db_name, collection_name=collection_name)
            else:
                self.__num__ -= 1  # 复原，以便只需要实例化一次
...
def main():
    ...
    c.set_up_to_mogo = True
    c.set_up_to_mysql = True
