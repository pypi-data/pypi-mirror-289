...
class OptimizeDatas:
    ...
    def delete_duplicate(self, collection_name, date,):
        ...
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S ')
        print(f'{now}{self.db_name}/{collection_name} -> {date.strftime("%Y-%m-%d")} '
              f'remove: {len(duplicate_id)},  took: {cost_time}')
    ...
    def delete_duplicate2(self, collection_name,):
        ...
        cost_time = round(time.time() - before, 2)
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S ')
        print(f'{now}{self.db_name}/{collection_name} -> remove: {len(duplicate_id)},  took: {cost_time}')
