...
class DataTypes:
    def __init__(self):
        if platform.system() == 'Windows':
            self.path = f'C:\\同步空间\\BaiduSyncdisk\\自动0备份\\py\\数据更新\\support'
        elif platform.system() == 'Darwin':
            self.path = f'/Users/{getpass.getuser()}/数据中心/自动0备份/py/数据更新/support'
        else:
    ...
    def read_dypes(self, db_name, collection_name, df=pd.DataFrame(), is_file_dtype=True):
        """
    ...
    def convert_df_cols(self, df):
        """ 清理 dataframe 列名的不合规字符 """
        # dtypes = df.dtypes.apply(str).to_dict()  # 将 dataframe 数据类型转为字典形式
        df.replace([np.inf, -np.inf], np.nan, inplace=True)  # 清理一些非法值
        cols = df.columns.tolist()
        for col in cols:
