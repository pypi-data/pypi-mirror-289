...
from darwin.importer.importer import _get_remote_files
...
class DarwinAPI(foua.AnnotationAPI):
    ...
    def download_annotations(self, results):
        ...
        OBB = results.backend.config.OBB
        full_annotations = {}
        with (
            contextlib.nullcontext(tempfile.mkdtemp())
            if _DEBUG
                    ...
                    if is_darwin_video:
                        ...
                        for annotation in video_annotations:
                            ...
                            if "properties" in annotation:
                                for prop in annotation["properties"]:
                                    ...
                                    if prop["name"] in darwin_attributes:
                                        if (
                                            type(darwin_attributes[prop["name"]])
                                            == list
                                        ):
                            ...
                            if "polygon" in frame_annotation and label_type in [
                                ...
                                if (
                                    len(frame_annotation["polygon"]["paths"]) == 1
                                    and not OBB
                                ):
                                    ...
                                    logger.info(f"Processing complex polygons")
                                    voxel_annotation = _v7_to_51_polygon(
                                        annot_name,
                                        frame_annotation["polygon"],
                                        height,
                                ...
                                if OBB:
                                    logger.info(f"Creating Oriented Bounding Box")
                                    OBB_path = _v7_polygon_to_obb(
                                        frame_annotation["polygon"]
                                    )
                                    voxel_annotation = _v7_to_51_polygon(
                                        annot_name,
                            ...
                            # Need to add condition for adding both bbox, obb, and polygon
                            else:
                    ...
                    # Image annotations
                        ...
                        for annotation in data["annotations"]:
                            ...
                            if "polygon" in annotation and label_type in [
                                ...
                                if len(annotation["polygon"]["paths"]) == 1 and not OBB:
                                    logger.info(f"Processing simple polygons")
                                    voxel_annotation = _v7_to_51_polygon(
                                        annot_name,
                                        annotation["polygon"],
                                ...
                                if OBB:
                                    logger.info(f"Creating Oriented Bounding Box")
                                    OBB_path = _v7_polygon_to_obb(annotation["polygon"])
                                    voxel_annotation = _v7_to_51_polygon(
                                        annot_name,
                                        OBB_path,
                                        height,
...
def _v7_to_51_polygon(label, polygon, height, width, attributes=None, OBB=False):
    """
    Converts a V7 polygon to a 51 polygon
    """

    filled = True if len(polygon["paths"]) == 1 and OBB == False else False
    poly_list = [
...
def _v7_polygon_to_obb(polygon_paths):
    """
    Function to convert a polygon to an oriented bounding box by taking the principal components
    """
    points = np.array(
        [[p["x"], p["y"]] for path in polygon_paths["paths"] for p in path]
    )

    covariance_points = np.cov(points, y=None, rowvar=0, bias=1)

    v, vect = np.linalg.eig(covariance_points)
    ...
    corners = [{"x": corner[0], "y": corner[1]} for corner in list(corners)]
    return {"paths": [list(corners)]}
