...
FlowAI is a powerful CLI tool for multi-agent LLM tasks. It allows you to interact with various Language Models from different providers and manage complex, multi-step tasks.

Let's get started by setting up your FlowAI configuration!
"""
    console.print(Panel(Markdown(welcome_message), expand=False))
...
def init_config():
    ...
    # Prepare choices for providers
    provider_choices = []
    for provider, models in available_models.items():
        if models[0] != "No API key set" and models[0] != "Error fetching models":
            provider_choices.append(Choice(provider, value=provider))

    if not provider_choices:
        console.print("[bold red]No models available. Please check your API keys and try again.[/bold red]")
        return
    ...
    shutil.copy("template-commit-message.txt", prompts_dir / "template-commit-message.txt")
    shutil.copy("template-pull-request.txt", prompts_dir / "template-pull-request.txt")
...
def main(
    ...
    if list_models:
        ...
        for provider, provider_models in models.items():
            ...
            if provider_models[0] == "No API key set":
                console.print(f"  [yellow]No API key detected. See {provider_urls[provider]} to set one.[/yellow]")
            elif provider_models[0] == "Error fetching models":
                console.print(f"  [yellow]Error fetching models. Please check your configuration.[/yellow]")
            elif provider == "ollama" and provider_models[0] == "Ollama not installed or running":
                console.print(f"  [yellow]Ollama is not installed or not running. Go to {provider_urls['ollama']} to install it.[/yellow]")
            else:
