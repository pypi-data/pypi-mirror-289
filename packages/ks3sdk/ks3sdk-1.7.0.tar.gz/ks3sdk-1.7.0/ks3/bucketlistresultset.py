...
def bucket_lister(bucket, marker='', **params):
    """
    ...
    k = None
    while more_results:
        rs = bucket.get_all_keys(marker=marker, **params)
        for k in rs:
            yield k
        if k:
...
class BucketListResultSet(object):
    ...
    def __init__(self, bucket=None, prefix='', delimiter='', marker='', max_keys='', encoding_type='', continuation_token='', fetch_owner='', list_type=None, start_after=''):
        self.bucket = bucket
        self.prefix = prefix
    ...
    def __iter__(self):
        return bucket_lister(self.bucket, prefix=self.prefix,
                             delimiter=self.delimiter, marker=self.marker, max_keys=self.max_keys, encoding_type=self.encoding_type, continuation_token=self.continuation_token, fetch_owner=self.fetch_owner, list_type=self.list_type, start_after=self.start_after)
