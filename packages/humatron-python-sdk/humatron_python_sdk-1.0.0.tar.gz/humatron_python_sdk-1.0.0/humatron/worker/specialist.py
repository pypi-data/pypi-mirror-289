...
_logger = logging.getLogger('humatron.sdk')


class HumatronSpecialist(ABC):
    """
    Abstract base class for a Humatron specialist.
    """
...
class HumatronSpecialistAsyncAdapter(HumatronSpecialist, ABC):
    """
    Asynchronous adapter for Humatron specialist using a thread pool executor.
    """
    ...
    def post_request(self, req: Request) -> Optional[Response]:
        ...
        return Response(
            HumatronSpecialistAsyncAdapter.make_id(), _utc_now(), (payloads if payloads else None), self._storage.copy()
        )
