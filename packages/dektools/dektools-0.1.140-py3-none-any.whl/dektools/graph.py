...
class Graph:
    ...
    def load_from_data(self, nodes, edges, node=None, label=None, ns=None, es=None):
        ...
        for _node in nodes:
            ...
            self.g.add_node(node_, label=ln, **_ns)
        for edge in edges_flat:
            _es = {}
            if es is not None:
                _es = es(*edge) or _es
            self.g.add_edge(*edge, **_es)
        return self
    ...
    def to_svg(self):
        return networkx.drawing.nx_pydot.to_pydot(self.g).create_svg()

    def get_cycles(self):
        return networkx.recursive_simple_cycles(self.g)
