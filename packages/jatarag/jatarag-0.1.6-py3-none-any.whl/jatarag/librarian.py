...
from typing import Literal, Generator, get_args
...
def synthesize_answer(agent: Agent, query: str, paragraphs: list[str], stream: bool = False, chat_history: list[Message] = []) -> str | Generator[str, None, None]:
    """
    ...
    Parameters:
        ...
        stream (bool): whether to stream the answer or not. If False, returns a single string, Otherwise returns a generator. Defaults to False
        chat_history (list[Message]): the chat history to use as context. Defaults to an empty list
