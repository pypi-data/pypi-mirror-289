...
import pathlib
import shutil
from config_poetry import generate_output as go
#import generate_output as go
#import platformView as pv
from config_poetry import platformView as pv
from config_poetry import SMC_errors as SMCErrors
from config_poetry import NPC_errors as NPCErrors
from config_poetry import PanoramaTSF
from config_poetry import EOL_errors as EOLErrors
...
from config_poetry import DP_CPU_reports as DPCPUReports
from config_poetry import MP_CPU_reports as MPCPUReports 
...
def main():
    ...
    DP_CPU_Files = DPCPUReports.DP_CPU_reports.find_dp_monitor_log(path)
    DP_CPU_Contents = DPCPUReports.DP_CPU_reports.open_dp_monitor_log(DP_CPU_Files)
    DP_CPU_Report = []

    if(DP_CPU_Contents != []):
        # look for high CPU
        DP_CPU_Report = DPCPUReports.DP_CPU_reports.dp_monitor_report(DP_CPU_Contents)
        
    MP_CPU_Files = MPCPUReports.MP_CPU_reports.find_mp_monitor_log(path)
    MP_CPU_Contents = MPCPUReports.MP_CPU_reports.open_mp_monitor_log(MP_CPU_Files)
    MP_CPU_Report = []
    Process_CPU_Report = []

    if(MP_CPU_Contents != []):
        MP_CPU_Report = MPCPUReports.MP_CPU_reports.mp_monitor_report(MP_CPU_Contents)
        
        Process_CPU_Report = MPCPUReports.MP_CPU_reports.mp_process_report(MP_CPU_Contents)
