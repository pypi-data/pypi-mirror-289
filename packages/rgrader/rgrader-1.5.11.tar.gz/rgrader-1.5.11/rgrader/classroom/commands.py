...
def parse_grading_schema(grading_schema_path: str) -> GradingSchema:
    ...
    root_directory = ""
    if 'root_directory' in grading_schema_dict['schema']:
        root_directory: str = grading_schema_dict['schema']['root_directory'].strip()
        if root_directory[-1] != '/':
            root_directory += '/'
    else:
        root_directory = os.path.dirname(os.path.abspath(grading_schema_path)) + '/'
    rules = []
    for rule in grading_schema_dict['schema']['rules'].values():
        if rule['test_path'][0] != '/':
            rule['test_path'] = root_directory + rule['test_path']
