...
def get_user_data_dir(browser_type):
    ...
    # This is required as BrowserType the object is not the same across different instances of
    # the enum BrowserType. ex. From the backend to the CLI
...
async def create_browser(
    ...
    # Need to check browser_type.value as BrowserType enum on the backend and
    # in the CLI are not the same.
    if use_existing_instance:
        user_data_dir = get_user_data_dir(browser_type)

        if not os.path.exists(user_data_dir):
            raise JelverBrowserException(
                f'User browser data directory does not exist: {user_data_dir}'
            )

        if browser_type.value == BrowserType.FIREFOX:
            return await playwright.firefox.launch_persistent_context(
                user_data_dir=user_data_dir
            )

        if browser_type.value in [BrowserType.CHROMIUM, BrowserType.EDGE]:
            return await playwright.chromium.launch_persistent_context(
                user_data_dir=user_data_dir
            )

        if browser_type.value in [BrowserType.WEBKIT, BrowserType.SAFARI]:
            raise JelverBrowserException(
                f'Using existing sessions is not supported for {browser_type.value}.'
            )
    ...
    if browser_type.value == BrowserType.FIREFOX.value:
        return await playwright.firefox.launch(args=args)

    if browser_type.value in [BrowserType.CHROMIUM.value, BrowserType.EDGE.value]:
        return await playwright.chromium.launch(args=args)

    if browser_type.value in [BrowserType.WEBKIT.value, BrowserType.SAFARI.value]:
