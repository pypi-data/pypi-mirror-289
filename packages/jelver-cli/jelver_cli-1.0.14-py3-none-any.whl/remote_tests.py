...
class RemoteTests:
    ...
    def validate_input(self):  
        ...
        if not self.url or not self.username or not self.password:
            print('Please provide the following information to run the test')
    ...
    def initialize_test(self):
        ...
        print('Initializing the test witht the following information:')
    ...
    def run(self):
        ...
        if not flag_success:
            failed_cases = status['result']['caseResults']
            raise JelverTestException(
                status['result']['testException'],
                failed_cases,
                self.job_id
            )
