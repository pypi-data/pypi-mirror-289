...
def scatterplots( ip ):
    ...
    gu.plot_scatterplot_of_two_features( feature_1, feature_2, features_df.index, [resin_data.loc[i]["Label"] for i in features_df.index], errorbars = True, std = std, line_of_best_fit = False, exponential_fit = False, xlabel = features_metadata.loc[feature_1_labels[0]]["Axis_Label"], ylabel = features_metadata.loc[feature_2_labels[0]]["Axis_Label"], savefig = True, filename = ip.output_directory + "Global/Scatterplots/New_Figure" + ext )

    return feature_1, feature_2, std
