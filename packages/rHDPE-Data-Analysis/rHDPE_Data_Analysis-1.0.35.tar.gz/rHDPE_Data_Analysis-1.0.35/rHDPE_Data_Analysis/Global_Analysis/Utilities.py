...
def scatterplots( directory, output_directory, sample_mask ):

    feature_1_labels = ["DSC_Crystallinity"]
    feature_2_labels = ["TT_YM"]

    features_df, std_of_features_df = Preprocessing.read_unnormalised_data( output_directory, feature_1_labels, feature_2_labels, sample_mask )
    ...
    gu.plot_scatterplot_of_two_features( feature_1, feature_2, features_df.index, [resin_data.loc[i]["Label"] for i in features_df.index], errorbars = True, std = std, line_of_best_fit = False, exponential_fit = False, xlabel = "Crystallinity [%]", ylabel = "Young's Modulus", savefig = False, filename = output_directory + "Global/Scatterplots/New_Figure.pdf" )
