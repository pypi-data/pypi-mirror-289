...
def read_documents(directory, api_key):
    ...
    new_files = [filename for filename in all_files if filename not in cache]
    ...
    for filename in new_files:
        ...
        cache[filename] = {"text": text, "embedding": document_embedding, "metadata": metadata, "id": doc_id}

    # Retrieve cached data for all files in the directory
    for filename in all_files:
        if filename in cache:
            cached_data = cache[filename]
            documents.append(cached_data["text"])
            metadatas.append(cached_data["metadata"])
            ids.append(cached_data["id"])
...
def query_index(query_embedding, index, metadatas, documents, embeddings, heat="temperate", n_results=1, roaming=False):
    heat_levels = {"frostbite": 0.1, "glacial": 0.2, "arctic": 0.3, "chill": 0.4, "temperate": 0.5, "warmth": 0.6, "blaze": 0.7, "scorch": 0.8, "inferno": 0.9, "supernova": 1.0, "docusearch": 1.1}
...
def process_best_matching_documents(query_text, api_key, folder_path, model="gpt-4", short_response=False, document_type="default", num_documents=1, heat="temperate", roaming=False):
    ...
    logging.info(f"Processing documents...")
