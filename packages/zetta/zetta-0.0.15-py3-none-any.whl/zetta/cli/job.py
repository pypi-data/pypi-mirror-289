...
@job_cli.command(name="init", help="Initialize a new fine-tune job at designated location.")
@synchronizer.create_blocking
async def init(framework: str = "llama-factory", location: str = "."):
    """
    Download a new fine-tune job config template at designated file location.
    """
    pass

@job_cli.command(name="run", help="Create a new fine-tune job with given configuration.")
@synchronizer.create_blocking
async def run(config: str):
    """
    Submit a new fine-tune job with given config file. Return a job id.
    """
    pass
