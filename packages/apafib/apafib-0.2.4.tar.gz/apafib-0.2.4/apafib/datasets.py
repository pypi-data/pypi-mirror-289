...
def fetch_dataset(fname, compressed=False, index=False):
    if compressed:
        if index:
            return pd.read_csv(
                f"{DATALINK}/{fname}.csv.gz", na_values="", sep=",", header=0, index_col=0
            )
        else:
            return pd.read_csv(
...
def load_sms_spam():
    return fetch_dataset("SMSSpamCollection", compressed=True)
...
def load_MNIST(digits=(4, 7, 9), sel=4):
    ...
    X_train_o = load_data("https://drive.google.com/file/d/1mC7W4gVYehyskvv41kIulKrOPYCEYbj5")
    X_test_o = load_data("https://drive.google.com/file/d/1l7wY1-ISJhNxrMXcvB82t4WgXsXoFoLE")
    y_train_o = load_data(
        "https://drive.google.com/file/d/1g3R8ce87JQkrfRfGhMOpCDDKji_t_H79", labels=True
    )
    y_test_o = load_data(
        "https://drive.google.com/file/d/1DtQ_B1N3iW1jEB23XCMljgGiuSlRjq3J", labels=True
    )
    ...
    # y_train_o = load_data(
    #     "http://yann.lecun.com/exdb/mnist/train-labels-idx1-ubyte.gz", labels=True
    # )
    # y_test_o = load_data(
    #     "http://yann.lecun.com/exdb/mnist/t10k-labels-idx1-ubyte.gz", labels=True
    # )

    X_train = X_train_o[
        np.logical_or(
            y_train_o == digits[0],
            np.logical_or(y_train_o == digits[1], y_train_o == digits[2]),
        )
    ]
    y_train = y_train_o[
        np.logical_or(
    ...
    X_test = X_test_o[
        np.logical_or(
            y_test_o == digits[0],
            np.logical_or(y_test_o == digits[1], y_test_o == digits[2]),
        )
    ]
    y_test = y_test_o[
        np.logical_or(
