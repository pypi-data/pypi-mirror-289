...
def OutFile_Docx(folder_path,*args):
    for i in range(len(args)//2):
        dic = args[2*i]
        file_path = args[2*i+1]
        ...
        save_path = os.path.join(folder_path, os.path.basename(file_path)) 
        doc.save(save_path)
...
def getFileLoc(*files):
    ...
    if isinstance(files[-1],bool):
        path = filedialog.askdirectory(title=files[0])
