...
class CassandraBuilder(BaseBuilder):
    operators = [
        '=', '<', '>', '<=', '>=', '<>', '!=',
    ...
    def create(self, data):
        if data:
            if isinstance(data, dict):
                data = [{key: value for key, value in data.items() if key in self.__model__.columns}]
                return self._get_connection().execute(self._compile_create(data))
            if isinstance(data, list):
                connect = self._get_connection()
                session = connect.connect()
                batch = BatchStatement()
                for item in data:
                    columns = tuple(item.keys()).__str__().replace("\'", '"')
                    values = tuple(item.values())
                    sql = "insert into {} {} values ({})".format(self._tablename(), columns, ",".join(["?"] * len(values)))
                    log = "insert into {} {} values {}".format(self._tablename(), columns,  tuple(values))
                    connect.log(log)
                    batch.add(session.prepare(sql), values)
    ...
    def orderby(self, column, direction='asc'):
        if direction.lower() == 'asc':
            self.__orderby__.append('"{}"'.format(column))
        else:
            self.__orderby__.append('"{}"'.format(column) + ' desc')
        return self

    def execute(self, sql):
        return list(self._get_connection().execute(sql))
    ...
    def _compile_dict(self, data):
        return ['"{}"={}'.format(index, expr.format_string(value)) for index, value in
                data.items()]
