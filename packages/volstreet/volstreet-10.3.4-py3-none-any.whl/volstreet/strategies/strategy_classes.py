    ...
    def logic(
        ...
        if diversify_times is None:
            ...
            def scan_for_eligible_strangle(diversify_time_: datetime) -> Strangle:
                ...
                cutoff_time = diversify_time_ + timedelta(
                    minutes=cut_off_minutes
                )  # Hardcoded
                logger.info(
                    f"{underlying.name} finding strangle with cutoff time {cutoff_time}"
                )
                ...
                while current_time() < cutoff_time:
                    ...
                    if strangle_ not in previous_strangles:
                        logger.info(
                            f"{underlying.name} diversification strangle found at {current_time()} : {strangle_}"
                        )
                        return strangle_
            ...
            for diversify_time in diversify_times:
                diversify_time = datetime.combine(
                    current_time().date(), time(*diversify_time)
                )
                strangle_at_time = scan_for_eligible_strangle(diversify_time)
                previous_strangles.add(strangle_at_time)

            # noinspection PyUnboundLocalVariable
            actual_start_time = diversify_time + timedelta(
                minutes=start_interval_minutes
            )  # Last diversify time + start interval
            strangle = scan_for_eligible_strangle(actual_start_time)
