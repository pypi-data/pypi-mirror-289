...
if __name__ == '__main__':
    ...
    REPLAY = int(input('Кол-во повторов? '))
    logger = Logger('log.txt', level='DEBUG', buffer_size=int(input('Размер буфера?')))
    logger.rotate_logs(0.01, True)
    all_ex = []
    for i in range(REPLAY):
        start = datetime.datetime.now()
        print(f'Попытка {i + 1}: {start}')
        ...
        job_time = end - start
        all_ex.append(job_time)
        print(f'Затраченное время на работу: \033[92m{job_time.total_seconds()}\033[0m ({i + 1})')

    all_ex_seconds = [td.total_seconds() for td in all_ex]
    average_time = sum(all_ex_seconds) / len(all_ex_seconds)
    print('\n------------------------------------')
    print(f'\nИТОГ: {average_time} секунд на {COUNT * 5} сообщений')
    print(f'Лучший результат: {min(all_ex_seconds)}')
    print(f'На 1000 сообщений: {average_time / COUNT / 5 * 1000}')
