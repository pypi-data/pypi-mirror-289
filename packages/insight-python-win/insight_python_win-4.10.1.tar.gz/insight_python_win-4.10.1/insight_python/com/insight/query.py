...
def get_all_basic_info(security_type=None, exchange=None, today=True):
    """
    :param security_type: 证券类型，必填，指数index, 股票stock, 基金fund, 债券bond, 期权option, 期货future, 期货组合合约spfuture, 权证warrant, 利率rate
    :param exchange: 证券市场，选填，支持多市场查询
    :param today: 是否查询当天最新数据，布尔类型，默认True
    :return: pandas.DataFrame
    """

    if security_type:
        if security_type not in ['index', 'stock', 'fund', 'bond', 'option', 'future', 'spfuture', 'warrant', 'rate']:
