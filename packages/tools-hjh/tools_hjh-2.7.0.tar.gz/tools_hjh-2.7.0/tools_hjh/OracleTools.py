...
class OracleTools:
    ...
    def get_table_ddl(dba_conn, username, table):
        ...
        sql = 'create table ' + metadata['owner'] + '.' + metadata['name'] + ''
        
        if metadata['comments'] != None:
            comments_str = '\ncomment on table ' + username + '.' + table + " is '" + metadata['comments'] + "';\n"
        else:
            comments_str = '\n'
        ...
        for col in metadata['columns']:
            ...
            col_str = col_str + name + ' ' + type_ + ' ' + default_value + ' ' + nullable + ',\n'
        col_str = Tools.merge_spaces(col_str.rstrip().rstrip(',')).replace(' ,', ',')
        col_str = col_str + '\n)'
                    ...
                    if len(part['subpartitions']) > 0:
                        ...
                        for subpart in part['subpartitions']:
                            if partition['subpartition_type'] == 'RANGE':
                                part_str = part_str + 'subpartition ' + subpart['name'] + ' values less than (' + part['value'] + '),\n'
                            if partition['subpartition_type'] == 'LIST':
                                part_str = part_str + 'subpartition ' + subpart['name'] + ' values (' + part['value'] + '),\n'
        ...
        sql = sql + col_str + part_str + comments_str + index_str + k_str
        return sql    
