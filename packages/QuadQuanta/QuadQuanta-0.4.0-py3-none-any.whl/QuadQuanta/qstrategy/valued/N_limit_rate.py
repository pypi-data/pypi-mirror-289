...
class NLimit(BaseStrategy):
    ...
    def on_day_bar(self, bars: np.ndarray):
        ...
        #
        if pre_close < pre_limit and pre_three_close < pre_three_limit:
            if pre_alp_rate > 6 and pre_close_rate < -3 and today_open_rate > -5:
                if today_high_rate > 7:
                    ...
                    if today_close == today_limit:
                        ...
                        self.not_limit_symbol.append(code)
    ...
    def syn_backtest(self):
        ...
        for i in range(2, len(self.trading_date)):
            try:
                ...
                pre_two_date = self.trading_date[i - 2]
                ...
                if len(self.not_limit_symbol) > 0:
                    logger.info(f"未涨停:{self.not_limit_symbol}")
                logger.info(f"总标:{self.all_count}")
                logger.info(f"涨停标:{self.limit_count}")
                self.res_symbol = []
                self.not_limit_symbol = []
