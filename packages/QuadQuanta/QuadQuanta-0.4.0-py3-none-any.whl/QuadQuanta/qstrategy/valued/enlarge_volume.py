...
class EnlargeVolume(BaseStrategy):
    def __init__(self,
                 code=None,
                 start_date=None,
                 end_date=None,
                 frequency='day'):
        super(EnlargeVolume, self).__init__(code, start_date, end_date,
                                            frequency)
    ...
    def on_day_bar(self, bars):
        ...
        last_two_close = round(last_two_bar['close'], 2)
        last_two_limit = round(last_two_bar['high_limit'], 2)

        today_high_rate = 100 * (today_high - last_close) / last_close
        # 收盘回落
        down_rate = 100 * (today_high - today_close) / today_close

        if (today_amount > 10 * pow(10, 8)) and  today_high == today_limit:
            if last_close == last_limit and down_rate > 6:
                if today_volume > 1.5 * last_volume:
                    if last_two_close < last_two_limit:
    ...
    def syn_backtest(self):
        for i in range(0, len(self.trading_date)):
            try:
                ...
                self.history_N_data = get_bars(end_time=date, count=3)
                ...
                logger.info(f"{date}")
                logger.info(f"放巨量：{self.limit_symbol_list}")
...
def daily_replay():
    ...
    replay = EnlargeVolume(start_date='2022-04-01',
                           end_date='2022-07-01',
                           frequency='daily')
