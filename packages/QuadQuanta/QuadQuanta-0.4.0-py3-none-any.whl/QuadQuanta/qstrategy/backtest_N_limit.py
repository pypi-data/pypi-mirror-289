...
class NLimit(BaseStrategy):
    ...
    def on_day_bar(self, bars: np.ndarray):
        ...
        if pre_three_high == pre_three_limit and pre_two_close < pre_two_limit:
            # 涨停次日高开
            # if pre_two_open > pre_three_close:
            #     # 涨停价格未突破高点
        ...
        # 隔三日涨停
        if pre_four_close == pre_four_limit and pre_two_close < pre_two_limit:
            # 非连扳
            if pre_five_close < pre_five_limit:
                if pre_three_close < pre_three_limit and pre_close < pre_limit:
        ...
        for i in range(0, len(self.trading_date)):
            try:
                ...
                for symbol in self.res_break_symbol:
                    if symbol in self.daily_break_symbol:
                        self.break_count += 1
                    else:
                        self.limit_count += 1
                ...
                if len(self.res_fall_symbol) > 0:
                    ...
                    res_symbol = []
                    for symbol in self.res_fall_symbol:
                        if symbol in last_ten_N_list:
                            res_symbol.append(symbol)
                    # logger.info(f"冲高回落N字板:{res_symbol}")
                if len(self.res_three_symbol) > 0:
                    logger.info(f"三日N字板:{self.res_three_symbol}")
