    ...
    def syn_backtest(self):
        ...
        for i in range(0, len(self.trading_date)):
            try:
                ...
                # self.last_two_limit_dict = []
                # 前日涨停股
                self.last_two_limit_dict = query_mongodb('QuadQuanta', 'daily_limit', sql={
                    'date': last_two_trade_day})
                self.last_two_limit_symbol = [symbol_data['code'] for symbol_data in self.daily_limit_dict]

                self.daily_break_symbol = []
                # 前日涨停股
                self.last_two_break_dict = query_mongodb('QuadQuanta', 'daily_break', sql={
                    'date': last_two_trade_day})
                self.last_two_break_symbol = [symbol_data['code'] for symbol_data in self.daily_break_dict]
