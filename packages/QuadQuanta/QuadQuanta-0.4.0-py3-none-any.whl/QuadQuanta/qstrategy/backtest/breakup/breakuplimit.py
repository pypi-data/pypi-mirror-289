...
class DailyNLimit(BaseStrategy):
    ...
    def on_day_bar(self, bars: np.ndarray):
        ...
        # if str(today_bar["code"]).endswith("600"):
        #     print("hello")
        # 振幅
        alp = 100 * (max_20_high - min_20_low) / min_20_low
        # 最近20日振幅小于50%
        if today_high > max_40_high:
            # 前N日未突破
            for i in range(-5, -1):
                ...
                max_high = round(max(bars[10:i - 1]['high']), 2)
                if close > max_high and close_rate > 3:
                    break
                if i == -2:
...
def daiyl_n_limit():
    ...
    test = DailyNLimit(
        start_date="2023-05-11", end_date="2023-06-01", frequency="daily", solid=True
    )
    test.syn_backtest()
