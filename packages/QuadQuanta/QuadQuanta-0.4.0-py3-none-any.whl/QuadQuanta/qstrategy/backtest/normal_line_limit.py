...
class LineLimit(BaseStrategy):
    ...
    def pick_symbol(self, bar):
        ...
        if amount > pow(10, 8):
            if open_ == high_limit and high_rate > 6:
    ...
    def next_day_backtest(self, bars):
        ...
        # 一字前一日实体阳线涨停
        if pre_close == pre_high_limit and pre_open < 0.95 * pre_close:
...
def daiyl_n_limit():
    ...
    test = LineLimit(
        start_date="2023-01-01", end_date="2023-04-01", frequency="daily", solid=True
    )
    test.syn_backtest()
