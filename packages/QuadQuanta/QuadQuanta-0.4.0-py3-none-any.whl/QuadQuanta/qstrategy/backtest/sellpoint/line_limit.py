...
class LineLimit(BaseStrategy):
    ...
    def pick_symbol(self, bar):
        ...
        if amount > pow(10, 8):
            if close == high_limit and open_ < close:
    ...
    def on_day_bar(self, bars: np.ndarray):
        ...
        if today_high > max_60_high * 0.97 and pre_close_rate < 9.9:
            # MA_60 = talib.SMA(bars['close'], 60)[60:]
            # # 30收盘价价序列
            # close_60 = bars['close'][60:]
            # lte_count = 0
            # for ma60,close60 in zip(MA_60,close_60):
            #     if ma60 > close60:
            #         lte_count += 1
            # if lte_count < 3:
    ...
    def next_day_backtest(self, bars):
        ...
        final_low = round(final_bar['low'], 2)
        ...
        if final_high_limit == final_open and high_rate > 6:
            #昨日未涨停
            if pre_close < pre_high_limit:
                self.success_symbol.append(symbol)
                if final_high_limit > final_close:
                    self.fail_symbol.append(symbol)
    ...
    def syn_backtest(self):
        ...
        for i in range(0, len(self.trading_date) - next_N_day):
            try:
                ...
                # history_N_data = get_bars(self.symbol_list, end_time=date, count=51)
                #
                ...
                history_Next_data = get_bars(self.symbol_list, end_time=next_date, count=3)
                for symbol in self.symbol_list:
                    if str(symbol).startswith('30'):
                        continue
                    bars = history_Next_data[history_Next_data["code"] == symbol]
                    if len(bars) == 3:
                        self.next_day_backtest(bars)
                if len(self.success_symbol) > 0:
                    logger.info(f"日期{date}")
                    # logger.info(f"{self.res_symbol}")
                    logger.info(f"{self.success_symbol}")
                    logger.info(f"一字炸板 {(self.fail_symbol)}")
...
def daiyl_n_limit():
    ...
    test = LineLimit(
        start_date="2023-01-01", end_date="2023-08-01", frequency="daily", solid=True
    )
    test.syn_backtest()
