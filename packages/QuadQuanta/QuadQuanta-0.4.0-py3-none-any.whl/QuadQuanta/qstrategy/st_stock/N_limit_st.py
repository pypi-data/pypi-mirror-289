    ...
    def syn_backtest(self):
        ...
        for i in range(0, len(self.trading_date)):
            try:
                ...
                self.daily_limit_dict = query_mongodb('QuadQuanta', 'daily_limit_st', sql={
                    'date': date})
                self.daily_limit_symbol = [symbol_data['code'] for symbol_data in self.daily_limit_dict]
                self.daily_break_symbol = []
                self.daily_break_dict = query_mongodb('QuadQuanta', 'daily_break_st', sql={
                    'date': date})
                ...
                if len(self.res_symbol) > 0:
                    ...
                    # save_mongodb('QuadQuanta', 'N_limit_st',
                ...
                if len(self.res_two_symbol) > 0:
                    ...
                    # save_mongodb('QuadQuanta', 'TwoN_limit_st',
...
def n_limit():
    ...
    #               end_date=end_time,
    #               frequency='daily')
    test = NLimit(start_date='2023-02-01',
                  end_date='2023-03-01',
                  frequency='daily')
