...
class PreNLimi(BaseStrategy):
    ...
    def on_day_bar(self, bars: np.ndarray):
        ...
        # 昨日最高大于5，收盘回落5个点，前日涨停
        if pre_high_rate > 5:
            if pre_high_rate > pre_close_rate + 5:
                if pre_two_close == pre_two_limit and pre_three_high < pre_three_limit:
    ...
    def syn_backtest(self):
        ...
        for i in range(2, len(self.trading_date)):
            try:
                ...
                # print(date_date)
                last_two_trade_day = self.trading_date[i - 2]
...
def n_limit():
    ...
    #               end_date=end_time,
    #               frequency='daily')
    test = PreNLimi(start_date='2022-08-01',
                    end_date='2022-09-01',
                    frequency='daily')
