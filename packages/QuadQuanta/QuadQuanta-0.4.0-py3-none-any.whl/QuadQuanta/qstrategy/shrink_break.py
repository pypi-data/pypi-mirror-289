...
class ShrinkLimit(BaseStrategy):
    def __init__(self,
                 code=None,
                 start_date=None,
                 end_date=None,
                 frequency='day'):
        super(ShrinkLimit, self).__init__(code, start_date, end_date, frequency)
    ...
    def on_symbol_bar(self, bar):
        ...
        code = bar['code']
        # 开盘大于3%,突破近期高点
        if open_ > 3 and open_ < 5:
            history_N_data = get_bars(bar['code'], end_time=str(today), count=30)[:-1]
            max_high = np.max(history_N_data['high'])
            if bar['open'] > max_high and bar['pre_close'] < max_high:
    ...
    def on_day_bar(self, bar):
        ...
        try:
            ...
            # 炸板股
            if (close == limit) and close_rate > 8:
...
if __name__ == '__main__':
    ...
    test = ShrinkLimit(start_date='2021-01-01',
                      end_date='2021-07-01',
                      frequency='daily')
