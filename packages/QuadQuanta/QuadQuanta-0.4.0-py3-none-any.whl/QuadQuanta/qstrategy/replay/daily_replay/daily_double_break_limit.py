...
class DailyDoubleLimitBreak(BaseStrategy):
    def __init__(self,
                 code=None,
                 start_date=None,
                 end_date=None,
                 frequency='day'):
        super(DailyDoubleLimitBreak, self).__init__(code, start_date, end_date,
                                                    frequency)
    ...
    def on_day_bar(self, bar):
        ...
        try:
            ...
            if close < limit and high_rate > 9 and high == limit:
        ...
        for i in range(0, len(self.trading_date)):
            try:
                ...
                for bar in self.today_data:
                    ...
                    self.on_day_bar(bar)
                logger.info(f"{date}")
                logger.info(f"连板炸板：{self.double_symbol_list}")
                self.double_symbol_list = []
...
def daily_double_replay():
    ...
    #                          end_date=end_time,
    #                          frequency='daily')
    test = DailyDoubleLimitBreak(start_date='2019-01-01',
                                 end_date='2021-01-01',
                                 frequency='daily')
