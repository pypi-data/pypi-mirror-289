...
class OpenDown(BaseStrategy):
    def __init__(self,
                 code=None,
                 start_date=None,
                 end_date=None,
                 frequency='day'):
        super(OpenDown, self).__init__(code, start_date, end_date,
                                       frequency)
        self.pre_volume_dict = {}
        self.limit_symbol_list = []
        self.fall_symbol_list = []
        self.break_symbol_list = []
        self.high_open_symbol_list = []
        self.pre_data_dict = {}
    ...
    def on_day_bar(self, bar):
        ...
        try:
            ...
            if pre_amount > 1 * pow(10, 8) and high_limit_rate > 9 and high_limit_rate < 11 :
                if open_rate > self.down and open_rate < self.up:
                    if low_rate < open_rate - 3:
                        self.touch_limit_count += 1
                        # self.high_open_symbol_list.append(code)
                        if close_rate > open_rate - 3:
                            self.sucess_limit_count += 1
                            # self.high_open_symbol_list.append(code)
    ...
    def syn_backtest(self):
        self.touch_limit_count = 0
        self.sucess_limit_count = 0
        logger.info(f"下限：{self.down}")
        logger.info(f"上限：{self.up}")
        for i in range(0, len(self.trading_date)):
            try:
                ...
                logger.info(f"{date}")
                logger.info(f"连板：{self.sucess_limit_count}")
                logger.info(f"封板成功率：{self.sucess_limit_count / self.touch_limit_count}")
...
if __name__ == '__main__':
    ...
    test = OpenDown(start_date="2016-01-01",
                    end_date="2016-12-30",
                    frequency='daily')
