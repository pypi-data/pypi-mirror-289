        ...
        for i in range(0, len(self.trading_date)):
            try:
                ...
                for symbol in self.code_list:
                    if not (str(symbol).startswith('688') or str(symbol).startswith('30')):
                        bar = self.today_data[self.today_data['code'] == symbol][-1]
                        self.pick_symbol(bar)

                self.history_N_data = get_bars(self.symbol_list, end_time=date, count=5)
...
def n_limit():
    ...
    #               end_date=end_time,
    #               frequency='daily')
    test = NLimit(start_date='2023-03-01',
                  end_date='2023-03-28',
                  frequency='daily')
