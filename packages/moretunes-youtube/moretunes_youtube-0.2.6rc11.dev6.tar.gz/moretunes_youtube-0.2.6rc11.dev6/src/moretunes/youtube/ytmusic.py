...
from moretunes.youtube.helpers import (
    SUPPORTED_LANGUAGES,
    SUPPORTED_LOCATIONS,
    USER_AGENT,
    YTM_BASE_API,
...
from .exceptions import WrongAuthType, API404Exception
...
class YTMusicBase:
    def __init__(
        ...
        """
        self.use_req_sess = requests_session
        self.rebuilds = rebuilds  #: controls connection error client rebuilds
        self.warnings_enabled = warnings_enabled  #: controls verbosity of warning messages
        ...
        if isinstance(requests_session, requests.Session):
            ...
            # override rebuilds if a dedicated session is being used
            self.rebuilds = 0
        else:
            self.init_session()
    ...
    def init_session(self):
        if self.use_req_sess:  # Build a new session.
    ...
    @property
    def market(self):
        return self.context['context']['client'].get('gl')
    ...
    def _disconnect_wrapper(func):
        def wrapped(self, *args, **kwargs):
            try:
                ...
                if self.rebuilds:
                    print(f'CONNECTION ERROR :: ATTEMPTING REBUILD :: [{self.rebuilds}]')
                    self.rebuilds -= 1
                    self.init_session()
    ...
    def _send_request(self, endpoint: str, body: Dict, additional_params: str = "") -> Dict:
        ...
        if response.status_code >= 400:
            if response.status_code == 404:
                raise API404Exception('Resource not found.')
