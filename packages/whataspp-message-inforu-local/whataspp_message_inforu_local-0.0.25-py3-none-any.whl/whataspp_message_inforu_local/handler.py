...
#TODO What is the capabilities of this?
# @handler_decorator(logger)
#TODO insertMessageRecievedToTheDatabase
#TODO request_parameters_dict
def createHandler(request_parameters: dict) -> list[int]:
    ...
    logger.start(object={"request_parameters": request_parameters})
    generic_crud_ml_instance = GenericCRUDML(default_schema_name="message")
    message_data_list: list[dict] = __extract_message_data(request_parameters)
    messages_ids_list: list = []
    for message_data in message_data_list:
        message_id = generic_crud_ml_instance.insert(
...
"""
def __extract_message_data(request_parameters: dict) -> list[dict]:
    """Extract message data from the payload"""
    logger.start(object={"request_parameters": request_parameters})
    message_data_list: list[dict] = []
    message_customer_id = request_parameters.get("CustomerId")
    message_project_id = request_parameters.get("ProjectId")
    phones_local = PhonesLocal()
    profiles_local = ProfilesLocal()

    request_parameters_data: list[dict] = request_parameters.get("Data")
    if not request_parameters_data:
        logger.end(object={"message_data_list": message_data_list})
        return []
    for request_parameters_data_item in request_parameters_data:
        message_data: dict = {}
        message_data["inforu_customer_id"] = message_customer_id
        message_data["inforu_project_id"] = message_project_id
        message_data["inforu_type"] = request_parameters_data_item.get("Type")
        message_data["body"] = request_parameters_data_item.get("Message")
        message_data["inforu_channel"] = request_parameters_data_item.get("Channel")
        message_data["actual_channel_id"] = WHATSAPP_CHANNEL_ID
        message_data["inforu_network"] = request_parameters_data_item.get("Network")
        message_data["inforu_incoming_json"] = json.dumps(request_parameters)
        receiver_phone_number = request_parameters_data_item.get("ShortCode")
        message_data["inforu_short_code"] = receiver_phone_number
        message_data["inforu_application_id"] = request_parameters_data_item.get("ApplicationID")
        message_data["inforu_customer_param"] = request_parameters_data_item.get("CustomerParam")
        message_data["inforu_mo_session_id"] = request_parameters_data_item.get("MoSessionId")
        message_data["inforu_keyword"] = request_parameters_data_item.get("Keyword")
        sender_phone_number = request_parameters_data_item.get("Value")
        message_data["inforu_value"] = sender_phone_number
