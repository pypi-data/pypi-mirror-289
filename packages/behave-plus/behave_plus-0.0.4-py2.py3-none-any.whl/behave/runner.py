...
class Runner(ModelRunner):
    ...
    def run_with_paths(self):
        ...
        try:
            if self.config.job_language is not None:
                job_language = self.config.job_language
            else:
                environment_parameters = {}
                env_path = os.path.join(self.base_dir, self.config.environment_file)
                exec(open(env_path).read(), environment_parameters)
                job_language = environment_parameters['job_language']
        except:
            job_language = None
        features = parse_features(feature_locations, language=self.config.lang, job_language=job_language)
