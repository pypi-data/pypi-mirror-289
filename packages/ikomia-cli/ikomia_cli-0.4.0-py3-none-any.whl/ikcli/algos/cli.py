...
def cli_algo_add(algos, namespace, name, visibility):
    ...
    with ikcli.utils.rich.HTTPTransferProgress("Package", algo.get_http_request(), download=False, upload=True):
        package = algo.upload()
...
def cli_algo_update(algos, name):
    """Update remote algo NAME with local informations and upload local code."""
    # Get algo
    try:
        algo = algos.get(name=name)
    except ikcli.net.api.exceptions.ObjectNotFoundException:
    ...
    # Update with local informations and push to remote server
