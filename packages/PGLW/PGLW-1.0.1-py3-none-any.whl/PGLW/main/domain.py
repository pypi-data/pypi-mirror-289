    ...
    with FortranFile(filename, mode="w") as f:
        ...
        for name, block in sorted(domain.blocks.items()):
            if exclude_ghost:
                ...
                x = np.append(x, block.y[1:-1, 1:-1, 1:-1].flatten(order="F"))
                x = np.append(x, block.z[1:-1, 1:-1, 1:-1].flatten(order="F"))
            else:
    ...
    for name, dom in sorted(domain.blocks.items()):
        ...
        if imin > 0 or imax != -1:
            ...
            if twoD:
                ...
                for i in range(dom.x[imin:imax, imin:imax, 0].size):
                    fid.write(
                        "1   %24.18e   %24.18e   %24.18e\n"
                        % (domX[i], domY[i], domZ[i])
                    )
        else:
            ...
            for i in range(dom.x[:, :, 0].size):
                if domZ[i] < zmin or domZ[i] > zmax:
                    fid.write(
                        " 103   %24.18e   %24.18e   %24.18e\n"
                        % (domX[i], domY[i], domZ[i])
                    )
                else:
                    fid.write(
                        " %i   %24.18e   %24.18e   %24.18e\n"
                        % (attr[i], domX[i], domY[i], domZ[i])
                    )
...
class Domain(object):
    ...
    def add_domain(self, d):
        for name, b in sorted(d.blocks.items()):
            ...
            bb = copy.deepcopy(b)
            # bb._n = self.nb
            # bb.name = self._check_name(bb.name)
