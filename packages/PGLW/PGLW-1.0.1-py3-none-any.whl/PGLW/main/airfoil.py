...
class AirfoilShape(Curve):
    ...
    def join_seg_points(self, segs=[], te_seg=None):
        ...
        if te_seg is not None:
            points = np.append(
                te_seg.points[self.te_isplit : -1], points, axis=0
            )
            points = np.append(
    ...
    def redistribute(
        ...
        # add trailing edge segment if added
        if hasattr(self, "te_seg"):
            points = self.join_seg_points(
                segs=[self.main_seg], te_seg=self.te_seg
            )
        else:
            points = self.main_seg.points
    ...
    def add_LE_topography(
        ...
        if hasattr(self, "te_seg"):
            points = self.join_seg_points(
                segs=[self.eroded_main_seg], te_seg=self.te_seg
            )
            clean_points = self.join_seg_points(
                segs=[self.clean_main_seg], te_seg=self.te_seg
            )
            for i in range(2):
                self._eroded_seg_info[i] += self.te_isplit
        else:
            points = eroded_main
            clean_points = main_seg.points.copy()
        self.points = points
