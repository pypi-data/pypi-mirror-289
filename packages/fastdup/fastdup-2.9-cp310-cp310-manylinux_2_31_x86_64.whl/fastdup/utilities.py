...
def package_webdataset(input_dir, work_dir, output_dir, _images_per_tar = 10000, image_suffix = ['.jpg', '.png'], num_threads=32, limit=None, exclude_dups=None, exclude_bad=None):
    ...
    elif not input_dir.endswith(".csv"):
    ...
    print(image_files[:10])
    groups = [(i, input_dir, output_dir, image_files[i:i + images_per_tar]) for i in
              range(0, len(image_files), images_per_tar)]
    ...
    with ThreadPoolExecutor(num_threads) as executor:
        executor.map(process_group, zip(*groups))
