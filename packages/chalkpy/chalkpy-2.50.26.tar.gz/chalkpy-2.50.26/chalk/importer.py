...
supported_aggs = ("sum", "count", "mean", "std", "var", "min", "max", "approx_count_distinct")
...
def _check_types(
    ...
    if aggregation == "std" or aggregation == "var" or aggregation == "mean":
        if this_annotation is not float:
            raise ChalkParseError(
                f"feature '{feature_name}' should be a 'float' for '{aggregation}' aggregation; found '{this_annotation.__name__}'"
            )

    elif aggregation == "min" or aggregation == "max":
        if this_annotation != joined_annotation:
            raise ChalkParseError(
                f"feature '{feature_name}' of type '{this_annotation.__name__}' should match joined feature '{joined_feature.name}' of type '{joined_annotation.__name__}' for '{aggregation}' aggregation"
            )
...
def _parse_agg_function_call(expr: Underscore | None) -> Tuple[str, Underscore]:
    ...
    return aggregation, function_attribute._chalk__parent  # pyright: ignore[reportPrivateUsage]
...
def parse_grouped_window(f: Feature) -> WindowConfigResolved:
    ...
    if isinstance(has_many_parent, UnderscoreItem):
        ...
        has_many_parent = has_many_parent._chalk__parent  # pyright: ignore[reportPrivateUsage]
...
def extract_filters_and_projections(
    ...
    keys = expr._chalk__key  # pyright: ignore[reportPrivateUsage]
...
def clean_filters(joined_class: Type[Features], filters: list[UnderscoreBinaryOp]) -> List[Filter]:
    parsed_filters = []
    for filt in filters:
        op = filt._chalk__op  # pyright: ignore[reportPrivateUsage]
        left = filt._chalk__left  # pyright: ignore[reportPrivateUsage]
        right = filt._chalk__right  # pyright: ignore[reportPrivateUsage]
        ...
        if isinstance(left, UnderscoreAttr):
            left_attr = left._chalk__attr  # pyright: ignore[reportPrivateUsage]
        ...
        if isinstance(right, UnderscoreAttr):
            right_attr = right._chalk__attr  # pyright: ignore[reportPrivateUsage]
...
def parse_windowed_materialization(f: Feature) -> WindowConfigResolved | None:
    ...
    if isinstance(getitem_expression, UnderscoreItem):
        ...
        keys = getitem_expression._chalk__key  # pyright: ignore[reportPrivateUsage]
        if not isinstance(keys, tuple):
            keys = (keys,)
        ...
        if isinstance(agg_on, UnderscoreAttr):
            aggregated_value = agg_on._chalk__attr  # pyright: ignore[reportPrivateUsage]
            if not isinstance(agg_on._chalk__parent, UnderscoreRoot):  # pyright: ignore[reportPrivateUsage]
    ...
    if bucket_duration is None:
        raise ChalkParseError(f"No bucket duration was found for the window {timedelta_to_duration(f.window_duration)}")
