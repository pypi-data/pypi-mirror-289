...
logger = logging.getLogger("mw_python_sdk")
...
class DatasetConstructor:
    ...
    data_files: Set[DatasetFile] = field(default_factory=set)
...
def upload_file(
    ...
    _update_dataset(
        id, new_dataset_files, commit_message=f"添加文件 {path_in_dataset}", token=token
    )
...
def download_file(
    ...
    try:
        ...
        slk_path = _download_single_file(
            download_url, id, filename, etag, commit, cache_dir, local_dir
        )
    except NoCredentialsError:
        print("Error: AWS credentials not available.")
    except PartialCredentialsError:
...
class DatasetIterator:
    ...
    def __next__(self):
        if not self.current_batch and not self.finished:
            ...
            logger.debug(f"Processed: {self.processed_count} / Total: {self.total}")
...
def list_all_dataset(title: str = "") -> List[Dataset]:
    """
    列出所有满足给定标题的数据集
