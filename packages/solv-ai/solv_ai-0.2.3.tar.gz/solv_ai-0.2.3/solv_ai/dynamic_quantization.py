...
class DynamicQuantizedLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.layer(x.dequantize().long())  # Convert back to LongTensor for the model
        ...
        return torch.quantize_per_tensor(output.float(), scale=0.1, zero_point=0, dtype=torch.qint8).dequantize()