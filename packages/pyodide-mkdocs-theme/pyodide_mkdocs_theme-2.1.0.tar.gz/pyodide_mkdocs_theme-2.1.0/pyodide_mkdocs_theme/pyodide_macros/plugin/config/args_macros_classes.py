...
class ArgConfigDumpable:
    ...
    """ Path to access the wanted string, for values depending on the language. """
    ...
    __LANG_DEFAULTS: ClassVar[Dict[str, 'ArgConfigDumpable']] = {}


    @classmethod
    def update_lang_defaults(cls, env:'PyodideMacrosPlugin'):
        for arg in cls.__LANG_DEFAULTS.values():
            arg._build_default_lang(env.lang, env)


    def _build_default_lang(self, lang:Lang=Lang(), env:Optional['PyodideMacrosPlugin']=None):
            prop,msg = self.lang_default_access.split('.')
            value = getattr( getattr(lang, prop), msg)
            self.default = value
            if env:
                obj = env.config
                for prop in self.setter_path:
...
class ArgConfigNotIde(ArgConfig):
    """ Just to avoid to put the argument each time... """
    def __post_init__(self):
        super().__post_init__()
        self.ide_link = False
