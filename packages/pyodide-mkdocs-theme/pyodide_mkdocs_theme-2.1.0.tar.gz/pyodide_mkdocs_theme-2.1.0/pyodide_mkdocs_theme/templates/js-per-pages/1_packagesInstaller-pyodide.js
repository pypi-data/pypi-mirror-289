...
const installPythonPackages=(function(){
   ...
   *          sys.modules.
   *        - This makes them visible as "installed" on a next import attempt...
   *        - ...which may occur right after the current call because imports in terminals commands
   *          amy go through env, then envTerm, and BOTH will check if an import has to be done in
   *          the cmd.
   *    * Packages with different installation and import names (ex: Pillow vs PIL):
   *        Packages whose the installation name differ from the import name have to be
   ...
   *        - Then the redactor's import from env or env_term will determine if the module ends up
   *          in the global scope or not.
   *
  ...
  const getImportedPackages = featureRunner("alreadyImported", out=>new Set(out.split(' ')) )
  ...
  const enforceImports = []   // python imports to make sure python_libs are imported (or other stuffs...)

  const importFinalizer = (name, custom_code="") =>{
    const conf = installConfigFor(name)
    const importCode = custom_code || `import ${ conf.toImport }`
  ...
  return async (runner, code, _runtime, _isFromEnv)=>{
    ...
    const importedModules  = getImportedPackages()
    const missingWhiteList = runner.whiteList.filter(name=>!importedModules.has(name))
    const installNeeded    = getWantedImports(code).filter(isNeededAndAllowedInstall(importedModules, runtime, isFromEnv))
                                                   .concat(missingWhiteList)
