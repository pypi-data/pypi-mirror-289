...
class Synthesizer_CNN:
    ...
    def generate(self, shape_x, shape_y):
        ...
        参数:
        - shape_x (int): 生成数据样本的数量
        - shape_y (int): 生成数据样本的维度
        ...
        np.save(os.path.join(self.expPATH, 'synthetic.npy'), gen_samples, allow_pickle=False)
        return gen_samples

    def evaluate(self, origin_data, synthetic_data):
        """
        评估函数，评估原始数据与合成数据的：分布相似度 (MMD) 与脱敏效果 (RMSE)

        参数:
        - origin_data (Dataframe): 原始数据
        - synthetic_data (numpy): 合成数据
