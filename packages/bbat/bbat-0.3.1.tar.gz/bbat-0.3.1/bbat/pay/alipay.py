...
from .python_alipay_sdk import AliPay, utils
...
class Alipay:
    ...
    def initalipay(
        ...
        alipay = AliPay(
            appid=self.ALIPAY_APPID,
            app_notify_url=None,  # 默认回调url
            app_private_key_string=self.app_private_key_string,  # 私钥路径
            alipay_public_key_string=self.alipay_public_key_string,  # 公钥路径(支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥)
            sign_type="RSA2",
            debug=False,  # 上线则改为False , 沙箱True
        )
        return alipay

    def alipay_trade_app(self, out_trade_no, total_amount, notify_url):
        """
        功能：app支付场景
        out_trade_no:本地订单号
        total_amount:需要支付金额
        return 返回支付地址
        """
        alipay = self.initalipay()
        order_string = alipay.api_alipay_trade_app_pay(
            subject=out_trade_no,
            out_trade_no=out_trade_no,
            total_amount=utils.float2dot(total_amount),
    ...
    def alipay_trade_refund(self, trade_no, refund_amount, notify_url):
        """
        功能：支付宝退款申请
        ...
        order_string = alipay.api_alipay_trade_refund(
            trade_no=trade_no,
            refund_amount=utils.float2dot(refund_amount),
            notify_url=notify_url,  # 可选, 不填则使用默认notify url
        )
