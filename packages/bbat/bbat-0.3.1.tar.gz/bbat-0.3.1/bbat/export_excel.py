...
def export_excel(field_data: list, data: list, FileName: str, file_path: str = BASE_DIR):
    """
    Excel导出
    :param data: 数据源
    ...
    wbk.save(path_name)
    return BASE_DIR + 'systemexport' + "/" + monthTime + "/" + FileName
...
def excel_to_data(file_url, field_data):
    ...
    """
    # 读取excel 文件
    data = xlrd.open_workbook(os.path.join(BASE_DIR.replace('\\', os.sep), *file_url.split(os.sep)))
    table = data.sheets()[0]
    # 创建一个空列表，存储Excel的数据
    tables = []
