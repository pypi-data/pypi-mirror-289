...
class WxAppPay:
    ...
    def __init__(self):
        self.appid = self.WXPAY_APPID
        self.appid_app = self.WXPAY_APPID_APP
        self.mchid = self.WXPAY_MCHID
        self.apikey = self.WXPAY_APIKEY
        ...
        self.serial_no = self.WXPAY_SERIAL_NO  # 商户号证书序列号
        self.certificates = []#动态请求微信支付平台证书列表(验证回调应答签名要使用，目前只能通过api访问)
        self.cert_dir = self.WXPAY_CERT_DIR_RESPONSE + '/' if self.WXPAY_CERT_DIR_RESPONSE else None## 微信支付平台证书缓存目录，减少证书下载调用次数。 初始调试时可不设置，调试通过后再设置，示例值：'./cert'
    ...
    def payorder(self, order_no, total, description,attach,notify_url=None):
        ...
        sign = get_sign(sign_str, self.WXPAY_CLIENT_KEY_PATH)
    ...
    def payorder_jsapi(self, order_no, total, description,attach, openid,notify_url=None):
        ...
        sign_str_app = f"{self.WXPAY_APPID}\n{time_stamps_app}\n{random_str_app}\n{package}\n"
        sign_str_app_sign_v3_rsa = get_sign(sign_str_app, self.WXPAY_CLIENT_KEY_PATH)
    ...
    def get_gzh_access_token(self):
        ...
        get_url = api_url.format(self.WX_GZH_APPID, self.WX_GZH_APPSECRET)
    ...
    def build_authorization(self,path,method,mchid,serial_no,datas=None,nonce_str=None):
        ...
        signature = get_sign(sign_str=sign_str, path=self.WXPAY_CLIENT_KEY_PATH)
