...
class JPush:
    ...
    def jpush_msg(self, id, messages):
        """
        id:要发送的标志数组(这里设置发送标识未userid) ['xxxxxx','xxxxxxx']
        messages:要发送的消息
        """
        _jpush = jpush.JPush(self.appkey, self.secret)
        ...
        try:
            ...
            print("极光推送服务错误，推送给：%s，推送内容：%s" % (str(id), str(e)))


if __name__ == "__main__":
    device_id = '001023123123'  # device_id 需要事先获得
    message = "This is a test"
    JPush(appkey='', secret='').jpush_msg(device_id, message)
