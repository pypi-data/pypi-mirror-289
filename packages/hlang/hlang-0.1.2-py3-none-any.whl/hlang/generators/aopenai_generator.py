...
from hlang.openai_lazy_client import OpenAILazyAClient


class AOpenaiGenerator(ABCChatGenerator):
    def __init__(self, model_name: str, base_url: str):
        self.client = OpenAILazyAClient(base_url=base_url)
        self.model_name = model_name

    async def generate(self, messages: [ChatMessage],
    ...
    async def stream(self, messages: [ChatMessage],
                     config: OpenAIGenerationStreamConfig = OpenAIGenerationStreamConfig()
                     ) -> AsyncIterator[str]:
        iterator = await self.client.chat.completions.create(
            messages=[m.to_dict() for m in messages],
            model=self.model_name,
            frequency_penalty=config.frequency_penalty,
            function_call=config.function_call,
