...
{Fore.WHITE}{Style.BRIGHT}Usage:{Style.RESET_ALL}       {Fore.LIGHTCYAN_EX}gmap2baidu [-r] (-wgs | -gcj | -bd) latitude longitude{Fore.RESET}
...
def print_urls(coords, url_list):
    [print(f"{url['name']}: {url['url'].format(lat=coords.lat, lon=coords.lon)}") for url in url_list]
