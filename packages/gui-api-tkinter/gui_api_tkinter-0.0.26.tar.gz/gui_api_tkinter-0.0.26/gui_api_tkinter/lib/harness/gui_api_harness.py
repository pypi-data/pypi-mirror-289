...
class GuiApiHarness:
    ...
    # --------------------
    def menu_state(self, menu_path: list):
        rsp = Response('menu_state')
        ...
        item = self._search_for_menu(self._content, menu_path)
    ...
    # --------------------
    def _search_for_menu(self, content, menu_path):
        if content is None or \
                menu_path is None or \
                len(menu_path) == 0:
            return None

        # assume root is first item in content
        root = content[0]
        # assume menu is the first item in the root
        menus = root['children'][0]

        path_index = 0
        done = False
        while not done:
            if menus['class'] != 'Menu':
                # self.logger.info(f'DBG first child is not "Menu": {menus}')
                return None

            for posn in range(len(menus['menu'])):
                sys.stdout.flush()

                menuitem = menus['menu'][posn]
                # self.logger.info(f'DBG searching for index:{menu_path[path_index]} in menuitem:{menuitem["label"]} idx:{menuitem["index"]}')
                if menuitem['index'] != menu_path[path_index]:
                    continue

                # at this point found a match
                # self.logger.info(f'DBG found match: menuitem:{menuitem["index"]} path:{menu_path[path_index]}')

                if path_index == len(menu_path) - 1:
                    # it's the last in the path
                    # self.logger.info(f'DBG last in path: {menuitem}')
                    return menuitem

                # found it but there's more in the menu_path, so move on to the next submenu
                path_index += 1
                menus = menus['children'][0]
                break
