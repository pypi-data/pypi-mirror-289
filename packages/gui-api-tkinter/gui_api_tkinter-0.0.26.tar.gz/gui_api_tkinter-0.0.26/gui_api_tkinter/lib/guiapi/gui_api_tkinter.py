...
class GuiApiTinker:
    ...
    def menu_invoke(self, cmd):
        ...
        ok = False
        # uncomment to debug
        # svc.logger.info(f'DBG guiapi {cmd["menu_path"]}')
        for posn, index in enumerate(cmd['menu_path']):
            # uncomment to debug
            # import tkinter as tk
            # try:
            #     name = f'{item.entrycget(item.index(index), "label")}'
            # except tk.TclError:
            #     name = "unknown"
            # try:
            #     state = f'{item.entrycget(item.index(index), "state")}'
            # except tk.TclError:
            #     state = "unknown"
            # svc.logger.info(f'DBG    guiapi [{posn}] index:{index} item.index:{item.index(index)} {name} {state}')

            # check if this index is the last one in the menu_path and it matches
            if posn == len(cmd['menu_path']) - 1 and index == item.index(index):
                # svc.logger.info(f'DBG    guiapi invoke {index} of {cmd["menu_path"]}')
                item.invoke(index)
                ok = True
                break

            # uncomment to debug
            # svc.logger.info(f'DBG guiapi getting children')
        ...
        # uncoment to debug
        # svc.logger.info(f'DBG guiapi exiting: {rsp}')
        return rsp
