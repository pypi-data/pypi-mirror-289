...
from virtmat.language.utilities.types import typemap, settype, checktype, DType
...
from virtmat.language.utilities import ioops
...
def series_value(self):
    ...
    if self.need_input:
        retval = ioops.load_value(self.url, self.filename, self.type_)
...
def bool_str_array_value(self):
    ...
    if self.url or self.filename:
        return ioops.load_value(self.url, self.filename, self.type_)
...
def plain_type_value(self):
    ...
    if self.need_input:
        self.__value = ioops.load_value(self.url, self.filename, self.type_)
        self.need_input = False
    return self.__value
...
def quantity_value(self):
    ...
    if self.need_input:
        val = ioops.load_value(self.url, self.filename, self.type_)
        self.need_input = False
        return val
