    ...
    def _update_models_old_vars(self, vary, df_old, model_str):
        ...
        def get_types(dfr):
            ...
            for col in dfr.columns:
                ...
                self.logger.debug('dfr: %s', formatter(dfr[col]))
                self.logger.debug('dfr_types_set: %s', dfr_types_set)
                assert len(dfr_types_set) == 1
                dfr_types[col] = next(iter(dfr_types_set))
            return dfr_types

        # start check type
        if get_types(df_norm) != get_types(df_old):
            msg = f'type mismatch in vary: {get_types(df_norm)} vs. {get_types(df_old)}'
            self.logger.error(msg)
            raise VaryError(msg)
        # end check type
