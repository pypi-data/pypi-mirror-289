...
def get_dialogues(quest, required_dialogues_schema, example, locations, characters):
    ...
    dialogue_trees = []
    for npc in npcs:
        dialogue_tree = dialogue_brainstormer.generate_dialogue(quest, dialogue_cues[npc], example, locations, characters)
        dialogue_tree = dialogue_refiner.check_dialogue_tree(dialogue_tree, example)
        dialogue_tree = dialogue_formatter.get_formatted_dialogue_tree(dialogue_tree)
        dialogue_trees.append(dialogue_tree)
    
    print(npcs)
    for dialogue_tree in dialogue_trees:
        print(dialogue_tree)
