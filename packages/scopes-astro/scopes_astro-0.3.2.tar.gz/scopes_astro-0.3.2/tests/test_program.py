...
def test_program_equality(example_instrument):
    program1 = Program(progID="Prog123", instrument=example_instrument, priority=1)
    program2 = Program(progID="Prog123", instrument=example_instrument, priority=2)
    program3 = Program(progID="Prog456", instrument=example_instrument, priority=1)
    assert program1 == program2  # Same progID, should be equal
    assert program1 != program3  # Different progID, should not be equal


def test_program_equality_different_type(example_instrument):
    program = Program(progID="Prog123", instrument=example_instrument, priority=1)
    assert program != "Not a Program"  # Should not be equal to a different type
