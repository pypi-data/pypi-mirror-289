...
class Selenium(Scraper):
    ...
    def __init__(self, config: Config, headless: bool=True, fullscreen: bool=False, window: dict={}, user_dir: str=None, disable_javascript: bool=False, proxy_address: str=None):
        ...
        if user_dir is not None or url is None:
            chromedriver_autoinstaller.install(path='/usr/local/bin')

        if user_dir is not None:
            self.driver = uc.Chrome(options=options)
        elif url is None:
            self.driver = webdriver.Chrome(service=Service(executable_path='/usr/local/bin/127/chromedriver'), options=options)
        else:
            self.driver = webdriver.Remote(command_executor=f'{url}', options=options)

        if 'browserVersion' in self.driver.capabilities:
            Log.info(f'chrome: {self.driver.capabilities["browserVersion"]}')
        else:
            Log.info(f'driver: {self.driver.capabilities["version"]}')
        
        Log.info('driver = ' + str(self.driver is None))
