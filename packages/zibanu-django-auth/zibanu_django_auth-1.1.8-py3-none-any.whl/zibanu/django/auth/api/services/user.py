        ...
        try:
            ...
            if {"old_password", "new_password"} <= request.data.keys():
                ...
                if user.check_password(request.data.get("old_password")):
                    if user.profile.secure_password and not CodeGenerator.validate_secure_code(
                            request.data.get("new_password"), settings.ZB_AUTH_SECURE_PASSWORD_LENGTH):
                        raise ValidationError(_("The password must contain lowercase, uppercase, digits and symbols."))
