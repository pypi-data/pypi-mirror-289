...
from .common import ordinal, get_fields_aux, set_display_field_aux, standarize_columns, delete_field_aux, add_field_aux
...
from .checks import parse_response, has_multiple_dates
...
def get_date_columns(gdf):
    columns = []
    for column, data_type in gdf.dtypes:
        if str(data_type).find("dsatetime") >= 0:
...
def create_layer(gdf, gis, title=None, folder=None, publish_batch=BATCH_AMOUNT):
    ...
    multiple_dated = has_multiple_dates(sdf)
    if multiple_dated:
        date_columns = get_date_columns(sdf)
        other_dates = date_columns[1: ]
        keep_columns = []
        for column in sdf.columns:
            if column not in other_dates:
                keep_columns.append(column)
        use_sdf = sdf[keep_columns]
    else:
        use_sdf = sdf
        other_dates = []

    layer = use_sdf.spatial.to_featurelayer(gis=gis, title=title, folder=folder)
    object_id = get_objectid_field_aux(layer.layers[0])
    if multiple_dated:
        object_ids = list(range(len(sdf)))
        sdf[object_id] = object_ids
        for date_col in other_dates:
            add_field_aux(layer.layers[0], date_col, "datetime")
            update_layer_aux(layer.layers[0], sdf, [date_col])
    ...
    for i in range(0, len(sdf), publish_batch):
        ...
        if len(date_columns) > 1:
            ...
            for date_col in date_columns:
                update_layer_aux(layer, aux_sdf, [date_col])
...
def read_layer_gdf_aux(layer, query="1=1", geometry_filter=None, geometry_crs=4326, out_fields=None, out_crs=None, geometry_post=True):
    ...
    if len(out_fields) == 1 and (out_fields[0] == "SHAPE" or out_fields[0] == "geometry"):
