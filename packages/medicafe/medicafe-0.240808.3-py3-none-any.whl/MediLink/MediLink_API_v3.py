...
def submit_uhc_claim(client, x12_request_data):
    ...
    # Check if TestMode is enabled and return simulated response if so
    test_mode_response = is_test_mode(client, body, 'claim_submission')
...
def get_claim_acknowledgement(client, transaction_id):
    """
    Retrieves the claim acknowledgement details for the given transaction ID.

    :param client: An instance of APIClient
    :param transaction_id: The transaction ID for which claim acknowledgement details are requested
    :return: Response from the API or a dummy response if in test mode
    """
    endpoint_name = 'UHCAPI'
    url_extension = client.config['MediLink_Config']['endpoints'][endpoint_name]['additional_endpoints']['claim_details']
    ...
    # Check if TestMode is enabled and return simulated response if so
    test_mode_response = is_test_mode(client, body, 'claim_details')
    if test_mode_response:
        return test_mode_response
    ...
    try:
        ...
        print("Error during claim acknowledgement retrieval: {}".format(e))
        raise
