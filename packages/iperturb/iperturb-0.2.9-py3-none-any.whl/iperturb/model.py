...
class XDecoder_possion(nn.Module):
    # This __init__ statement is executed once upon construction of the neural network.
    # Here we specify that the neural network has input dimension z2_dim
    # and output dimension num_genes.
    def __init__(self, num_genes, z_dim, hidden_dims1, hidden_dims2):
        super().__init__()
        dims = [z_dim] + hidden_dims1 + [num_genes]
        self.fc = make_fc(dims)
...
class ZEncoder_possion(nn.Module):
    def __init__(self, z_dim, num_genes, hidden_dims1, hidden_dims2):
        super().__init__()
        dims = [num_genes] + hidden_dims1 + [z_dim]
        self.fc = make_fc(dims)
...
class iPerturb_model_init(nn.Module):
    def __init__(self, num_genes, num_batch, latent_dim1=20, latent_dim2=10, latent_dim3=10, alpha=1e-5, scale_factor=1.0, mode = 'lognorm'):
        ...
        if mode == 'lognorm':
            ...
            self.x_decoder = XDecoder_possion(num_genes=num_genes, hidden_dims1=[100], hidden_dims2=[50], z_dim=self.latent_dim1)
    ...
    def model(self, x, one_hot):
        ...
        with pyro.plate("batch", len(x)), poutine.scale(scale=self.scale_factor):
            ...
            if mode == 'lognorm':
                ...
                xu = self.x_decoder(z)
                x = pyro.sample("x", dist.Poisson(xu).to_event(1), obs=x)
...
def model_init_(hyper, latent_dim1, latent_dim2, latent_dim3, optimizer, lr, gamma, milestones, set_seed=123, cuda = False, alpha=0, mode = 'lognorm'):
    ...
    iPerturb_model = iPerturb_model_init(num_genes=num_genes,num_batch=num_batch,latent_dim1=latent_dim1, latent_dim2=latent_dim2, latent_dim3=latent_dim3, scale_factor=scale_factor,alpha=alpha, mode = mode)
    if cuda == True:
...
def RUN_model(epochs,svi,scheduler,dataloader):
    ...
    for epoch in range(epochs):
        ...
        for x,one_hot,_ in dataloader:
            ...
            params = [p for p in pyro.get_param_store().values() if p.grad is not None]
            clip_grad_norm_(params, max_norm=5.0) # gradiant cut off
...
# 总的RUN
def RUN(datasets,iPerturb_model,svi,scheduler,epochs,hyper,cuda,batch_size=100,if_likelihood=False):
    ...
    # 重构
    mode = iPerturb_model.mode
    if mode == 'lognorm':
        x_pred = iPerturb_model.x_decoder(z_pred)[0]
    elif mode == 'possion':
        x_pred = iPerturb_model.x_decoder(z_pred)
    z_s = iPerturb_model.ts_encoder(z_pred)[0]
    t_logits = iPerturb_model.zs_encoder(z_s)
    z_t = iPerturb_model.zs2_encoder(z_s, t_logits)[0]
    ...
    if if_likelihood:
        ...
        mode = iPerturb_model.mode
        # if mode == 'lognorm':
        #     x_pred = iPerturb_model.x_decoder(z_pred_)[0]
        # elif mode == 'possion':
        #     x_pred = iPerturb_model.x_decoder(z_pred_)
    ...
    # 创建 AnnData 对象
    # datasets = utils.construct_anndata(data = x_pred, columns_names = hyper['var_names'], index_names = hyper['index_names'], raw=datasets)  
    datasets.X = x_pred
