...
def process_batch_X(datasets):
    ...
    if isinstance(datasets.X, csr_matrix):
        datasets.layers['X'] = datasets.X.toarray()
    else:
        datasets.layers['X'] = datasets.X
        
    x = torch.tensor(datasets.layers['X'])
    x_list = [torch.tensor(datasets.layers['X'][datasets.obs['batch'] == label]) for label in batch_labels] # 各个条件的数据
...
def create_hyper(datasets,batch_size=100):
