    ...
    if _match(_filename, include) and not _match(_filename, exclude):
        if os.path.isfile(filename):
            ...
            try:
                ...
                removed.append(f'(not removed) {filename}')
...
def _clean(dirpath: str = None, patterns: list[str] = None):
    if not dirpath:
        return bold('No directory specified. ').red()
    if not os.path.exists(dirpath):
        return bold(f'Directory {dirpath} does not exist. ').red()
    if not patterns:
        return bold('No patterns specified. ').red()
