...
class SecureEnv:
    ...
    def decrypt(self, env_variable):
        ...
        if self.mode == "encrypted":
            if self.check_env_exists(env_variable): #checks if the environment variable exists
                return self.cipher.decrypt(os.environ[env_variable]).decode()
            elif self.check_secure_keys(env_variable): #checks if the key is present in the file secure_keys.json
