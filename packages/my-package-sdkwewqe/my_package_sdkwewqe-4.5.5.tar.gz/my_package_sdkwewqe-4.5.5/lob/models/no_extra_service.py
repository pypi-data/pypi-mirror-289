...
class NoExtraService(object):

    """Implementation of the 'no_extra_service' model.

    TODO: type model description here.

    Attributes:
        tracking_events (List[TrackingEventNormal]): An array of tracking
            events ordered by ascending `time`.
        cards (List[Card]): An array of cards associated with a specific
            letter
        buckslips (List[Buckslip]): An array of buckslip(s) associated with a
            specific letter
    ...
    def __init__(self,
        ...
        """Constructor for the NoExtraService class"""
    ...
    def from_dictionary(cls,
        ...
        if 'cards' in dictionary.keys():
            cards = [Card.from_dictionary(x) for x in dictionary.get('cards')] if dictionary.get('cards') else None
        else:
            cards = APIHelper.SKIP
        if 'buckslips' in dictionary.keys():
            buckslips = [Buckslip.from_dictionary(x) for x in dictionary.get('buckslips')] if dictionary.get('buckslips') else None
        else:
            buckslips = APIHelper.SKIP
    ...
    def validate(cls, dictionary):
        ...
        if isinstance(dictionary, cls):
            return APIHelper.is_valid_type(value=dictionary.color,
        ...
        return APIHelper.is_valid_type(value=dictionary.get('color'),
                                       type_callable=lambda value: isinstance(value, bool)) \
