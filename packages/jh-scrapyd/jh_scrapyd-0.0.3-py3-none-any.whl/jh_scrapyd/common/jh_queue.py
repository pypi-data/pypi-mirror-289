...
class RedisQueue(Queue):
    ...
    def put(self, z_key: str, key: str, data: dict, score: float = 1) -> bool:
        ...
        try:
            ...
            self.storage.hset(name=z_key_str, mapping=hash_data_encode(data))
            return True
    ...
    def pop(self, z_key: str, desc: bool = True):
        try:
            ...
            return hash_data_decode(result)
        except redis.RedisError as e:
    ...
    def list(self, z_key: str, desc: bool = True):
        try:
            ...
            # 结果集
            result = [
                hash_data_decode(self.storage.hgetall(self._gen_hash_key(z_key, item[0])))
