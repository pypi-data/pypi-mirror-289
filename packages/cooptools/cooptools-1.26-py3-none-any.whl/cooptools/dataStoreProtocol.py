import uuid
from typing import Protocol, TypeVar, List, Iterable, Tuple, Dict
...
class ItemAlreadyInStoreException(Exception):
    def __init__(self, item: Storable):
        err = f"item {item.id()} already in store"
        logger.error(err)
        super().__init__(err)
...
class ItemNotInStoreException(Exception):
    def __init__(self, item: Storable):
        err = f"item {item.id()} not in store"
        logger.error(err)
        super().__init__(err)
...
class DataStoreProtocol(Protocol):
    ...
    def add(self,
            items: Iterable[Storable]) -> Dict[UniqueIdentifier, Storable]:
        pass
    ...
    def remove(self,
               items: Iterable[Storable] = None,
               cursor_range: Tuple[int, int] = None,
               ids: Iterable[UniqueIdentifier] = None) -> Dict[UniqueIdentifier, Storable]:
        pass
...
class InMemoryDataStore:
    ...
    @property
    def Store(self) -> Dict[int, Tuple[UniqueIdentifier, Storable]]:
        return dict(self._store)

    @property
    def IdKeyStore(self) -> Dict[UniqueIdentifier, Storable]:
        return {
    ...
    def _subset_of_store(self,
                         ...
                         cursor_range: Tuple[int, int] | Iterable = None) -> Dict[UniqueIdentifier, Storable]:
        if ids is not None:
        ...
        raise ValueError(f"At least one of ids, cursors must be non-none")

    def add(self,
            items: Iterable[Storable]) -> Dict[UniqueIdentifier, Storable]:
        for item in items:
            if item.id() in self.IdKeyStore.keys():
                raise ItemAlreadyInStoreException(item)

        new = {self._increment_cursor(): (x.id(), x) for x in items}
        self._id_cursor_map = {**self._id_cursor_map, **{v[0]: k for k, v in new.items()}}
        self._store = {**self._store, **new}
        return self._subset_of_store(ids=[x.id() for x in items])

    def update(self,
               items: Iterable[Storable]) -> Dict[UniqueIdentifier, Storable]:
        for item in items:
            if item.id() not in self.IdKeyStore.keys():
                raise ItemNotInStoreException(item)

        for item in items:
            self._store[self._id_cursor_map[item.id()]] = (item.id(), item)

        return self._subset_of_store(ids=[x.id() for x in items])

    def add_or_update(self,
                      items: Iterable[Storable]) -> Dict[UniqueIdentifier, Storable]:
        to_add = [item for item in items if item.id() if item.id() not in self.IdKeyStore.keys()]
        to_update = [item for item in items if item.id() if item.id() in self.IdKeyStore.keys()]
    ...
    def remove(self,
        ...
        if items is not None:
            return self.remove(ids=[x.id() for x in items])
