...
class PlotlyTA(PltTA):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.charting_settings = kwargs.get("charting_settings", None)
        self.theme = self.setup_theme(
            chart_style=getattr(self.charting_settings, "chart_style", ""),
            user_styles_directory=getattr(
                self.charting_settings, "user_styles_directory", ""
            ),
        )
        self._locate_plugins(getattr(self.charting_settings, "debug_mode", False))
        if self.plugins:
            self.add_plugins(self.plugins)
        self.inchart_colors = self.theme.get_colors()
    ...
    def init_plot(self, symbol: str = "", candles: bool = True) -> OpenBBFigure:
        ...
        """
        fig = OpenBBFigure(
            charting_settings=self.charting_settings, create_backend=True
        )
        ...
        cc_linewidth = (
            0.8
            if self.df_stock is not None and len(self.df_stock) > 500
            else 0.9 if self.intraday else 1.1
        )
        if candles:
            fig.add_candlestick(
            ...
            fig.add_scatter(
                ...
                name=f"{symbol} {self.df_stock[self.close_column].name.title()}",
                connectgaps=True,
                row=1,
                col=1,
                secondary_y=False,
