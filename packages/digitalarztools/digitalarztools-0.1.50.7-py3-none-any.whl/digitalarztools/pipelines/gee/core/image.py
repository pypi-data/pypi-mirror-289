...
class GEEImage:
    ...
    @staticmethod
    def get_image_stats( image, scale = None, reducer='all', bands=None) ->dict:
        stats = {}
        roi = image.geometry()
        if roi is None:  # or geometry.type().getInfo() == 'GeometryCollection':
            raise Exception("Region is not set for this image use image.clip()")
        if bands is None:
        ...
        for band in bands:
            ...
            if reducer == 'all':
                for reducer in reducers:
                    stats[band][reducer] = image.select(band).reduceRegion(
