...
# ------------------------------------------------------------------
# File Name:        arbitrary_deriv_zhxyao.py
# Author:           Jiwei Huang
# Version:          0.0.1
# Created:          2023/08/05
# Description:      Main Function: 对带有噪音的数据进行任意阶求导。
#                                  该算法由广西科技大学“姚志湘”老师开发，
#                                  本模块代码是依据“姚志湘”老师写的matlab
#                                  代码经转译和整理而来。
#                   Outer Parameters: xxxxxxx
# Class List:       xxxxxxx
# Function List:    deriv_gl_zhxyao(data_y, deriv_order) --
#                        基于格伦瓦尔德·莱特尼科夫（Grunwald-Letnikov,GL）定义，
#                        计算指定数据（data_y）的指定阶（deriv_order）导数。
#                   sech(x) -- 计算指定自变量x的"双曲正割函数"值。
#                   sech_numpy(x, *args, **kwargs) -- 计算指定自变量x的
#                        "双曲正割函数"值。此方法基于numpy.cosh函数实现。
#                   quasi_sech(x, peak_width, peak_steepness) --
#                        计算指定自变量x的"拟双曲正割函数"值。此方法基于sech(x)。
#                   quasi_sech_ifft0(peak_width, peak_steepness) --
#                        生成“拟双曲正割函数”数据，并对其进行逆傅里叶变换，
#                        返回变换后数据的实数部分。
#                   quasi_sech_ifft(peak_width, peak_steepness,
#                                          **kwargs) --
#                        生成“拟双曲正割函数”数据，并对其进行逆傅里叶变换，
#                        返回变换后数据的实数部分。
#                   deriv_quasi_sech0(data_y, deriv_order,
#                                            peak_width, peak_steepness) --
#                        基于“拟双曲正割函数”，计算指定数据（data_y）的
#                        指定阶（deriv_order）导数。
#                   deriv_quasi_sech(data_y, deriv_order,
#                                            peak_width, peak_steepness,
#                                            **kwargs) --
#                        基于“拟双曲正割函数”，计算指定数据（data_y）的
#                        指定阶（deriv_order）导数。
#                   deriv_quasi_sech0_reviews(data_y, deriv_order,
#                                     peak_width, peak_steepness, data_x,
#                                     **kwargs) --
#                        基于“拟双曲正割函数”，计算指定数据（data_y）的
#                        指定阶（deriv_order）导数。
#                   deriv_quasi_sech_reviews(data_y, deriv_order,
#                                     peak_width, peak_steepness, data_x,
#                                     **kwargs) --
#                        基于“拟双曲正割函数”，计算指定数据（data_y）的
#                        指定阶（deriv_order）导数。
...
#       Jiwei Huang        0.0.1         2023/08/05     create
#       Jiwei Huang        0.0.1         2023/11/09     revise
# ------------------------------------------------------------------
# 导包 ==============================================================
import os
import math
...
# 定义 ==============================================================
def deriv_gl_zhxyao(data_y: npt.ArrayLike, deriv_order: float) \
        -> npt.NDArray[np.float64]:
    """
    基于格伦瓦尔德·莱特尼科夫（Grunwald-Letnikov,GL）定义，
    计算指定数据（data_y）的指定阶（deriv_order）导数。

    此算法由广西科技大学“姚志湘”老师开发，对应matlab代码为：glfd.m
    ...
    y_len = len(y_arr)
    w = np.ones(y_len, dtype=np.float64)
    res = np.zeros(y_len, dtype=np.float64)
    for j in range(1, y_len):
        w[j] = w[j - 1] * (1 - (deriv_order + 1) / j)
    for j2 in range(y_len):
        res[j2] = np.dot(w[0:(j2 + 1)], y_arr[0:(j2 + 1)][::-1])
    return res


def sech(x: npt.ArrayLike) -> npt.NDArray[np.float64]:
    """
    计算指定自变量x的"双曲正割函数"值。
    ...
    with np.errstate(all='ignore'):
        ...
        res = 2.0 / (np.exp(x_arr) + np.exp(-x_arr))
        return res


def sech_numpy(x: npt.ArrayLike, *args, **kwargs) -> npt.NDArray[np.float64]:
    """
    计算指定自变量x的"双曲正割函数"值。
    ...
    :param x: 自变量。
    :param args: 可选参数，将被直接传入numpy.cosh函数。
    :param kwargs: 可选关键字参数，将被直接传入numpy.cosh函数。
    :return: 函数值。
    """
    with np.errstate(all='ignore'):
        x_arr = np.array(x, dtype=np.float64)
        return 1.0 / np.cosh(x_arr, *args, **kwargs)


def quasi_sech(x: npt.ArrayLike, peak_width: float, peak_steepness: float):
    """
    计算指定自变量x的“拟双曲正割函数”值。
...
def quasi_sech_ifft0(peak_width: float, peak_steepness: float) \
        -> Tuple[
...
def quasi_sech_ifft(peak_width: float, peak_steepness: float, **kwargs) \
        -> Tuple[
            npt.NDArray[np.float64], npt.NDArray[np.float64],
            npt.NDArray[np.float64]
        ]:
...
def deriv_quasi_sech0(data_y: npt.ArrayLike, deriv_order: float,
                      peak_width: float, peak_steepness: float) -> Tuple[
    npt.NDArray[np.float64], npt.NDArray[np.float64],
    npt.NDArray[np.float64], npt.NDArray[np.float64], npt.NDArray[
        np.float64]]:
    """
    ...
    mr = np.concatenate([y_arr[0] * np.ones(100),
                        ...
                        axis=0)
    # print("mr:{}".format(mr))
    sc0, igg, r = quasi_sech_ifft0(peak_width, peak_steepness)
    # print("sc0:{}".format(sc0))
    sc = deriv_gl_zhxyao(sc0, deriv_order)
    # print("sc:{}".format(sc))
...
def deriv_quasi_sech(data_y: npt.ArrayLike, deriv_order: float,
    ...
    mr = np.concatenate([y_arr[0] * np.ones(y_side_len),
                        ...
                        axis=0)
    sc0, igg, r = quasi_sech_ifft(peak_width, peak_steepness, **kwargs)
    sc = deriv_gl_zhxyao(sc0, deriv_order)
...
def deriv_quasi_sech0_reviews(data_y: npt.ArrayLike, deriv_order: float,
    ...
    t = deriv_quasi_sech0(data_y, 0, peak_width, peak_steepness)
    s = deriv_quasi_sech0(data_y, deriv_order, peak_width, peak_steepness)
...
def deriv_quasi_sech_reviews(data_y: npt.ArrayLike, deriv_order: float,
    ...
    t = deriv_quasi_sech(data_y, 0, peak_width, peak_steepness, **kwargs)
    s = deriv_quasi_sech(data_y, deriv_order, peak_width, peak_steepness, **kwargs)
    fs = FittingStatistics(data_y, t[0], x=data_x)
    # 数据输出 -----------------------------------------
    data_dict = {'x': data_x, 'y': data_y, 't': t[0], 's': s[0]}
    if 'is_data_out' in kwargs and kwargs['is_data_out']:
