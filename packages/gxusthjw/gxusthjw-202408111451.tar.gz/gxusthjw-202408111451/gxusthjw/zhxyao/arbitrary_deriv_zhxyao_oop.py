...
# ------------------------------------------------------------------
# File Name:        arbitrary_deriv_zhxyao_oop.py
# Author:           Jiwei Huang
# Version:          0.0.1
# Created:          2023/10/14
# Description:      Main Function: 对带有噪音的谱数据进行任意阶求导。
...
#       Jiwei Huang        0.0.1         2023/10/14     create
...
from ..commons import DataTable, FittingStatistics
...
# -------------------------------------------------------------------
def envelope_ifft_zhxyao_oop(envelope: Optional[EnvelopeFunction] = None,
...
def arbitrary_deriv_zhxyao_oop(data_y: npt.ArrayLike, deriv_order: float,
    ...
    with np.errstate(all='ignore'):
        mr = np.concatenate([data_y[0] * np.ones(y_side_len),
                            ...
                            axis=0)
        sc0, igg, r_values = envelope_ifft_zhxyao_oop(envelope, r_values, n_ifft, side_len)
...
class ArbitraryOrderDerivative(object):
    ...
    def deriv(self, y: npt.ArrayLike, deriv_order: float = 0,
        ...
        t, t_deriv, t_envelope_ifft, t_envelope, t_r_values = arbitrary_deriv_zhxyao_oop(
            y, 0, envelope, deriv_alg,
            r_values, n_ifft, side_len, y_side_len,
            deriv_slice_len)
        s, s_deriv, s_envelope_ifft, s_envelope, s_r_values = arbitrary_deriv_zhxyao_oop(
        ...
        data_table = DataTable(x, y, t, t_deriv, t_envelope_ifft, t_envelope, t_r_values,
                               ...
                               col_names=('x', 'y', 't', 't_deriv', 't_envelope_ifft',
