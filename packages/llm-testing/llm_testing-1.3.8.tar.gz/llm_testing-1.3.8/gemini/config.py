...
def set_slack_api_key(api_key: str):
    os.environ["SLACK_API_KEY"] = api_key

def get_slack_api_key():
    api_key = os.getenv("SLACK_API_KEY")
    if not api_key:
        raise ValueError("SLACK_API_KEY environment variable not set. Use `set_slack_api_key` to configure it.")
    return api_key
