...
from NaxToPy.Modules.Fasteners._N2PFastenerAnalysis.Core.Classes.ARBolt import ARBolt
from NaxToPy.Modules.Fasteners._N2PFastenerAnalysis.Core.Classes.AR1DElement import AR1DElement
from NaxToPy.Modules.Fasteners._N2PFastenerAnalysis.Core.Classes.ARPlate import ARPlate
from NaxToPy.Modules.Fasteners._N2PFastenerAnalysis.Core.Classes.ARPlateElements import ARPlateElements
from NaxToPy.Modules.Fasteners._N2PFastenerAnalysis.Core.Functions.ARAuxiliarFunctions import point_in_element, equal_points, same_direction
import numpy as np
...
def get_bolts(model: N2PModelContent) -> list[ARBolt]:
    ...
    for fastener in fasteners_list:
        ...
        if fastener.TypeElement == "CFAST":
            ...
            # Get CFAST card
            fastener_card = [i for i in model.ModelInputData.get_cards_by_field(str(fastener.ID), 0, 1) if
            ...
            # Property of CFAST
            prop_card = [i for i in model.ModelInputData.get_cards_by_field(str(fastener.Prop), 0, 1) if
                         i.CharName == 'PFAST']
            if len(prop_card) == 1:
