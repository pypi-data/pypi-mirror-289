...
def handle_duplicated_output(dst: str) -> str:
    if os.path.exists(dst):
        ...
        if ow_mode == "yes":
            ...
            logger.warning(
                f"{orig_dst} has been renamed to {dst} because OVERWRITE = {ow_mode}",
                category=UserWarning,
            )
...
def write_VESTA(
    ...
    Interpolating volumetric data...
    volumetric data interpolated
    ==> /home/xbugs/work/dspaw-manual-cn/dspawpy_proj/dspawpy_tests/outputs/doctest/my_potential.txt...
...
def _get_structure_grid_vd(
    ...
    """
    dspaw output vd array in F or C order according to data_type.
    """
    import numpy as np
    ...
    if in_filename.endswith(".h5"):
        ...
        if data_type == "rho" or data_type == "rhoBound":
            vd = np.asarray(data["/Rho/TotalCharge"]).reshape(grid, order="F")
        elif data_type == "pcharge":
            vd = np.asarray(data["/Pcharge/1/TotalCharge"]).reshape(grid, order="F")

        elif data_type == "potential":
            if subtype is None:
                subtype = "TotalElectrostaticPotential"
            vd = np.asarray(data[f"/Potential/{subtype}"]).reshape(grid, order="C")
        elif data_type == "elf":
            vd = np.asarray(data["/ELF/TotalELF"]).reshape(grid, order="C")
        else:
        ...
        if data_type == "rho" or data_type == "rhoBound":
            ...
            vd = np.asarray(data["Pcharge"][0]["TotalCharge"]).reshape(grid, order="F")
        elif data_type == "potential":
            if subtype is None:
                subtype = "TotalElectrostaticPotential"
            vd = np.asarray(data["Potential"][subtype]).reshape(grid, order="C")
        elif data_type == "elf":
            vd = np.asarray(data["ELF"]["TotalELF"]).reshape(grid, order="C")
        else:
            raise NotImplementedError(
