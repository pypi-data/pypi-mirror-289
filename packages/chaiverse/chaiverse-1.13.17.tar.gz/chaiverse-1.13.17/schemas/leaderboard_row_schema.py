...
class BaseLeaderboardRow(BaseModel, extra=Extra.allow):
    ...
    alignment_samples: Optional[int] = Field(default_factory=lambda: None)
...
class BasicLeaderboardRow(BaseLeaderboardRow):
    ...
    def ineligible_reason(self) -> Optional[str]:
        ...
        if self.model_name and self.developer_uid == config.E2E_DEVELOPER_UID:
            reason = 'model is only for e2e test'
        return reason


class FunctionLeaderboardRow(BaseLeaderboardRow):
