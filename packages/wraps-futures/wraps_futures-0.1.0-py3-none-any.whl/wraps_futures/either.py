...
if TYPE_CHECKING:
    from wraps_core.typing import EitherAsyncCallable

    from wraps_futures.typing import FutureEitherCallable
...
class FutureEither(Future[Either[L, R]]):
    """[`Future[Either[L, R]]`][wraps_futures_core.future.Future],
...
"""An alias of [`FutureEither.from_either`][wraps_futures.either.FutureEither.from_either]."""

future_left = FutureEither.from_left
"""An alias of [`FutureEither.from_left`][wraps_futures.either.FutureEither.from_left]."""

future_right = FutureEither.from_right
"""An alias of [`FutureEither.from_right`][wraps_futures.either.FutureEither.from_right]."""


def wrap_future_either(function: EitherAsyncCallable[P, L, R]) -> FutureEitherCallable[P, L, R]:
    """Wraps an asynchronous `function` returning [`Either[L, R]`][wraps_core.either.Either]
    into a function returning [`FutureEither[L, R]`][wraps_futures.either.FutureEither].
