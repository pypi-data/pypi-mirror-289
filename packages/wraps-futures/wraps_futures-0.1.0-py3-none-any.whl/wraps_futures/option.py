...
from wraps_core.option import NULL, Option, Some
from wraps_core.result import Result
from wraps_futures_core.future import Future
from wraps_futures_core.reawaitable import ReAwaitable

if TYPE_CHECKING:
    from wraps_core.typing import OptionAsyncCallable

    from wraps_futures.typing import FutureOptionCallable
...
class FutureOption(Future[Option[T]]):
    """[`Future[Option[T]]`][wraps_futures_core.future.Future],
...
"""An alias of [`FutureOption.from_option`][wraps_futures.option.FutureOption.from_option]."""

future_some = FutureOption.from_some
"""An alias of [`FutureOption.from_some`][wraps_futures.option.FutureOption.from_some]."""

future_null = FutureOption.from_null
"""An alias of [`FutureOption.from_null`][wraps_futures.option.FutureOption.from_null]."""


def wrap_future_option(function: OptionAsyncCallable[P, T]) -> FutureOptionCallable[P, T]:
    """Wraps an asynchronous `function` returning [`Option[T]`][wraps_core.option.Option] into a function
    returning [`FutureOption[T]`][wraps_futures.option.FutureOption].
