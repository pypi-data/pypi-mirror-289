...
def load_inputs(image, input_height, input_width, normalize_255=True, transpose=True, is_rgb=False):
    if is_rgb:
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
...
def load_inputs_std_mean(image, input_height, input_width, std=1, mean=0, transpose=True, is_rgb=False):
    if is_rgb:
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    image = cv2.resize(image, (input_width, input_height))
    image = np.ascontiguousarray(image, dtype=np.float32)  # uint8 to float32
