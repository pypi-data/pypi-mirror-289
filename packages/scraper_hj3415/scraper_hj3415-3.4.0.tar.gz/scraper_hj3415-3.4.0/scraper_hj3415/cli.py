...
def nfs():
    ...
    if args.spider in spiders.keys():
        if len(args.targets) == 1 and args.targets[0] == 'all':
            ...
            is_success = spiders[args.spider](*all_codes)
            if args.noti and is_success:
            ...
            if is_valid:
                is_success = spiders[args.spider](*args.targets)
                if args.noti and is_success:
                    noti.telegram_to('manager', f"{len(args.targets)}갯수 코드의 {args.spider}를 저장했습니다.")
            else:
