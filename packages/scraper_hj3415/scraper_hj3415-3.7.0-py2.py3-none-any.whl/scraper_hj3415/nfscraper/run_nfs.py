...
from db_hj3415 import mongo, redis_mongo
...
def initialing_db():
    # 몽고db 초기화
    addr = db_cli.load_mongo_addr()
    mongo.Base.initialize_client(mongo.connect_mongo(addr))

    # redis 초기화
    client = redis.Redis(host=db_cli.load_redis_addr(), port=6379, db=0)
    redis_mongo.Redis.initialize_client(client)
