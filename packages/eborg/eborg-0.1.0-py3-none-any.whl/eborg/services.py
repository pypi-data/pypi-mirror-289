...
from .model import Book, Highlight, Section
...
def parse_toc_as_list(
    toc: List[Union[epub.Link, tuple]], book: epub.EpubBook
) -> List[Section]:
...
def convert_toc_to_org(book: Book, initial_level: int = 1) -> str:
    ...
    def format_toc(sections: List[Section], level: int):
...
def update_orgmode_file(books_orgmode_path: Path, toc_org: str) -> None:
    ...
    if books_orgmode_path.exists():
        try:
            with open(books_orgmode_path, "r") as file:
    ...
    try:
        ...
        with open(books_orgmode_path, "w") as file:
            file.write(toc_org + "\n\n" + current_content)
    except IOError as e:
...
def mount_ereader(mount_point: Path = Path("/tmp/ebook")) -> None:
    ...
    try:
        subprocess.run(
            ["sudo", "mount", str(device_path), str(mount_point)], check=True
        )
        ...
        raise RuntimeError(
            f"Failed to mount device {device_path}. You might need to run this script with superuser privileges."
        )
...
def copy_epub_to_mount(epub_path: Path, mount_point: Path) -> None:
    ...
    if destination_file.exists():
        ...
        if destination_file.stat().st_size == epub_path.stat().st_size:
    ...
    try:
        subprocess.run(
            ["sudo", "cp", str(epub_path), str(destination_file)], check=True
        )
        log.info(f"Successfully copied {epub_path} to {destination_file}")
    except subprocess.CalledProcessError as e:
        log.error(f"Failed to copy EPUB file to {destination_file}: {e}")
        raise RuntimeError(f"Failed to copy EPUB file to {destination_file}.")
...
def find_kobo_sqlite(mount_point: Path) -> Optional[Path]:
    ...
    """
    for file in mount_point.rglob("KoboReader.sqlite"):
        return file
    return None
...
def match_highlights_to_sections(book: Book, highlights: List[Highlight]) -> Book:
    """
    Match highlights extracted from an EPUB's SQLite database to sections in
    the book's TOC based on section IDs and update the Book object with these
    highlights.
...
def convert_highlights_to_org(book: Book, initial_level: int = 1) -> str:
    ...
    def format_section(section: Section, level: int) -> str:
        ...
        # Only include the section if it has highlights or if any of its subsections have highlights
