...
from eborg.model import Highlight, Book, Section
from eborg.services import (
    convert_toc_to_org,
    extract_epub_data,
...
@pytest.fixture
def sqlite_temp_db(tmpdir: Path) -> Path:
    """
    ...
    cursor.executemany(
        ...
        [
            (
                "OEBPS/xhtml/Burk_9780735232464_epub3_c02_r1.xhtml#point(/1/4/2/34/3:128)",
                "Highlight 1",
                "Sample Book Title",
            ),
            (
                "OEBPS/xhtml/Burk_9780735232464_epub3_c03_r1.xhtml#point(/2/5/3/45/4:200)",
                "Highlight 2",
                "Sample Book Title",
            ),
            (
                "OEBPS/xhtml/Burk_9780735232464_epub3_c02_r1.xhtml#point(/1/6/2/50/1:300)",
                "Highlight 3",
                "Another Book Title",
            ),
        ],
...
class TestMatchHighlightsToSections:
    ...
    def test_nested_sections(self) -> None:
        ...
        assert len(result.toc[0].highlights) == 1
        assert result.toc[0].highlights[0].text == "Highlight in Chapter 1."
        assert len(result.toc[0].sections[0].highlights) == 1
        assert (
            result.toc[0].sections[0].highlights[0].text == "Highlight in Section 1.1"
        )
...
class TestExtractHighlights:
    def test_basic(self, sqlite_temp_db: Path) -> None:
        ...
        expected_highlights = [
            ...
            Highlight(
                section_id="Burk_9780735232464_epub3_c03_r1.xhtml",
    ...
    def test_no_matches(self, sqlite_temp_db: Path) -> None:
        """
        Test extraction of highlights when there are no matching book titles.
        """
    ...
    def test_empty_text(self, sqlite_temp_db: Path) -> None:
        """
        Test extraction of highlights that ignores the highlights that have empty text fields.
        """
...
class TestUpdateOrgmodeFile:
    ...
    def test_update_orgmode_file_read_error(self, temp_orgmode_file: Path) -> None:
        # Create an empty file to simulate existing Org-mode file
        with open(temp_orgmode_file, "w") as _:
            pass
        # Simulate read error by changing permissions to read-only
        temp_orgmode_file.chmod(0o444)
        toc_org = "* Sample TOC\n"

        with pytest.raises(OSError):
            update_orgmode_file(temp_orgmode_file, toc_org)

        # Restore permissions for cleanup
        temp_orgmode_file.chmod(0o644)
    ...
    def test_update_orgmode_file_no_existing_file(
        ...
        with open(temp_orgmode_file, "r") as file:
            content = file.read()
