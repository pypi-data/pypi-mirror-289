...
class StatusBarWidget extends VDomRenderer<StatusBarWidgetModel> {
    ...
    private _accessDeniedCounter = 0;
    ...
    constructor() {
        ...
        Worker.getInstance().accessDeniedSignal.connect(this._onAccessDenied, this);
    ...
    // Disable auto trigger if there are more than 3 access denied exception
    // in this browser session. This is to prevent client from spamming the service.
    // User can still enable it by clicking the button.
    private _onAccessDenied(sender: any, status: boolean) {
        this._accessDeniedCounter += 1;
        if (this._accessDeniedCounter > 2) {
            AutoTrigger.getInstance().enabled = false;
            saveState(AUTO_SUGGESTION, false);
            this._accessDeniedCounter = 0;
        }
    }
