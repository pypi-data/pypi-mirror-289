...
class ExpressionBoxplotTask(AnalysisTaskBase):
    ...
    def process(self) -> None:
        is_tumor = [not lable.endswith('.N')
                    for lable in self.preprocess_data.index]
        is_normal = [lable.endswith('.N')
                     for lable in self.preprocess_data.index]
        ...
        self.p_value = scipy.stats.ttest_ind(
            self.normal_preprocess_data, self.tumor_preprocess_data).pvalue
        print(self.p_value)
    ...
    def plot(self):
        ...
        boxplot = sns.stripplot(x=tumor_normal_label, y=self.preprocess_data[self.gene_pro_name], data=self.preprocess_data, color='.3',
                                order=["Tumor", "Normal"])
        boxplot.set(
            title=f'{self.cancer_name} protein expression boxplot for {self.gene_name} \np-value = {self.p_value}')
        figPath = os.path.join(
            os.getcwd(), f'{self.cancer_name}_{self.gene_name}_expression_boxplot.png')
        print('figure done')
