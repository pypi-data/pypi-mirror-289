from cmoat.GUI_lib.app_init import packages_check
...
class PackagesInstallGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        denpendence_packages_frm.pack(
            fill=tk.X, expand=True, anchor=tk.N, side=tk.TOP)
        ttk.Label(denpendence_packages_frm,
                  text="Dependence packages:").pack(anchor='w')
        for package in self.dependence_packages:
            ...
            install_label = ttk.Label(
                package_frm, text="Installed" if is_installed else "Not installed")
    ...
    class InstallPackagesDialog(simpledialog.Dialog):
        ...
        def body(self, master):
            ...
            self.prograssbar = ttk.Progressbar(
                master, orient=tk.HORIZONTAL, mode='determinate', maximum=self.package_count, length=500)
            self.prograssbar.pack(fill=tk.BOTH, expand=True)

        def buttonbox(self):
            self.btn = ttk.Button(
                self, text='Start Install', command=self.install_all)
        ...
        def refresh(self):
            self.prompt.config(
                text=f"Installing package [{self.packages[self.package_index]}] {self.package_index}/{self.package_count}...")
            self.update()

    def install_packages(self) -> None:
        dialog = PackagesInstallGUI.InstallPackagesDialog(
            self, 'Installing packages...', self.uninstall_packages)
