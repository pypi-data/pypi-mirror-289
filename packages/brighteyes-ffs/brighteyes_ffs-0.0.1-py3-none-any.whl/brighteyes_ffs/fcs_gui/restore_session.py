...
def restorelib_ffz(libfile, root=0):
    ...
    lib.activeImage = read_text(os.path.join(folder, 'activeImage.txt'), dtype=int)
    ...
    for i in range(Nimages):
        ...
        # add image
        lib.addImage(image, fname)
        imageObj = lib.getImage(i)
        # active file
        imageObj.activeFFS = read_text(os.path.join(imagefolder, 'activeFFS.txt'), dtype=int)
        ...
        for j in range(Nfiles):
            ...
            if -1 in fileObj.airy:
                fileObj.airy = None
            fileObj.activeAnalysis = read_text(os.path.join(filefolder, 'activeAnalysis.txt'), dtype=int)
            md = FFSmetadata()
            md.numberOfPixels = read_text(os.path.join(filefolder, 'numberOfPixels.txt'), dtype=int)
            md.numberOfLines = read_text(os.path.join(filefolder, 'numberOfLines.txt'), dtype=int)
            md.numberOfFrames = read_text(os.path.join(filefolder, 'numberOfFrames.txt'), dtype=int)
            md.rangeX = read_text(os.path.join(filefolder, 'rangeX.txt'), dtype=int)
            md.rangeY = read_text(os.path.join(filefolder, 'rangeY.txt'), dtype=int)
            md.rangeZ = read_text(os.path.join(filefolder, 'rangeZ.txt'), dtype=int)
            md.numberOfDataPoints = read_text(os.path.join(filefolder, 'numberOfDataPoints.txt'), dtype=int)
            md.holdOffx5 = read_text(os.path.join(filefolder, 'holdOffx5.txt'), dtype=int)
            md.holdOff = read_text(os.path.join(filefolder, 'holdOff.txt'), dtype=int)
            md.timeResolution = read_text(os.path.join(filefolder, 'timeResolution.txt'), dtype=float)
            ...
            md.coords = [yc, xc]
            fileObj.metadata = md
            imageObj.ffsList.append(fileObj)
            # check for analysis in file
            fileObj = imageObj.getFFSfile(j)
            ...
            for k in range(Nanalyses):
                ...
                corrObj.activeFit = read_text(os.path.join(analysisfolder, 'activeFit.txt'), dtype=int)
                ...
                corrSett.chunksOff = [int(l) for l in csv2array(os.path.join(analysisfolder, 'chunksOff.csv'))]
                ...
                if len(corrFiles) > 0:
                    corrObj.corrs = corrs
                fileObj.analysisList.append(corrObj)
                # check overall fit
                anObj = fileObj.getanalysis(k)
                ...
                for l in range(Nfits):
                    ...
                    for m in range(Nsinglefits):
                        ...
                        indfitObj.fitfunctionLabel = ffitfunctionLabel
                        indfitObj.fitarray = ffitarray
                        indfitObj.startvalues = fstartvalues
                        indfitObj.paramFactors10 = fparamFactors10
                        ...
                        cfitObj.fitAllCurves.append(indfitObj)
...
def restorelib_ffs(libfile, root=0):
    ...
    with h5py.File(libfile) as f:
        ...
        lib.activeImage = check_none(f['info'].attrs['activeImage'], dtype=int)
        ...
        for i in range(Nimages):
            ...
            fname = f['image' + str(i)].attrs['imageName']
            
            # add image
            lib.addImage(image, fname)
            imageObj = lib.getImage()
            
            # active file
            imageObj.activeFFS = check_none(f['image' + str(i)].attrs['activeFFS'], dtype=int)
            ...
            for j in range(Nfiles):
                ...
                for arg in ['fname', 'label', 'activeAnalysis']:
                    setattr(fileObj, arg, check_none(f[currFile + '/'].attrs[arg]))
                ...
                md = FFSmetadata()
                for key in ['numberOfPixels', 'numberOfLines', 'numberOfFrames', 'rangeX', 'rangeY', 'rangeZ', 'numberOfDataPoints', 'holdOffx5', 'holdOff']:
                    setattr(md, key, check_none(f[currFile].attrs[key], dtype=int))
                
                for key in ['timeResolution', 'dwellTime', 'duration', 'pxsize']:
                    setattr(md, key, check_none(f[currFile].attrs[key], dtype=float))
                ...
                for k in range(Nanalyses):
                    ...
                    corrObj.activeFit = check_none(f[currAnal].attrs['activeFit'], dtype=int)
                    corrSett = CorrSettings()
                    ...
                    gList = check_none(f[currAnal].attrs['listOfG'])
                    ...
                    corrSett.algorithm = getattr(f[currAnal].attrs, 'algorithm', None)
                    ...
                    corrSett.chunksOff = check_none(f[currAnal]['chunksOff'][:], dtype=None)
                    ...
                    for l in range(Nfits):
                        ...
                        for m in range(Nfits3):
                            ...
                            fitSingleCurveFolder = fitAllCurves5 + "/fitSingleCurve" + str(m)
                            ...
                            fparamFactors10 = fitSingleCurve['paramFactors10'][:]
                            
                            ffitfunctionLabel = f[fitSingleCurveFolder].attrs['fitfunctionLabel']
...
def savelib_ffz(FFSlib, root=0, fname=''):
    ...
    if fname is not None:
        ...
        save_text(FFSlib.version, folderName + '/version.txt')
        save_text(FFSlib.notes, folderName + '/notes.txt')
        save_text(FFSlib.activeImage, folderName + '/activeImage.txt')
        Nimages = len(lib)
        for i in range(Nimages):
            if root != 0:
                root.progress = i / Nimages
            ...
            # store image name
            save_text(im.imageName, imageFolder + '/imageName.txt')
            # store image
            array2csv(im.image, imageFolder + '/image.csv', dtype=int)
            array2csv(im.imageRaw, imageFolder + '/imageRaw.csv', dtype=int)
            # store active file
            save_text(im.activeFFS, imageFolder + '/activeFFS.txt')
            # to through ffs files
            Nfiles = len(im.ffsList)
            for j in range(Nfiles):
                if root != 0:
                ...
                save_text(file.activeAnalysis, fileFolder + '/activeAnalysis.txt')
                # save metadata
                md = file.metadata
                ...
                for k in range(Nanalysis):
                    ...
                    # save g list
                    save_text(analysis.settings.listOfG, analysisFolder + '/listOfG.txt')
                    # save average list
                    ...
                    # save chunks off
                    array2csv(analysis.settings.chunksOff, analysisFolder + '/chunksOff.csv')
                    # save active fit
                    save_text(analysis.activeFit, analysisFolder + '/activeFit.txt')
                    # save correlations
                    c = analysis.corrs
                    if c is not None:
                    ...
                    for l in range(Nfits):
                        ...
                        fitAllCurves = fits[l].fitAllCurves # contains 3 fits: central, sum3, sum5
                        ...
                        for m in range(Nfits3):
                            ...
                            save_text(fitSingleCurve.fitfunctionLabel, fitSingleCurveFolder + '/fitfunctionLabel.txt')
                            ...
                            array2csv(fitSingleCurve.paramFactors10, fitSingleCurveFolder + '/paramFactors10.csv')
...
def savelib_ffs(FFSlib, root=0, fname=''):
    ...
    if fname is not None:
        ...
        with h5py.File(fname, "w") as f:
            ...
            inf.attrs['version'] = check_none(FFSlib.version, dtype=str)
            inf.attrs['notes'] = check_none(FFSlib.notes, dtype=str)
            inf.attrs['activeImage'] = check_none(FFSlib.activeImage, dtype=int)
            ...
            for i in range(Nimages):
                ...
                im5.attrs["imageName"] = check_none(im.imageName, dtype=str)
                im5.create_dataset('image',data=im.image, compression="gzip")
                im5.create_dataset('imageRaw',data=im.imageRaw, compression="gzip")
                
                # store active file
                im5.attrs["activeFFS"] = check_none(im.activeFFS)
                ...
                for j in range(Nfiles):
                    ...
                    # save active analysis
                    file5.attrs["activeAnalysis"] = check_none(file.activeAnalysis)
                    ...
                    for k in range(Nanalysis):
                        ...
                        # save list of G
                        print('list of g')
                        print(analysis.settings.listOfG)
                        analysis5.attrs["listOfG"] = check_none(analysis.settings.listOfG)
                        print(analysis5.attrs["listOfG"])
                        # save average
                        ...
                        # save chunks off
                        analysis5.create_dataset("chunksOff",data=analysis.settings.chunksOff)
                        
                        # save active fit
                        analysis5.attrs["activeFit"] = check_none(analysis.activeFit)
                        # save correlations
                        c = analysis.corrs
                        if c is not None:
                            for key in list(c.__dict__.keys()):
                                if key != "dwellTime" and key != "chunksOff":
                                    # DWELLTIME IS ALREADY STORED IN METADATA
                                    print(key)
                        ...
                        for l in range(Nfits):
                            ...
                            for m in range(Nfits3):
                                ...
                                fitSingleCurve5 = fitAllCurves5.create_group("fitSingleCurve" + str(m))
                                fitSingleCurve = fitAllCurves[m]
                                # save all fields
                                ...
                                fitSingleCurve5.create_dataset('paramFactors10',data=fitSingleCurve.paramFactors10)
                                ...
                                fitSingleCurve5.attrs['fitfunctionLabel'] = check_none(fitSingleCurve.fitfunctionLabel)
