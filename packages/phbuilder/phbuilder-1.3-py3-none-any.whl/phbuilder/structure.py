...
class Residue:
    ...
    def __init__(self, atoms: List[str], resname: str, chain: str, resid: int, x: List[float], y: List[float], z: List[float]):
...
class Structure:
    ...
    def write(self, name: str):
        ...
        if extension == ".pdb":
            self._write_pdb(name)
            self._write_record()

        if extension == ".gro":
            self._write_gro(name)
            self._write_record()

    def _read_pdb(self, name: str):
