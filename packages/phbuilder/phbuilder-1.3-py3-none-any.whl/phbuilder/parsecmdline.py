...
def parsecmdline():
    ...
    desc_2 = "phbuilder VERSION 1.3. For the user manual visit https://gitlab.com/gromacs-constantph/phbuilder."
    ...
    parser = ArgumentParser(prog='phbuilder', description=desc_1, epilog=desc_2)
    ...
    parser_1.add_argument('-groups',
                          required=False,
                          dest='groups',
                          action='store',
                          help='[<str>] Specify which titratable residue types to consider. Example: \"ASPT GLUT\" will only consider Asp and Glu residues for CpHMD. Also residue alternative name (e.g. \"ASP GLU\") can be provided.')
    ...
    if vars(CLI) == {}:
        parser.print_help()
        from sys import exit  # Lazy/deffered import.
        exit(0)
