...
class LambdaType:
    ...
    """
    def __init__(self, groupname: str, incl: List[str], pKa: List[float], atoms: List[str], qqA: List[float], qqB: List[float], dvdl: List[float]):
...
class phbuilder(User):
    ...
    def __init__(self, CLI):
        ...
        if (CLI.target == 'gentopol'):
            ...
            self.ph_groups = CLI.groups.split() if CLI.groups else None
    ...
    def parseLambdaGroupTypesFile(self):
        ...
        def defineLambdaType(groupname: str, incl: List[str], pKa: List[float], atoms: List[str], qqA: List[float], qqB: List[float], dvdl: List[float]):
            ...
            # The lines below were added to enable the -groups functionality of
            # gentopol. Effectively, we check if lambda group name is specified on
            # cmdline for -groups (is in the ph_groups list). The check has two stages
            # 1. We check the name directly
            # 2. We check all the alternative names (provided under incl keyword)

            residue_in_groups_flag = True
            # defaults to True, since we consider all titratable residues by default.

            if hasattr(self, 'ph_groups') and self.ph_groups is not None:
                residue_in_groups_flag = False
                if (groupname in self.ph_groups) or (any([x in self.ph_groups for x in incl])):
                    residue_in_groups_flag = True

            if not already_present and residue_in_groups_flag:
    ...
    def checkCPHMDGROMACSLoaded(self) -> None:
        """Check whether the constant-pH beta is active by attempting to run 'gmx cphmd -h'."""

        if run("gmx cphmd -h", shell=True, stdout=-1, stderr=-1).returncode:
            self.warning("The GROMACS constant-pH beta does not appear to be loaded. While this might be fine for gentopol and genparams, it MUST be loaded when using neutralize. Please source or module load the GROMACS constant-pH beta.")
    ...
    def gromacs(self, command: str, stdin: list = [], terminal: bool = False, logFile: str = 'builder.log') -> int:
        ...
        if terminal:
            process = run("gmx " + command, shell=True, env=envDict)
        else:
            with open(logFile, 'a+') as file:
                process = run("gmx " + command, shell=True, stdout=file, stderr=file, env=envDict)
    ...
    def runner(self) -> int:
        ...
        return 0  # phbuilder ran succesfully if we end up here.
    ...
    def gentopol(self):
        ...
        with open('lambdareference.dat', 'w+') as file:
            file.write('resname state# resid chain coordinateFile\n')
    ...
    def neutralize(self):
        ...
        def getcpHMDcharge(name: str, file: str, Structure: Structure, LambdaTypeNames: List[str], constrainCharge: bool):
        ...
        # Molecular weight of water as a global-scope constant.
        # Used to calculate ion concentrations based on solvent volume.
        WATER_MOL_WEIGHT = 0.01808

        def getIonData(Structure: Structure):
            """Retrieve positive ion, negative ion, and salt concentrations from
            the input structure file. Calculation performed by solvent volume."""

            N_pos = self.countRes(Structure, self.d_pname)
            N_neg = self.countRes(Structure, self.d_nname)
            N_salt = min(N_pos, N_neg)
            N_water = self.countRes(Structure, self.d_solname)

            C_pos = N_pos / (WATER_MOL_WEIGHT * N_water)
            C_neg = N_neg / (WATER_MOL_WEIGHT * N_water)
            C_salt = N_salt / (WATER_MOL_WEIGHT * N_water)

            return N_pos, N_neg, N_salt, C_pos, C_neg, C_salt

        def fromSolVol(Structure, concentration: float):
            ...
            return int(round(WATER_MOL_WEIGHT * Nwater * concentration))

        N_pos, N_neg, N_salt, C_pos, C_neg, C_salt = getIonData(pdb)

        if (C_pos or C_neg):  # if we already have some ions...
            self.update(f"Detected {N_pos} {self.d_pname} and {N_neg} {self.d_nname} in {self.d_file}, corresponding to [{self.d_pname}{self.d_nname}] = {C_salt:.3f} mol/L for given solvent volume.")
            self.verbose(f"Individual ion concentrations are [{self.d_pname}] = {C_pos:.3f} mol/L and [{self.d_nname}] = {C_neg:.3f} mol/L.")
        else:
            self.update(f"No previous ions detected in {self.d_file}.")

        # If #salt ions that are already present corresponds to a larger concentration
        # than #salt ions corresponding to the requested concentration, we throw an error.
        if N_salt > fromSolVol(pdb, self.d_conc):
            self.error('Target concentration of {} mol/L is smaller than the current concentration in {} ({:.2f}). Either remove -conc or increase it.'.format(self.d_conc, self.d_file, C_salt))
        ...
        if not (isNeutral and self.d_conc == 0):
            ...
            if self.d_conc != 0:
                factor = fromSolVol(pdb, self.d_conc - C_salt)

                np += factor
                nn += factor

                self.update(f"Requested salt concentration of {self.d_conc} mol/L requires {factor} {self.d_pname}{self.d_nname} for given solvent volume")
        ...
        N_pos, N_neg, N_salt, C_pos, C_neg, C_salt = getIonData(pdb)
        self.update(f"{N_pos} {self.d_pname} and {N_neg} {self.d_nname} for solvent volume in {self.d_output} corresponds to:")
        self.update(f"[{self.d_pname}{self.d_nname}] = {C_salt:.3f} mol/L")
        self.update(f"[{self.d_pname}] = {C_pos:.3f} mol/L")
        self.update(f"[{self.d_nname}] = {C_neg:.3f} mol/L")
    ...
    def writeLambda_mdp(self, Type: str, Structure: Structure, LambdaTypeNames: List[str], constrainCharge: bool):
        ...
        def to_string(Input: list):
            """Internal function to convert a list to a string."""
            return ' '.join([str(float(val)) for val in Input])

        def writeLambdaGroupTypeBlock(number: int, name: str, multistates: int, qqA: List[float], qqB: List[float], pKa: List[float], dvdl: List[float]):
        ...
        def writeResBlock(number: int, name: str, initList: List[float], Edwp: float):
    ...
    def writeLambda_ndx(self, fileName: str, Structure: Structure, LambdaTypeNames: List[str], constrainCharge: bool):
        ...
        def writeTheGroup(number, atomIndexList: List[float]):
    ...
    def automaticLambdaInits(self, pKaList: List[float], systempH: float) -> int:
