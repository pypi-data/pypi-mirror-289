...
class KISSimpleDomesticStock:
    ...
    def get_today_minute_data(self, asset_code: str) -> pd.DataFrame:
        ...
        prev_acc_value = None
        prev_acc_volume = None

        for i in range(len(result)):
            idx = len(result) - i - 1
            curr = result[idx]

            if prev_acc_value is None:
                curr["value"] = curr["acc_value"]
            else:
                curr["value"] = curr["acc_value"] - prev_acc_value

            if prev_acc_volume is None:
                curr["acc_volume"] = curr["volume"]
            else:
                curr["acc_volume"] = curr["volume"] + prev_acc_volume

            prev_acc_value = curr["acc_value"]
            prev_acc_volume = curr["acc_volume"]

        df = pd.DataFrame(result, columns=["time", "open", "high", "low", "close", "volume", "value", "acc_volume", "acc_value"])
