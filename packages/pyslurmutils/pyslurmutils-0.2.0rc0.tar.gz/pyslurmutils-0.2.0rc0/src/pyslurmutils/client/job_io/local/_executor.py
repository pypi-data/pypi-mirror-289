...
from typing import Optional, Callable, Set, Generator, Any
...
class RemoteExecutor(futures.Executor):
    ...
    def _start_worker(self) -> None:
        if self._shutdown_flag:
            # Do not start workers when shutting down
            return False
        thread_name = f"{type(self).__name__}-{len(self._workers)}"
    ...
    def _worker_main(self):
        ...
        try:
            with self.execute_context() as execute:
                    ...
                    try:
                        ...
                        if future.set_running_or_notify_cancel():
                            ...
                            try:
                                ...
                                logger.error(
                                    "%s: failed (task capacity = %s)",
                                    worker.name,
                                    worker.task_capacity,
                                    exc_info=True,
                                )
                                if isinstance(exc, errors.RemoteExit):
                                    exit_reason = "RemoteExit error"
                                    break
                            else:
                                future.set_result(result)
                                logger.info(
                                    "%s: succeeded (task capacity = %s)",
                                    worker.name,
                                    worker.task_capacity,
                                )
                        else:
                            logger.warning(
                                "%s: cancelled (task capacity = %s)",
                                worker.name,
                                worker.task_capacity,
                            )
                    finally:
                ...
                with self._shutdown_lock:
                    ...
                    if not self._shutdown_flag:
                        if self._lazy_scheduling:
                            ...
                            self._start_worker()
                logger.debug("%s: exists", worker.name)
            except BaseException:
    ...
    def execute_context(
        self,
    ) -> Generator[Callable[[callable, tuple, dict, futures.Future], Any], None, None]:
        def execute(task, args, kwargs, future):
