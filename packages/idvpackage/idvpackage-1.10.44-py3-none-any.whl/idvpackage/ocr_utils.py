            ...
            if country == 'IRQ' and data.get('doc_type') == 'national_identity_card' and data.get('nfc_data', ''):
                ...
                try:
                    if len(main_properties['document_numbers']) == 4:
                        main_properties['document_numbers'][3]['value'] = nfc_data['document_numbers']
                        ...
                        main_properties['document_numbers'].append({
                            "type": "value",
                            "value": nfc_data['document_numbers']
                        })
                ...
                nfc_keys = [
                    'date_of_birth', 'date_of_expiry', 'nationality',
                    'gender', 'issuing_country', 'first_name', 'last_name'
                ]

                ## REPLACE DOB, DOE, NATIONALITY, GENDER, ISSUING COUNTRY, NAME  FROM NFC DATA
                for key in nfc_keys:
                    if nfc_data.get(key):
                        if key in ['date_of_birth', 'date_of_expiry']:
                            main_properties[key] = onfido_date_format(get_dates_to_generic_format(nfc_data[key]))
                        else:
                            main_properties[key] = nfc_data[key]
