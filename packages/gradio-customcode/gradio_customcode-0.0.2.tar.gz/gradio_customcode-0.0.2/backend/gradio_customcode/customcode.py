...
from .datamodel import CodeData

if TYPE_CHECKING:
    from gradio.components import Timer


class CustomCode(Component):
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        self.selection = {"from": 0, "to": 0, "text": ""}
        if language not in CustomCode.languages:
    ...
    def preprocess(self, payload: CodeData) -> CodeData:
        """
        preprocess: Converts the value from the format sent by the frontend to the format expected by the python function.
        Called when the component is being used as an input component in an event listener.
        Parameters:
        ...
        """
        code = payload.code
        start, end = payload.selected_code.start, payload.selected_code.end
        selected_text = payload.selected_code.text
        return payload

    def postprocess(self, value: tuple[str] | str | dict | None) -> CodeData:
        """
        postprocess: Converts the value returned by the python function to the format expected by the frontend
        Called when the component is being used as an output component in an event listener.
        Parameters:
        ...
        if value is None:
            ...
            with open(value[0], encoding="utf-8") as file_data:
                return CodeData(code=file_data.read())
        elif isinstance(value, dict):
            return CodeData(code=value["code"], diff=value["diff"])
        else:
            return CodeData(code=value.strip())
