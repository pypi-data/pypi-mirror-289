import { l as _, m as v, s as Ae, t as c, n as Be, P as rt, T as nt, o as E, p as st, a as q, E as H, q as T, e as le, r as it, u as ot, C as at, v as lt, w as ht, x as ft, y as ut, f as Ie, b as dt, z as pt, A as ct, B as he, D as mt } from "./Index-DDP5BmnT.js";
import { htmlCompletionSource as gt, html as kt } from "./index-CN58gyvS.js";
class j {
    ...
    if (y(r.defineNodes)) {
      ...
      for (let L of r.defineNodes) {
        ...
        if (d.some((B) => B.name == k))
          continue;
        S && (s[d.length] = (B, et, tt) => S(et, tt, B.value));
...
const Xe = { resolve: "Emphasis", mark: "EmphasisMark" }, $e = { resolve: "Emphasis", mark: "EmphasisMark" }, P = {}, W = {};
class C {
  constructor(e, r, n, s) {
...
const Z = {
  ...
  LinkEnd(t, e, r) {
    ...
    for (let n = t.parts.length - 1; n >= 0; n--) {
      let s = t.parts[n];
      if (s instanceof C && (s.type == P || s.type == W)) {
        if (!s.side || t.skipSpace(s.to) == r && !/[(\[]/.test(t.slice(r + 1, r + 2)))
          return t.parts[n] = null, -1;
        let i = t.takeContent(n), o = t.parts[n] = It(t, i, s.type == P ? u.Link : u.Image, s.from, r + 1);
        if (s.type == P)
          for (let a = 0; a < n; a++) {
            let l = t.parts[a];
            l instanceof C && l.type == P && (l.side = 0);
          }
        return o.to;
      }
    }
...
class Et {
  ...
  get hasOpenLink() {
    for (let e = this.parts.length - 1; e >= 0; e--) {
      let r = this.parts[e];
      if (r instanceof C && (r.type == P || r.type == W))
        return !0;
    }
    return !1;
  }
...
const Ht = Ae({
  ...
  LinkTitle: c.string,
  Paragraph: c.content
}), vt = new U(new Be(ze).extend(Ht), Object.keys(F).map((t) => F[t]), Object.keys(F).map((t) => Ct[t]), Object.keys(F), xt, Ee, Object.keys(Z).map((t) => Z[t]), Object.keys(Z), []);
function Nt(t, e, r) {
  let n = [];
...
}, Ge = /* @__PURE__ */ ut({ commentTokens: { block: { open: "<!--", close: "-->" } } }), Ve = /* @__PURE__ */ new v(), Ke = /* @__PURE__ */ vt.configure({
  props: [
...
const Vt = /* @__PURE__ */ oe(Ke), Kt = /* @__PURE__ */ Ke.configure([jt, qt, _t, Ut, {
  props: [
    /* @__PURE__ */ Ie.add({
  ...
  let r = q(t), { doc: n } = t, s = null, i = t.changeByRange((o) => {
    ...
    if (h.item && p) {
      ...
      if (w.to >= a || S && S.to < a || l.from > 0 && !/[^\s>]/.test(n.lineAt(l.from - 1).text)) {
        ...
        return h.node.name == "OrderedList" && G(h.item, n, B, -2), b && b.node.name == "OrderedList" && G(b.item, n, B), { range: H.cursor(A + M.length), changes: B };
      } else {
        let b = xe(f, t, l);
        return {
          range: H.cursor(a + b.length + 1),
    ...
    for (; k > l.from && /\s/.test(l.text.charAt(k - l.from - 1)); )
      k--;
    return L = ae(L, t), Wt(h.node, t.doc) && (L = xe(f, t, l) + t.lineBreak + L), d.push({ from: k, to: a, insert: t.lineBreak + L }), { range: H.cursor(k + L.length + 1), changes: d };
  });
    ...
    if (i.empty && Je.isActiveAt(t, i.from)) {
      ...
      if (f.length) {
        ...
        (!h.item || l.from <= h.item.from || !/\S/.test(l.text.slice(0, h.to)))) {
          ...
          if (d < o)
            return { range: H.cursor(d), changes: { from: d, to: o } };
        }
      }
    }
    return n = { range: i };
...
], re = /* @__PURE__ */ kt({ matchClosingTags: !1 });
function ar(t = {}) {
  let { codeLanguages: e, defaultCodeLanguage: r, addKeymap: n = !0, base: { parser: s } = Vt, completeHTMLTags: i = !0 } = t;
  if (!(s instanceof U))
    throw new RangeError("Base parser provided to `markdown` should be a Markdown parser");
  let o = t.extensions ? [t.extensions] : [], a = [re.support], l;
  r instanceof le ? (a.push(r.support), l = r.language) : r && (l = r);
  let f = e || l ? Jt(e, l) : void 0;
  o.push(yt({ codeParser: f, htmlParser: re.language.parser })), n && a.push(it.high(ot.of(rr)));
  let h = oe(s.configure(o));
  return i && a.push(h.data.of({ autocomplete: nr })), new le(h, a);
}
function nr(t) {
...
function sr() {
  if (V)
    return V;
  let t = gt(new at(lt.create({ extensions: re }), 0, !0));
